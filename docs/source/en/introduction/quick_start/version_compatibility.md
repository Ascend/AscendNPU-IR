...
If you want to keep the current CANN version without upgrading or replacing it while still using the new features of AscendNPU IR, you can try the following approach:
...
# Generally, the above steps should make it work. If issues persist, you can try further.
bash cann900/run_package/cann-bisheng-compiler_*.run --noexec --extract=$TMP_PATH
BiShengCompilerPath="${TMP_PATH}/bisheng_compiler" # After CANN 9.1.0, the path is "tools/bisheng_compiler/"
