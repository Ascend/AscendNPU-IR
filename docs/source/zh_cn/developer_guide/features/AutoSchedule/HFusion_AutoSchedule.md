...
3. **Tiling计算与选择**
   - 通过`StmtExprBuilder`与Expr系统，结合静态/动态shape，生成多种候选Tiling方案（TilingCases）。
...
- **Transform Dialect应用**
    - AutoScheduleInterpreter解析调度描述，将其翻译为Transform Dialect操作序列，并对原始HFusion IR进行变换。
...
- **在调度策略中的使用（以AnyPBR为例）**：
    ...
    - 最终输出已满足stride-align约束的TilingCases。
- **位置与时机**：
...
- **calculateTilingImpl()**
    - 构造并返回`TilingComputeFn`：
        ...
        - 为不同场景（小规模 / 大规模 / 高维 / 低维等）生成多套TilingCases以供选择。
...
- 通过TilingInfo + Expr系统，将维度大小、对齐规则、动态shape等统一抽象为表达式。
- 对于静态shape：
    - 表达式可以在编译期求值，折叠为常量Tiling参数；
- 对于动态shape：
    - 在host生成的Tiling函数中，根据实际输入shape求值；
