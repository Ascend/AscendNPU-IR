...
本文介绍HIVM中的`AutoBindSubBlock` Pass。该Pass通过Cube-Vector 1:2切分，针对CV类kernel进行优化。在阅读本文之前，建议先阅读CV Optimization了解CV编译相关术语。
...
1. 选轴分析失败，没有可切分的平行轴
2. `BubbleUpExtractSlicePattern`中途遇到了未支持的Op
