...
本文档从宏观角度介绍AscendNPU IR中Cube-Vector（CV）优化的整体流程。CV优化针对Cube（矩阵乘单元）和Vector（向量运算单元）两类核心的协同工作，在HIVM（Hybrid Intelligence Virtual Machine，混合智能虚拟机）层进行一系列变换，以提升混合内核（Mix Kernel）的执行效率。
