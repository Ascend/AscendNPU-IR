...
1. 下载CANN包：需下载CANN Toolkit包及与硬件对应的`ops`包，可从[昇腾社区 CANN 下载页](https://www.hiascend.com/cann/download)获取。
...
A：在调用`build-tools/build.sh`脚本时添加`-r`选项，重新执行CMake并生成新的`build.ninja`文件。
