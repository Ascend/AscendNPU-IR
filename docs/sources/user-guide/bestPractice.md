# 性能优化案例
## 1.  $\color{red}{tiling策略(计算)}$ 
### 待补充
## 2.  $\color{red}{kernel昇腾亲和改写(计算)}$ 
### 待补充
## 3.  性能优化选项使用(编译器)
### 常见案例
...
## $\color{red}{ub overflow类问题(计算) }$ 
### tiling值
### stride_align类
### 待补充

##  $\color{red}{d-cache类(计算)}$ 
### 无效地址访问
### 可能是offset负数
### i32溢出
### 待补充
...
## load行为(编译器)
### load非预期引入vtranspose op导致ub overflow(编译器)
### Load隐式转置(编译器)
### 数据load方式不合理
### 待补充


## $\color{red}{baseline(计算)}$ 
