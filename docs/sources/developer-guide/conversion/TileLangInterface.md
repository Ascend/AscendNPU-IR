...
```mlir
memref.copy %src, %dst : memref<32xf16, strided<[1], offset: ?>, #hivm.address_space<gm>> to memref<32xf16, strided<[1]>>
```
...
### npuir_sync_block_set

```python
tl.npuir_sync_block_set
```
...
### npuir_sync_block_wait

```python
tl.npuir_sync_block_wait
```
