...
// RUN: %if asserts %{bishengir-opt %s -hacc-append-device-spec=target=Ascend950PR_950z -hivm-plan-memory --debug-only="vf-inplace-reuse" -split-input-file -verify-diagnostics 2>&1 | FileCheck %s -check-prefix=CHECK-DEBUG %}
...
// test avoid inplace-reuse when gen/kill value reach store/load through subviews or inplace-reuse vf operands
// CHECK-LABEL: func.func @not_inplace_reuse_when_reach_load_and_store_0
// CHECK-DAG: %[[CONST0:.*]] = arith.constant 1024 : i64
// CHECK-DAG: %[[CONST1:.*]] = arith.constant 512 : i64
// CHECK-DAG: %[[CONST2:.*]] = arith.constant 0 : i64
// CHECK-DAG: hivm.hir.pointer_cast(%[[CONST2]])
// CHECK-DAG: hivm.hir.pointer_cast(%[[CONST1]])
// CHECK-DAG: hivm.hir.pointer_cast(%[[CONST0]])
...
module attributes {hacc.target = #hacc.target<"Ascend950PR_950z">} {
  func.func @_attn_fwd_mix_aic(%arg0: memref<16x16xf16, #hivm.address_space<gm>>,
  ...
  func.func @_attn_fwd_mix_aiv(%arg0: memref<16x16xf16, #hivm.address_space<gm>>,
...
// expected-error@below {{ub overflow, requires 1966080 bits while 1769472 bits avaliable!}}
