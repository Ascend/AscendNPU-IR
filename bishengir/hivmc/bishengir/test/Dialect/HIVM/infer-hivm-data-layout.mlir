...
// -----
func.func @mm_erase_convert_layout_op(%arg0: memref<128x128xf16, strided<[?, ?], offset: ?>, #hivm.address_space<gm>>, %arg1: i32, %arg2: i1) attributes {WorkspaceArgIdx = 0 : i64, func_dyn_memref_args = dense<[false, true, true, true, true, false, false, false, false, false]> : vector<10xi1>, global_kernel = "local", hacc.entry, hacc.function_kind = #hacc.function_kind<DEVICE>, hivm.func_core_type = #hivm.func_core_type<AIC>, mix_mode = "mix"} {
  %true = arith.constant true
  %c0 = arith.constant 0 : index
  %c128 = arith.constant 128 : index
  ...
  // CHECK: hivm.hir.nd2nz
  hivm.hir.load ins(%subview : memref<?x128xf16, strided<[?, ?], offset: ?>, #hivm.address_space<gm>>) outs(%subview_1 : memref<?x128xf16, strided<[128, 1]>, #hivm.address_space<cbuf>>)
...
// -----
// CHECK-LABEL:   func.func @test_infer_data_layout_basic_mmadmx
// CHECK-SAME:                                                   %[[VAL_0:.*]]: i32, %[[VAL_1:.*]]: i32, %[[VAL_2:.*]]: i32
// CHECK:           %[[VAL_3:.*]] = arith.constant 128 : index
// CHECK:           %[[VAL_4:.*]] = arith.constant 256 : index
// CHECK:           %[[VAL_5:.*]] = arith.constant 128 : index
// CHECK:           %[[VAL_6:.*]] = affine.apply #{{.*}}%[[VAL_4]], %[[VAL_5]]
// CHECK:           %[[VAL_7:.*]] = affine.apply #{{.*}}%[[VAL_4]], %[[VAL_5]]]
// CHECK:           %[[VAL_8:.*]] = arith.constant 16 : index
// CHECK:           %[[VAL_9:.*]] = arith.constant 16 : index
// CHECK:           %[[VAL_10:.*]] = memref.alloc(%[[VAL_7]], %[[VAL_6]], %[[VAL_8]], %[[VAL_9]]) {alignment = 64 : i64} : memref<?x?x?x?xf8E5M2>
// CHECK:           %[[VAL_11:.*]] = scf.for %[[VAL_12:.*]] = %[[VAL_0]] to %[[VAL_1]] step %[[VAL_2]] iter_args(%[[VAL_13:.*]] = %[[VAL_10]]) -> (memref<?x?x?x?xf8E5M2>)  : i32 {
// CHECK:             %[[VAL_14:.*]] = arith.constant 256 : index
// CHECK:             %[[VAL_15:.*]] = arith.constant 128 : index
// CHECK:             %[[VAL_16:.*]] = affine.apply #{{.*}}%[[VAL_14]], %[[VAL_15]]
// CHECK:             %[[VAL_17:.*]] = affine.apply #{{.*}}%[[VAL_14]], %[[VAL_15]]
// CHECK:             %[[VAL_18:.*]] = arith.constant 32 : index
// CHECK:             %[[VAL_19:.*]] = arith.constant 16 : index
// CHECK:             %[[VAL_20:.*]] = memref.alloc(%[[VAL_16]], %[[VAL_17]], %[[VAL_19]], %[[VAL_18]]) : memref<?x?x?x?xf8E5M2>
// CHECK:             %[[VAL_21:.*]] = arith.constant 128 : index
// CHECK:             %[[VAL_22:.*]] = arith.constant 128 : index
// CHECK:             %[[VAL_23:.*]] = affine.apply #{{.*}}%[[VAL_21]], %[[VAL_22]]
// CHECK:             %[[VAL_24:.*]] = affine.apply #{{.*}}%[[VAL_21]], %[[VAL_22]]
// CHECK:             %[[VAL_25:.*]] = arith.constant 16 : index
// CHECK:             %[[VAL_26:.*]] = arith.constant 32 : index
// CHECK:             %[[VAL_27:.*]] = memref.alloc(%[[VAL_24]], %[[VAL_23]], %[[VAL_25]], %[[VAL_26]]) : memref<?x?x?x?xf8E5M2>
// CHECK:             %[[VAL_28:.*]] = arith.constant 256 : index
// CHECK:             %[[VAL_29:.*]] = arith.constant 4 : index
// CHECK:             %[[VAL_30:.*]] = affine.apply #{{.*}}%[[VAL_28]], %[[VAL_29]]
// CHECK:             %[[VAL_31:.*]] = affine.apply #{{.*}}%[[VAL_28]], %[[VAL_29]]
// CHECK:             %[[VAL_32:.*]] = arith.constant 16 : index
// CHECK:             %[[VAL_33:.*]] = arith.constant 2 : index
// CHECK:             %[[VAL_34:.*]] = memref.alloc(%[[VAL_30]], %[[VAL_31]], %[[VAL_32]], %[[VAL_33]]) : memref<?x?x?x?xi8>
// CHECK:             %[[VAL_35:.*]] = arith.constant 128 : index
// CHECK:             %[[VAL_36:.*]] = arith.constant 4 : index
// CHECK:             %[[VAL_37:.*]] = affine.apply #{{.*}}%[[VAL_35]], %[[VAL_36]]
// CHECK:             %[[VAL_38:.*]] = affine.apply #{{.*}}%[[VAL_35]], %[[VAL_36]]
// CHECK:             %[[VAL_39:.*]] = arith.constant 16 : index
// CHECK:             %[[VAL_40:.*]] = arith.constant 2 : index
// CHECK:             %[[VAL_41:.*]] = memref.alloc(%[[VAL_37]], %[[VAL_38]], %[[VAL_39]], %[[VAL_40]]) : memref<?x?x?x?xi8>
// CHECK:             %[[VAL_42:.*]] = arith.cmpi eq, %[[VAL_12]], %[[VAL_1]] : i32
// CHECK:             %[[VAL_43:.*]] = hivm.hir.mmadmxL1 ins(%[[VAL_20]], %[[VAL_27]], %[[VAL_34]], %[[VAL_41]], %[[VAL_42]], %[[VAL_3]], %[[VAL_3]], %[[VAL_3]] : memref<?x?x?x?xf8E5M2>, memref<?x?x?x?xf8E5M2>, memref<?x?x?x?xi8>, memref<?x?x?x?xi8>, i1, index, index, index) outs(%[[VAL_13]] : memref<?x?x?x?xf8E5M2>) -> memref<256x128xf8E5M2>
// CHECK:             scf.yield %[[VAL_13]] : memref<?x?x?x?xf8E5M2>
// CHECK:           }
// CHECK:           return
// CHECK:         }
...
module {
  func.func @test_infer_data_layout_basic_mmadmx(%arg0 : i32,
    ...
    %ret = scf.for %iv = %arg0 to %arg1 step %arg2 iter_args(%l0c = %alloc) -> (memref<256x128xf8E5M2>) : i32 {
      ...
      %scaleA = memref.alloc() : memref<256x4xi8>
      %scaleB = memref.alloc() : memref<128x4xi8>
      %init_cond = arith.cmpi eq, %iv, %arg1 : i32
      %idk = hivm.hir.mmadmxL1 ins(%l1A, %l1B, %scaleA, %scaleB, %init_cond, %c128, %c128, %c128 :
                             memref<256x128xf8E5M2>, memref<128x128xf8E5M2>, memref<256x4xi8>, memref<128x4xi8>, i1, index, index, index)
