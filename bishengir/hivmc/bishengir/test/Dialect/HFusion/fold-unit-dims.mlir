...
// CHECK: %[[EXTRACT:.*]] = tensor.extract_slice %[[ARG0:.*]][0, 0] [%[[ARG2:.*]], 1] [1, 2] : tensor<341x2xf32> to tensor<?x1xf32>
// CHECK: tensor.collapse_shape %[[EXTRACT]] {{\[\[}}0, 1]] : tensor<?x1xf32> into tensor<?xf32>
...
// CHECK: %[[COLLAPSED:.*]] = tensor.collapse_shape %[[EXTRACTED]] {{\[\[}}0, 1, 2]] : tensor<1x1x1xi32> into tensor<1xi32>
// CHECK: bufferization.materialize_in_destination %[[COLLAPSED]]
...
// CHECK: scf.for
// CHECK-SAME: (tensor<20x32x4xf32>, tensor<32x4x8xf32>)
// CHECK-NOT: tensor.collapse_shape
func.func @scffor_iterargs(%33: tensor<32x4x8xf32>, %64: index) -> tensor<1x32x4xf32> {
  %c0_i32 = arith.constant 0 : i32
  ...
  %3 = linalg.fill ins(%cst_0 : f32) outs(%2 : tensor<32x4xf32>) -> tensor<32x4xf32>
  %8 = tensor.empty() : tensor<20x32x4xf32>
  %expanded_4 = tensor.expand_shape %8 [[0], [1], [2, 3]] output_shape [20, 32, 4, 1] : tensor<20x32x4xf32> into tensor<20x32x4x1xf32>
  %9 = linalg.fill ins(%cst_0 : f32) outs(%expanded_4 : tensor<20x32x4x1xf32>) -> tensor<20x32x4x1xf32>
  %34:2 = scf.for %arg16 = %c0_i32 to %c20_i32 step %c1_i32 iter_args(%arg17 = %9, %arg19 = %33) -> (tensor<20x32x4x1xf32>, tensor<32x4x8xf32>)  : i32 {
    %reduced_25 = linalg.reduce ins(%arg19 : tensor<32x4x8xf32>) outs(%3 : tensor<32x4xf32>) dimensions = [2]
      (%in: f32, %init: f32) {
        %68 = arith.addf %in, %init : f32
        linalg.yield %68 : f32
      }
    %collapsed_29 = tensor.collapse_shape %arg17 [[0], [1], [2, 3]] : tensor<20x32x4x1xf32> into tensor<20x32x4xf32>
    %67 = arith.index_cast %arg16 : i32 to index
    %inserted_slice = tensor.insert_slice %reduced_25 into %collapsed_29[%67, 0, 0] [1, 32, 4] [1, 1, 1] : tensor<32x4xf32> into tensor<20x32x4xf32>
    %expanded_30 = tensor.expand_shape %inserted_slice [[0], [1], [2, 3]] output_shape [20, 32, 4, 1] : tensor<20x32x4xf32> into tensor<20x32x4x1xf32>
    %66 = linalg.elemwise_unary {fun = #linalg.unary_fn<exp>} ins(%arg19 : tensor<32x4x8xf32>) outs(%0 : tensor<32x4x8xf32>) -> tensor<32x4x8xf32>
    scf.yield %expanded_30, %66 : tensor<20x32x4x1xf32>, tensor<32x4x8xf32>
  }
  %collapsed_25 = tensor.collapse_shape %34#0 [[0], [1], [2, 3]] : tensor<20x32x4x1xf32> into tensor<20x32x4xf32>
  %extracted_slice_26 = tensor.extract_slice %collapsed_25[%64, 0, 0] [1, 32, 4] [1, 1, 1] : tensor<20x32x4xf32> to tensor<1x32x4xf32>
  return %extracted_slice_26: tensor<1x32x4xf32>
}
