...
if (
    config.build_bishengir_template
    and config.use_autogen_bc_hex
    and "bisheng" in config.bisheng_compiler_executable
    and os.path.isfile(config.bisheng_compiler_executable)
):
    config.available_features.add("enable-lir-compile")
    llvm_config.with_environment(
        "BISHENG_INSTALL_PATH",
        os.path.dirname(config.bisheng_compiler_executable),
        append_path=True,
    )
