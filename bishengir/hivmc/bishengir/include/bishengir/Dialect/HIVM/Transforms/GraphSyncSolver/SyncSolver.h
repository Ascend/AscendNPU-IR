...
class Solver {
public:
  bool considerMergedBackwardSyncEventIds{false};
...
protected:
  void init(std::unique_ptr<IRTranslator> irTranslator) {
    syncMode = irTranslator->syncMode;
    ...
    occChildrenMem = std::move(irTranslator->occChildrenMem);
  ...
  bool isIntraCoreMode() const {
    return syncMode == SyncMode::NORMAL_SYNC ||
  ...
  std::pair<Occurrence *, Occurrence *>
  getPlaceHolderSetWaitOcc(Occurrence *setOcc, Occurrence *waitOcc);

  void inplaceFixPlaceHolderSetWaitOcc(Occurrence *&setOcc,
                                       Occurrence *&waitOcc);
