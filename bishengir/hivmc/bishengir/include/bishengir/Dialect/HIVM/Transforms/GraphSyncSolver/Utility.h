...
struct ConflictPair {
  ...
  const int id{-1};
  RWOperation *op1{nullptr};
  RWOperation *op2{nullptr};
  OperationBase *opSet{nullptr};
  OperationBase *opWait{nullptr};
  Occurrence *occSet{nullptr};
  Occurrence *occWait{nullptr};
  CorePipeInfo setCorePipeInfo;
  CorePipeInfo waitCorePipeInfo;
  int startIndex{-1};
  int endIndex{-1};
  ...
  ConflictPair(RWOperation *op1, RWOperation *op2, OperationBase *opSet,
               OperationBase *opWait, Occurrence *occSet, Occurrence *occWait,
               CorePipeInfo setCorePipeInfo, CorePipeInfo waitCorePipeInfo,
               int startIndex, int endIndex)
      : id(globalIdCounter++), op1(op1), op2(op2), opSet(opSet), opWait(opWait),
        occSet(occSet), occWait(occWait), setCorePipeInfo(setCorePipeInfo),
  ...
  void updateSetWaitOccs(Occurrence *setOcc, Occurrence *waitOcc) {
    if (setOcc != nullptr) {
      this->occSet = setOcc;
      this->opSet = setOcc->op;
      this->startIndex = setOcc->endIndex;
    }
    if (waitOcc != nullptr) {
      this->occWait = waitOcc;
      this->opWait = waitOcc->op;
  ...
  std::unique_ptr<ConflictPair> clone() {
    auto clonedConflictPair = std::make_unique<ConflictPair>(
        op1, op2, opSet, opWait, occSet, occWait, setCorePipeInfo,
