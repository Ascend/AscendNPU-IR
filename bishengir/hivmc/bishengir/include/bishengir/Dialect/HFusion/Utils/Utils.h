...
// Check if it's a div op with int or uint
bool isElemwiseBinaryDivWithIntOrUintInputs(Operation *op, bool isAscend950);

// Check if operations is in cube scope
bool isInCubeScope(Operation *op);

// Check if it's a modui op (always has int operators)
bool isElemwiseBinaryModUI(Operation *op, bool isAscend950);
