...
static bool isValueReachingMatmul(Value val) {
  for (auto *user : val.getUsers()) {
    ...
    if (auto genericOp = dyn_cast<linalg::GenericOp>(user)) {
      if (isa<linalg::MatmulOp>(genericOp.getInputs()[0].getDefiningOp()) ||
          isa<linalg::BatchMatmulOp>(genericOp.getInputs()[0].getDefiningOp()))
        return true;
    }
    ...
    if (auto forloop = dyn_cast<scf::ForOp>(user)) {
      ...
      return isValueReachingMatmul(arg);
    }
  }
  return false;
}
...
bool hfusion::isElemwiseBinaryDivWithIntOrUintInputs(Operation *op,
                                                     bool isAscend950) {
  if (!isAscend950) {
    return false;
  }
  auto linalgOp = dyn_cast<linalg::ElemwiseBinaryOp>(op);
  if (!linalgOp) {
    return false;
  }
  linalg::BinaryFn kind = linalgOp.getFun();
  if (kind != linalg::BinaryFn::div && kind != linalg::BinaryFn::div_unsigned) {
    return false;
  }

  auto checkIntType = [](Type type) {
    return type.isInteger(16) || type.isInteger(32);
  };
  return llvm::all_of(linalgOp.getDpsInputs(), [&](Value val) {
    return checkIntType(getElementTypeOrSelf(val.getType()));
  });
}
...
bool hfusion::isElemwiseBinaryModUI(Operation *op, bool isAscend950) {
  if (!isAscend950) {
    return false;
  }
  auto hfusionOp = dyn_cast<hfusion::ElemwiseBinaryOp>(op);
  if (!hfusionOp) {
    return false;
  }
  hfusion::BinaryFn kind = hfusionOp.getFun();
  if (kind != hfusion::BinaryFn::modui) {
    return false;
  }
  ...
  return llvm::all_of(hfusionOp.getDpsInputs(), [&](Value val) {
    return checkIntType(getElementTypeOrSelf(val.getType()));
  });
}
