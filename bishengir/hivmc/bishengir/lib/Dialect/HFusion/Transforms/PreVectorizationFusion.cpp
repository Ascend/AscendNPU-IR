...
struct HFusionGeneralizationPatterns
  ...
  LogicalResult matchAndRewrite(linalg::LinalgOp op,
    ...
    if (isa<hfusion::LoadOp, hfusion::StoreOp>(op) ||
        ...
        (hfusion::isSingleElementLinalgOp(op) && isa<linalg::FillOp>(op)) ||
...
struct FlattenIndirectLoadMask : public OpRewritePattern<hfusion::IndirectLoadOp> {
public:
  ...
  LogicalResult matchAndRewrite(hfusion::IndirectLoadOp op,
    ...
    if (!fillOp) {
      return rewriter.notifyMatchFailure(op, "mask is not defined by linalg.fill");
    }
    ...
    auto newFill = rewriter.create<linalg::FillOp>(
        loc, ValueRange{fillValue}, ValueRange{newEmpty});

    auto maybeReassociation = getReassociationIndicesForReshape(newEmptyType, maskType);
    if (!maybeReassociation.has_value()) {
      return rewriter.notifyMatchFailure(op, "failed to create reassociation for tensor.expand_shape");
    }
