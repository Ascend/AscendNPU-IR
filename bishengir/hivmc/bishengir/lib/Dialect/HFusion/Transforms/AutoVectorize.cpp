...
bool isNonVectorizableOp(Operation *op) {
  return isa<hfusion::LoadOp, hfusion::StoreOp, tensor::ExtractOp,
         ...
         hfusion::isElemwiseBinaryDivWithIntOrUintInputs(op,
                                                         archisAscend950) ||
         hfusion::isElemwiseBinaryModUI(op, archisAscend950);
}
...
struct PostprocessExtractSliceArgument : public OpRewritePattern<func::CallOp> {
  ...
  LogicalResult matchAndRewrite(func::CallOp op,
    ...
    for (;;) {
      ...
      for (auto [idx, operand] : llvm::enumerate(operands)) {
        if (auto defOp = operand.getDefiningOp<tensor::ExtractSliceOp>();
            defOp && !checkStandardStride(defOp)) {
          extractSliceOp = defOp;
          auto src = extractSliceOp.getSource();
          newOperands[idx] = src;
          insertOperands(newOperands, idx + 1, extractSliceOp);
          auto returnOp =
              cast<func::ReturnOp>(funcOp.getBody().front().getTerminator());
          for (auto [resIdx, res] : llvm::enumerate(returnOp.getOperands())) {
            if (traceFuncArgument(res) == ssize_t(idx)) {
              resNum = static_cast<int64_t>(resIdx);
              break;
            }
          }
          modifyVectorFunction(funcOp, idx, resNum, extractSliceOp, rewriter);
          res = success();
          break;
        }
      }
      if (!extractSliceOp)
        break;
      auto newOp =
          rewriter.create<func::CallOp>(op.getLoc(), funcOp, newOperands);
      SmallVector<Value> newResults(newOp->result_begin(), newOp->result_end());
      newOp->setAttrs(op->getAttrs());
      if (resNum != -1) {
        auto &res = newResults[resNum];
        res = rewriter.create<tensor::ExtractSliceOp>(
            res.getLoc(), res, extractSliceOp.getMixedOffsets(),
...
private:
  bool checkStandardStride(tensor::ExtractSliceOp op) const {
    ...
    if (auto prevOp = src.getDefiningOp<tensor::ExtractSliceOp>();
        prevOp && !checkStandardStride(prevOp))
      return false;
  ...
  void insertOperands(SmallVectorImpl<Value> &newOperands, size_t pos,
                      tensor::ExtractSliceOp extractSliceOp) const {
    auto offsets = extractSliceOp.getOffsets();
    auto sizes = extractSliceOp.getSizes();
    auto strides = extractSliceOp.getStrides();
    newOperands.insert(newOperands.begin() + pos, strides.begin(),
                       strides.end());
    newOperands.insert(newOperands.begin() + pos, sizes.begin(), sizes.end());
    newOperands.insert(newOperands.begin() + pos, offsets.begin(),
                       offsets.end());
  }

  void insertTypes(SmallVectorImpl<Type> &newTypes, size_t pos,
                   tensor::ExtractSliceOp extractSliceOp) const {
    auto offsets = TypeRange(extractSliceOp.getOffsets());
    auto sizes = TypeRange(extractSliceOp.getSizes());
    auto strides = TypeRange(extractSliceOp.getStrides());
    newTypes.insert(newTypes.begin() + pos, strides.begin(), strides.end());
    newTypes.insert(newTypes.begin() + pos, sizes.begin(), sizes.end());
    newTypes.insert(newTypes.begin() + pos, offsets.begin(), offsets.end());
  }

  SmallVector<Value> insertArguments(ValueRange args, size_t pos,
                                     Block &block) const {
  ...
  int64_t traceFuncArgument(Value v) const {
    ...
    if (auto forOp = v.getDefiningOp<scf::ForOp>()) {
      ...
      return traceFuncArgument(forOp.getInitArgs()[opRes.getResultNumber()]);
    }
    return -1;
  }

  void modifyVectorFunction(func::FuncOp &funcOp, size_t idx, int64_t resNum,
    ...
    auto newInputTypes = llvm::to_vector(oldFuncType.getInputs());
    auto newResultTypes = llvm::to_vector(oldFuncType.getResults());
    newInputTypes[idx] = src.getType();
    insertTypes(newInputTypes, idx + 1, extractSliceOp);
    if (resNum != -1)
      newResultTypes[resNum] = src.getType();
    rewriter.modifyOpInPlace(funcOp, [&]() {
      funcOp.setFunctionType(
          rewriter.getFunctionType(newInputTypes, newResultTypes));
      auto oldArg = block.getArgument(idx);
      newStrides = insertArguments(strides, idx, block);
      newSizes = insertArguments(sizes, idx, block);
      newOffsets = insertArguments(offsets, idx, block);
      ...
      auto newOp = rewriter.create<tensor::ExtractSliceOp>(
          ...
          newOffsets, newSizes, newStrides, extractSliceOp.getStaticOffsets(),
          extractSliceOp.getStaticSizes(), extractSliceOp.getStaticStrides());
      rewriter.replaceAllUsesWith(oldArg, newOp);
    ...
    if (resNum != -1) {
      ...
      rewriter.modifyOpInPlace(returnOp, [&]() {
        ...
        auto opr = returnOp.getOperands()[resNum];
        auto newOpr = rewriter.create<tensor::InsertSliceOp>(
            opr.getLoc(), cast<RankedTensorType>(newArg.getType()), opr, newArg,
            newOffsets, newSizes, newStrides, extractSliceOp.getStaticOffsets(),
            extractSliceOp.getStaticSizes(), extractSliceOp.getStaticStrides());
        returnOp.getOperandsMutable()[resNum].set(newOpr);
      });
    }
  }
};
...
void AutoVectorize::runOnOperation() {
  ...
  patterns.add<PostprocessExtractSliceArgument>(context);
