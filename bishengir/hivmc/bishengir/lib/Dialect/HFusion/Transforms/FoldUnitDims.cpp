...
FailureOr<Value> tryCollapseValue(Value oldVal, OpBuilder &builder,
  ...
  if (squeezedShape != oldShape) {
    ...
    if (dynShapeHasOne) {
      LLVM_DEBUG(
          llvm::dbgs()
              << "[tryCollapseValue] both dynamic shape and static shape "
...
struct DropUnitDimsInsertSlicePattern
  ...
  matchAndRewrite(tensor::InsertSliceOp op,
    ...
    if (!src.getDefiningOp()) {
      return rewriter.notifyMatchFailure(
          op, "[DropUnitDimsInsertSlicePattern] src has no definingOp");
    }
    ...
    if (!isEqualConstantIntOrValueArray(expandedShape, oldSizes)) {
      ...
      return rewriter.notifyMatchFailure(
          op, "[DropUnitDimsInsertSlicePattern] tensor.expand_shape does not "
              "fully match tensor.insert_slice's sizes");
    }

    auto dst = op.getDest();
    auto maybeCollapsedDst = tryCollapseValue(dst, rewriter, loc);
    if (!succeeded(maybeCollapsedDst)) {
      LLVM_DEBUG(llvm::dbgs() << "[DropUnitDimsInsertSlicePattern] "
                                 "InsertSliceOp fails to tryCollapseValue\n";);
      return failure();
    }
    ...
    Value collapsedSrc = expandOp.getSrc();
    ...
    Value expandSrc = newOp.getResult();
    // tensor::InsertSliceOp can be rank-reduced
    auto droppedDims = op.getDroppedDims();
    if (droppedDims.any()) {
      return rewriter.notifyMatchFailure(
          op, "[DropUnitDimsInsertSlicePattern] rank-extended "
              "tensor.insert_slice is unsupported");
    }
...
void HFusionFoldUnitDimsPass::runOnOperation() {
  ...
  // Then drop unit dims on linalg related ops
  {
    RewritePatternSet patterns(context);
    linalg::ControlDropUnitDims options;
    options.foldRankReducingSlices = false;
