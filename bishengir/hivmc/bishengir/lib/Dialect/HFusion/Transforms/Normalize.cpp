...
private:
  bool isConstant(Value v) const {
    Operation *def = v.getDefiningOp();
    if (!def)
      return false; // block arguments are never constant

    // The canonical MLIR way:
    return def->hasTrait<OpTrait::ConstantLike>();
  }

  Value getNonConstantValue(Value a, Value b) const {
    if (!isConstant(a))
      return a;
    if (!isConstant(b))
      return b;
  ...
  Value createDiv(PatternRewriter &rewriter, Location loc, Type resType,
    ...
    if (resType.isInteger()) {
      ...
      auto divOp = createLinalgBinaryOp(rewriter, loc, linalg::BinaryFn::div,
                                        src0, src1, src0);
  ...
  Value handleZeromodulusForIntegerType(PatternRewriter &rewriter, Location loc,
                                        Value modulus, Value result) const {
    ...
    auto negOneTensor =
        utils::createEmptyOpWithTargetElemType(rewriter, loc, result, elemType);
    auto tensorNegOne =
        rewriter.create<linalg::FillOp>(loc, constNegOne, negOneTensor)
            .getResults()[0];

    return createSelectOp(rewriter, loc, zeroFlag, tensorNegOne, result,
                          result);
  }
  ...
  Value handleUint8(PatternRewriter &rewriter, Location loc, Value x, Value y,
                    Value result) const {
    auto castedType = rewriter.getI16Type();
    auto xCasted = hfusion::castTo(rewriter, x, castedType,
                                   hfusion::TypeFn::cast_unsigned);
    auto yCasted = hfusion::castTo(rewriter, y, castedType,
                                   hfusion::TypeFn::cast_unsigned);
    auto moduiOp = createHFusionBinaryOp(
        rewriter, loc, hfusion::BinaryFn::modui, xCasted, yCasted, xCasted);
    return hfusion::castTo(rewriter, moduiOp, rewriter.getI8Type());
  }
...
public:
  ...
  LogicalResult matchAndRewrite(hfusion::ElemwiseBinaryOp op,
    ...
    if (op.getFun() == hfusion::BinaryFn::modui) {
      if (elemType.isInteger(8)) {
        auto resultUint8 =
            handleUint8(rewriter, op.getLoc(), xOrig, yOrig, resTensor);
        rewriter.replaceOp(op, resultUint8);
        return success();
      } else {
        // other modui will be handled in hivm.
        return failure();
      }
    }
    ...
    if (!getNonConstantValue(xOrig, yOrig)) {
      return rewriter.notifyMatchFailure(
          op, "mod between two constants is not supported");
    }
    ...
    if (elemType.isF32() || elemType.isF16() || elemType.isBF16()) {
      result =
          handleInfinityModulus(rewriter, op.getLoc(), xOrig, yOrig, result);
    }

    // step 9: res_f32 = select(div == 0, -1, result)
    if (elemType.isInteger()) {
      result = handleZeromodulusForIntegerType(rewriter, op->getLoc(), yOrig,
                                               result);
    }
...
struct Elemwise : public OpRewritePattern<hfusion::StoreOp> {
  ...
  inline bool isHardwareSupported(Type dtype) const {
    ...
    static const llvm::DenseSet<Type> hardwareSupportedTypes{
        IntegerType::get(ctx, 8),  IntegerType::get(ctx, 16),
        IntegerType::get(ctx, 32), Float16Type::get(ctx),
        Float32Type::get(ctx),     BFloat16Type::get(ctx)};
    Type t = getElementTypeOrSelf(dtype);
    return hardwareSupportedTypes.contains(t);
  }
