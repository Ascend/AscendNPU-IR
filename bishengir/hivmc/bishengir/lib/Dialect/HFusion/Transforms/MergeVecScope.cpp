...
bool haveDataFlowDependency(func::CallOp call1, func::CallOp call2) {
  if (call1->getParentRegion() != call2->getParentRegion()) {
    return false;
  }
  for (mlir::Value result : call1.getResults()) {
    DenseSet<Value> flag;
    auto hasAnyUse = utils::checkUsersAnyWithCondition(
        result, call1.getOperation(),
        /*condFn*/ [&](Operation *op) { return op == call2.getOperation(); },
        /*skipFn*/ [&](Operation *op) { return true; },
        flag);
    if (hasAnyUse) {
      return true;
    }
  }
  for (mlir::Value result : call2.getResults()) {
    DenseSet<Value> flag;
    auto hasAnyUse = utils::checkUsersAnyWithCondition(
        result, call2.getOperation(),
        /*condFn*/ [&](Operation *op) { return op == call1.getOperation(); },
        /*skipFn*/ [&](Operation *op) { return true; },
        flag);
    if (hasAnyUse) {
      return true;
...
void MergeVecScopePass::runOnOperation() {
    ...
    for (size_t j = i + 1; j < vfs.size(); ++j) {
      ...
      for (size_t i1 = 0; i1 < vf1_callers.size(); ++i1) {
        for (size_t j2 = 0; j2 < vf2_callers.size(); ++j2) {
          hasDependency |=
              haveDataFlowDependency(vf1_callers[i1], vf2_callers[j2]);
        }
      }
      if (hasDependency) {
  ...
  while (scoreIdx < startVFs.size()) {
    ...
    if (mergeSuccess) {
      ...
      // clang-format off
      LLVM_DEBUG(
        llvm::dbgs() << "mergedVfRef = " << &mergedVfRef << ", "
...
bool MergeVecScopePass::tryMerge(func::FuncOp root, func::FuncOp vf1,
  ...
  for (Operation *op : betweenOps) {
    if (isa<hivm::SyncBlockSetOp>(op) || isa<hivm::SyncBlockWaitOp>(op)) {
      ...
      LLVM_DEBUG(llvm::dbgs() << "SyncBlock op prevents fusion\n";);
      return false;
    }
  }
  ...
  for (Operation *memOp : memOps) {
    for (Value result : memOp->getResults()) {
  ...
  for (Operation *memOp : memOps) {
    // consider node with block
    memOp->walk([&](Operation *nestedOp) {
...
bool MergeVecScopePass::SplitMove(const SmallVector<Operation *> &memOps,
  ...
  if (mergeLevel == 1) {
    ...
    // iter over memOps，get ops that depend on vf1
    DenseSet<Value> dependOnVf1;
    for (Value res : call1.getResults()) {
      dependOnVf1.insert(res);
    }
    ...
    for (Operation *op : memOps) {
      ...
      op->walk([&](Operation *nestedOp){
        for (Value operand : nestedOp->getOperands()) {
          if (dependOnVf1.contains(operand)) {
            isDependent = true;
            break;
          }
        }
