...
struct FusedNode {
  ...
  // the location of this FusedNode. For example, if a FusedNode has three leafNode
  // A, B and C, and C is the last leafNode in IR, here we record C.
...
static bool isNonVectorizableOp(Operation *op) {
  ...
  return isa<hfusion::LoadOp, hfusion::StoreOp, hfusion::ReduceWithIndexOp,
             ...
             hfusion::CastOp, hfusion::CompareOp,
             tensor::ExtractOp, tensor::DimOp, tensor::ReshapeOp,
             ...
             hivm::StoreOp, hivm::BitcastOp, hivm::SyncBlockSetOp,
             hivm::SyncBlockWaitOp, scf::ForOp, scf::IfOp, func::CallOp>(op) ||
         hfusion::isSimtOps(op) || hfusion::isMatmulOps(op) ||
         hfusion::opCanFuseIntoMatmul(op) ||
         hfusion::isElemwiseBinaryDivWithIntOrUintInputs(op, true) ||
         hfusion::isElemwiseBinaryModUI(op, true);
}
...
static void computeNumLoopsAndShapeAndMaxElemBitWidth(Operation *op,
  ...
  for (Type ty : allTypes) {
    ...
    currElemBitWidth =
        (currElemBitWidth == 64) ? 32 : currElemBitWidth;
...
static bool isVsstbPatternTransposeOp(Operation *op) {
  ...
  if (!(elemType.isBF16() || elemType.isF16() || elemType.isF32() || elemType.isFloat8E4M3FN() || elemType.isFloat8E5M2())){
    return false;
  }
      ...
      if (shouldMultiAxisVectorize && opInfo.numLoops > 1) {
        ...
        tileSize[opInfo.numLoops - 1] =
            maxElemBitWidthInFusedNode == 1
                ? vectorLength
                : vectorLength /
                      static_cast<int64_t>(maxElemBitWidthInFusedNode / utils::INTR_BITS_PER_BYTE);
      }
...
static void
findUpstreamFusableOpOf(Operation *op, Block *block,
...
static void
findPreviousAndFollowingFusableOpOf(
    Operation *barrierOp, Block *block, DenseSet<Operation *> &previousOps,
...
/// If two fusable ops are conflict with each other, they cannot be fused into the
/// same VF:
/// 1. The producer(upstream op) and consumer(downstream op) of NonVectorizableOp
///    are confilict with each other. For example:
///       A(FusableOp)
///       B(NonVectorizableOp, use A)
///       C(FusableOp, use B)
///    Then A and C are confilict with each other
/// 2. If A fusable op has NonVectorizableOp users and FusableOp at the same time,
...
///       C(FusableOp, use A)
///    Then A and C are confilict with each other
/// 3. The previous and following of hivm.hir.sync_block_wait/hivm.hir.sync_block_set/
...
static void computeConflictLists(
    func::FuncOp func, llvm::MapVector<Operation *, FusableOpInfo> &fusableOpInfoMap) {
    ...
    if (isa<func::FuncOp, scf::ForOp, scf::IfOp>(block->getParentOp())) {
      block->walk([&](Operation *op) {
        if (isOpInBlock(op, block) && isNonVectorizableOp(op)) {
          ...
          if (isa<hivm::SyncBlockSetOp, hivm::SyncBlockWaitOp, scf::ForOp>(op)) {
...
static bool isFillOp(Operation *op) {
  if (auto linalgOp = dyn_cast<linalg::LinalgOp>(op)) {
...
static std::shared_ptr<FusedNode> findBestFusedNodeForProducer(
  ...
  for (auto user : DenseSet<Operation *>(producer->getUsers().begin(),
    ...
    if (bestFusedNode->fusedOps.contains(user)) {
      ...
      if (isa<linalg::TransposeOp>(user) &&
          !isVsstbPatternTransposeOp(user))
  ...
  // cannot fuse them into the same fusedNode. For vsstb pattern transpose, we fuse
  // the producers into this transpose op; for other pattern transpose, we fuse this
  // transpose op into its consumer ops.
  // For example: if we have user chain: op1 -> vsstb transpose -> op2, we will fuse
  // op1 into transpose op; if we have user chain: op1 -> normal transpose -> op2,
  // we will fuse transpose op into op2
  ...
  // If producer is reduction and user is broadcast, only fuse last-axis reduction
  // producer into last-axis broadcast op. Other fusion context will generate
  // memref.alloc inside VF and cannot be outlined, so give up fusing, see issue:
      ...
      if (bestFusedNode->fusedOps.contains(user)) {
        if (auto consumerLinalgOp = dyn_cast<linalg::LinalgOp>(user)) {
          ...
          if (map.getNumResults() < map.getNumDims()) {
            // last-axis broadcast, the affine map will be `affine_map<(d0,d1)->(d0)>`
            // non-last-axis broadcast, the affine map will be `affine_map<(d0,d1)->(d1)>`
...
static bool shouldUseTileReductionUsingFor(OpBuilder &builder, Operation *op,
  ...
  /// TODO: here we only handle those have parallel axis and tail-axis reduction.
...
static Value tileReductionOp(
    ...
    SmallVector<std::pair<std::string, SmallVector<int64_t>>> &otherVectorizableOps) {
  ...
  transform::TileUsingForOp parallelAxisTilingResult =
      builder.create<transform::TileUsingForOp>(
          loc, linalgOpHandle, parallelAxisTileSize);
  ...
  builder.create<transform::AnnotateOp>(
      loc, reductionAxisTilingResult.getSplitLinalgOp(),
      splitLinalgOpLabel, nullptr);
  ...
  builder.create<transform::AnnotateOp>(
      loc, generalizedCombiningLinalgOp, combiningLinalgOpLabel, nullptr);
...
private:
  void
  initFusableOpInfo(func::FuncOp func, int64_t vectorLength,
                    llvm::MapVector<Operation *, FusableOpInfo> &fusableOpInfoMap);
...
void AutoVectorizeV2::planFuseSiblingForLeafNodes(
  ...
  for (auto leafNodeGroup : leafNodeGroups) {
    ...
    fusedNode->numLoops = static_cast<unsigned>(fusableOpInfoMap[leafNodeGroup[0]].numLoops);
...
void AutoVectorizeV2::tileAndFuseSiblingForLeafNodes(
  ...
  for (SmallVector<Operation *> &leafNodeGroup : leafNodeGroups) {
    ...
    for (Operation *leafNode : leafNodeGroup) {
      ...
      if (shouldUseTileReductionUsingFor(builder, leafNode, leafNodeInfo.tileSize)) {
...
void AutoVectorizeV2::fuseProducersIntoConsumers(
  ...
  for (Operation *producer : producersToBeFusedInto) {
    ...
    if (shouldUseTileReductionUsingFor(builder, producer, producerInfo.tileSize)) {
...
void AutoVectorizeV2::buildTileAndFuseTransformSequenceForBlock(
  ...
  SmallVector<std::shared_ptr<FusedNode>> fusedNodes;
  SmallVector<SmallVector<Operation *>> leafNodeGroups;
  planFuseSiblingForLeafNodes(block, leafNodeGroups, fusableOpInfoMap, fusedNodes);
  SmallVector<Operation *> producersToBeFusedInto;
...
void AutoVectorizeV2::buildVectorizeAndOutlineTransformSequence(
  ...
  for (auto vectorizableOp : otherVectorizableOps) {
    builder.create<transform::VectorizeOp>(
        ...
        SmallVector<Value>(), vectorizableOp.second,
        nullptr, SmallVector<bool>(vectorizableOp.second.size(), false));
  }
  int idx = 0;
  ...
  for (auto info : fusableOpInfoMap) {
    ...
    if (!opInfo.fusedNode->loopIsOutlined) {
      ...
      builder.create<transform::AnnotateOp>(
          ...
          nullptr);
      builder.create<transform::AnnotateOp>(
          loc, outlineOp.getFunction(), "no_inline", nullptr);
      builder.create<transform::AnnotateOp>(
          loc, outlineOp.getCall(), "no_inline", nullptr);
    }
  }
}
...
void AutoVectorizeV2::runOnOperation() {
  ...
  for (func::FuncOp func : fusableFuncList) {
    ...
    func.walk([&](Block *block) {
      ...
      if (isa<func::FuncOp, scf::ForOp, scf::IfOp, scope::ScopeOp>(block->getParentOp()))
        buildTileAndFuseTransformSequenceForBlock(builder, seqOp, block,
                                                  fusableOpInfoMap,
                                                  otherVectorizableOps);
    });
    buildVectorizeAndOutlineTransformSequence(builder, func, seqOp,
                                              fusableOpInfoMap,
                                              otherVectorizableOps);
    // Apply transform
