...
handleHyperrectangleCase(PatternRewriter &rewriter, ArrayRef<int64_t> slicedRef,
                         ...
                         SliceModifyingOpResult &result) {
  std::optional<Hyperrectangle> hyperrectangle = getExtendHyperrectangleFromArray(
...
handleMutation(PatternRewriter &rewriter,
  ...
  for (auto idx : reassociationIndices) {
    ...
    if (constantFullExpandedRef[idx] != 1) {
      ...
      auto superviewShape = isa<RankedTensorType>(superview.getType())
                              ? tensor::getMixedSize(
                                  rewriter, superview.getLoc(),
                                  superview, dimensionIndex)
                              : memref::getMixedSize(
                                  rewriter, superview.getLoc(), superview,
                                  dimensionIndex);
...
checkHyperRectangle(PatternRewriter &rewriter,
  ...
  if (isSubview) {
    ...
    bool adjusted =
        adjustSubviewExpansion(superviewShape, laststride, staticStride.value(), resSlicedRef, srcSlicedRef);
    if (!adjusted) {
...
getSliceModifyingOp(PatternRewriter &rewriter, T slicingOp,
  ...
  for (unsigned i = 0; i < rank; i++) {
    if (srcShape[i] == resShape[i] && !ShapedType::isDynamic(srcShape[i])) {
      ...
      // Need to count how many units, fallback to non hyperrectangle for unit cases
      auto unitCount = llvm::count_if(reassociation[i],
                                      [&constantFullExpandedRef](
                                      const auto &reassoc) -> bool {
                                        return constantFullExpandedRef[reassoc]
                                               == 1;
                                      });
      bool allowHyperrectangle = true;
      if (static_cast<uint32_t>(unitCount + 1) >= reassociation[i].size()) {
...
/// @brief Attempts to expand an operand to match a target rank by creating an expand operation.
///
/// This function checks if the given operand's rank matches the target rank. If it does,
/// it creates a new expand operation from the provided collapse operation to expand the
/// operand. If the ranks don't match (e.g., scalar element-wise cases), the original
/// operand is returned unchanged.
///
/// @param collapseOp The `memref::CollapseShapeOp` used as a template for creating the expand operation
...
///
/// @return The expanded operand if ranks match, otherwise the original operand unchanged
Value tryExpandOperand(Operation* collapseOp,
                       PatternRewriter &rewriter,
                       Value operand,
                       int64_t targetRank) {
  ...
  if (auto memrefCollapse = dyn_cast<memref::CollapseShapeOp>(collapseOp)) {
    expandedOp = createNewExpandOpFromCollapseOp(
        memrefCollapse, rewriter, operand.getLoc(), operand);
  } else if (auto tensorCollapse = dyn_cast<
    tensor::CollapseShapeOp>(collapseOp)) {
    expandedOp = createNewExpandOpFromCollapseOp(
        tensorCollapse, rewriter, operand.getLoc(), operand);
  } else {
    return operand;
  }
...
/// @brief Transforms all operands of a user operation by potentially expanding them to a target rank.
///
/// This function iterates through all operands of the given user operation and attempts
/// to expand each one to match the specified target rank using `tryExpandOperand()`.
/// Operands that don't match the target rank are left unchanged.
///
/// @param collapseOp The `memref::CollapseShapeOp` used as a template for creating expand operations
/// @param rewriter The pattern rewriter used to create new operations
...
SmallVector<Value> getNewOperands(Operation* collapseOp,
  ...
  for (Value operand : userOp->getOperands()) {
    Value newOperand = tryExpandOperand(collapseOp, rewriter, operand,
                                        targetRank);
