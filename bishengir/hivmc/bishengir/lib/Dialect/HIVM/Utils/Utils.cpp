...
std::vector<std::pair<Value, Value>> getOperationAliasInfo(Operation *op) {
  ...
  if (auto subViewOp = dyn_cast<memref::SubViewOp>(op)) {
  ...
  } else if (auto memorySpaceCastOp = dyn_cast<memref::MemorySpaceCastOp>(op)) {
    result.emplace_back(memorySpaceCastOp.getResult(),
                        memorySpaceCastOp.getOperand());
    ...
    result.emplace_back(transferReadOp.getResult(), transferReadOp.getSource());
  } else if (auto transferWriteOp = dyn_cast<vector::TransferWriteOp>(op)) {
    result.emplace_back(transferWriteOp.getResult(),
