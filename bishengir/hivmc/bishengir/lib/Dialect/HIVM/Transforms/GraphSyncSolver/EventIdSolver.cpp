...
int64_t EventIdSolver::getEventIdsNum() {
  int64_t eventIdsNum = 0;
  assert(!needRecalculateEventIds);
  for (auto &node : nodes) {
    auto &eventIds = node->getEventIds();
    assert(!eventIds.empty());
    if (!eventIds.empty()) {
      eventIdsNum = std::max(eventIdsNum, eventIds.back() + 1);
    }
  }
  return eventIdsNum;
}
...
bool EventIdSolver::isColorable() {
  ...
  return getEventIdsNum() <= eventIdsNumMax;
}
...
EventIdSolver::getAdjNodesUsedEventIds(EventIdNode *node) {
  ...
  for (auto [otherNode, frq] : adjList[node]) {
    ...
    usedEventIds.insert(usedEventIds.end(), otherEventIds.begin(),
                        otherEventIds.end());
  }
  llvm::sort(usedEventIds.begin(), usedEventIds.end());
...
EventIdSolver::getChosenEventIds(EventIdNode *node, int64_t eventIdMax,
  ...
  if (!node->reversePriority) {
    ...
    int64_t curEventId = eventIdMax;
    if (!forceConsecutiveIds) {
      curEventId = std::max(eventIdMax, this->eventIdsNumMax - 1);
    }
...
void EventIdSolver::calcEventIds(bool forceConsecutiveIds) {
  ...
  for (auto *node : llvm::reverse(orderedNodes)) {
    auto chosenEventIds =
        getChosenEventIds(node, eventIdMax, forceConsecutiveIds);
