...
int64_t SolverTest::getTestEventIdNum(Occurrence *occ1, Occurrence *occ2,
  ...
  auto coreSrc = hivm::TCoreType::CUBE_OR_VECTOR;
  auto coreDst = hivm::TCoreType::CUBE_OR_VECTOR;
  if (barrierAllPairs.contains(
          {CorePipeInfo(coreSrc, setPipe), CorePipeInfo(coreDst, waitPipe)})) {
    return 1;
  }
    ...
    for (int i = iStart; i != iEnd; i += iStep) {
      if (llvm::isa_and_present<RWOperation>(syncIr[i]->op)) {
        ...
        for (auto [setPipe, waitPipe] :
          ...
          if (checkGraphConflict(
                  occ1, occ2, CorePipeInfo(rwOp1->coreType, setPipe),
                  CorePipeInfo(rwOp2->coreType, waitPipe), eventIdNum)) {
            handleConflict(occ1, occ2, CorePipeInfo(rwOp1->coreType, setPipe),
                           CorePipeInfo(rwOp2->coreType, waitPipe), isUseless,
                           eventIdNum, nullptr);
          }
        }
      }
