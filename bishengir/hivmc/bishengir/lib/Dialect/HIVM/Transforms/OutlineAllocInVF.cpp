...
/// If there are multiple writes to the same tensor within a VF, one-shot-bufferize may introduce
/// memref.alloc which VF cannot handle. Therefore, it needs to be moved outside of VF and passed
/// as an argument into VF, see issue:
...
void OutlineAllocInVFPass::runOnOperation() {
  ...
  funcOp.walk([&](memref::AllocOp allocOp) {
    if (auto memRefType = dyn_cast<MemRefType>(allocOp->getResultTypes()[0])) {
