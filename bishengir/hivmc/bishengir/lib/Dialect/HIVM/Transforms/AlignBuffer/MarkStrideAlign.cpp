...
/**
 * Reasons for retaining GM:
 * store ub to gm
 * ub size(2,1) stride(1,1)
 * gm size(2,1) stride(8,8)
 * The current processing will add a third dimension:1,
 * followed by store_ubuf_to_gm_3d_core_with_contiguous_last_dim,
 * requiring UB alignment at this stage.
 */
...
filterNonHivmSpace(const SmallVectorImpl<MemRefType> &memRefTypes) {
  ...
  for (auto memRefType : memRefTypes) {
    ...
    if (hivmSpace && (hivmSpace.getAddressSpace() == hivm::AddressSpace::UB ||
                      hivmSpace.getAddressSpace() == hivm::AddressSpace::GM)) {
