...
public:
  ...
  LogicalResult matchAndRewrite(OpType Op,
    ...
    // ----------------------------------------------------------------------
    /// TO DO: The below is a temporary settting to replace analyzer.getTilingDim(storeOp.getSrc()),
    /// will be enhanced in generalization version
    int64_t tilingDim = -1;
...
struct LimitUniqueSubBlockIdToCopy : public OpRewritePattern<hivm::CopyOp> {
public:
  ...
  LogicalResult matchAndRewrite(hivm::CopyOp op,
    ...
    if (op.getResults().empty()) {
      // case 1: copy op without results
      auto ifOp = rewriter.create<scf::IfOp>(loc, TypeRange(), cond, false);
      auto thenBodyBuilder = ifOp.getThenBodyBuilder(rewriter.getListener());
      thenBodyBuilder.clone(*op.getOperation());
      rewriter.replaceOp(op, ifOp);
      ifOp->setAttr(kLimitedSubBlockOpAttrName, rewriter.getUnitAttr());
      return success();
    }

    // case 2: copy op with results
    // FIXME: Force drop copy op results and dst use memref to fix
    // 'hivm.hir.copy' op Unsupported copy from cbuf to ub!.
    bufferization::ToTensorOp bufferToTensorOp =
        dyn_cast<bufferization::ToTensorOp>(op.getDst().getDefiningOp());
    if (bufferToTensorOp) {
      auto memRefValue = bufferToTensorOp.getMemref();
      auto ifOp = rewriter.create<scf::IfOp>(loc, TypeRange(), cond, false);
      auto thenBodyBuilder = ifOp.getThenBodyBuilder(rewriter.getListener());
      thenBodyBuilder.create<hivm::CopyOp>(loc, TypeRange{},
                                           /*src*/ op.getSrc(),
                                           /*dst*/ memRefValue);
      ifOp->setAttr(kLimitedSubBlockOpAttrName, rewriter.getUnitAttr());
      rewriter.replaceAllUsesWith(op.getResults()[0], op.getDst());
      rewriter.eraseOp(op);
    }
    return success();
  }
...
static LogicalResult limitUniqueSubBlockToStore(func::FuncOp funcOp) {
  ...
  patterns.add<LimitUniqueSubBlockIdToCopy>(funcOp.getContext());
...
static LogicalResult tileAndSliceOp(func::FuncOp func) {
  ...
  patterns.add<TileAndSliceStoreCopyOp<hivm::StoreOp>,
               TileAndSliceStoreCopyOp<hivm::CopyOp>>(func->getContext(), analyzer);
  GreedyRewriteConfig config;
  config.maxIterations = kMaxIterations;
...
public:
  ...
  LogicalResult matchAndRewrite(hivm::FixpipeOp op,
    ...
    if (!attr || !attr.getId().has_value())
      return failure();
    // int32_t cvId = attr.getId().value();

    // TODO: Determine splitMode based on cvId
  ...
  if (aivSuccessFlag && archIs950) {
    for (func::FuncOp originalFunc : functionsToProcess) {
      ...
      if (!funcCoreType.has_value() ||
          ...
          !originalFunc->hasAttrOfType<UnitAttr>(hivm::TPartOfMixAttr::name))
        continue;
      if (failed(runSplitFixpipe(originalFunc)))
        signalPassFailure();
    }
  }
