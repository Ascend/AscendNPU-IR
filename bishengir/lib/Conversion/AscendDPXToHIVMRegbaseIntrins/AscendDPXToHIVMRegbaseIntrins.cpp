...
// TODO: remove this define (multi load isn't generated by the upper triton to
// dpx pass, meaning it's currently untested)
#define DISABLE_MULTI_LOAD true
...
static Value buildSingleLoad(ConversionPatternRewriter &rewriter, Location loc,
  ...
  if (addrSpace == SHARED_ADDR_SPACE || cacheHint == 0) {
    ...
    return rewriter.create<LLVM::LoadOp>(loc, resultType, ptr);
  } else if (addrSpace == GLOBAL_ADDR_SPACE) {
    std::string opName = "llvm.hivm.ldg.cache.s" + std::to_string(bitWidth);
...
static Value buildMultiLoad(ConversionPatternRewriter &rewriter, Location loc,
                            ...
                            TypedValue<LLVM::LLVMPointerType> ptr,
                            uint32_t cacheHint) {
  if constexpr (DISABLE_MULTI_LOAD) {
    return rewriter.create<LLVM::LoadOp>(loc, resultType, ptr);
  }
  ...
  for (int i = 0; i < numElems; i += strideLength) {
    ...
    Value subLoadResult =
        buildLoad(rewriter, loc, subLoadType, subLoadOffset, cacheHint);
...
static Value buildLoad(ConversionPatternRewriter &rewriter, Location loc,
  ...
  if (bitWidth <= MAX_LOADSTORE_BITWIDTH) {
    ...
    return buildMultiLoad(rewriter, loc, vResult, ptr, cacheHint);
  } else {
...
struct AscendDPXLoadOpLowering
  ...
  matchAndRewrite(ascend_dpx::LoadOp load, ascend_dpx::LoadOp::Adaptor adaptor,
    ...
    uint32_t cacheHint = static_cast<uint32_t>(load.getCacheHintAttr().getValue());
    auto addrSpace = ptrType.getAddressSpace();
...
static void buildSingleStore(ConversionPatternRewriter &rewriter, Location loc,
  ...
  if (addrSpace == SHARED_ADDR_SPACE || cacheHint == 0) {
    ...
    std::string opName = "llvm.hivm.stg.cache.b" + std::to_string(storeWidth);
...
static void buildStore(ConversionPatternRewriter &rewriter, Location loc,
  ...
  if (bitWidth <= MAX_LOADSTORE_BITWIDTH) {
    buildSingleStore(rewriter, loc, val, ptr, cacheHint);
  } else if (auto vecVal = dyn_cast<TypedValue<VectorType>>(val)) {
    buildMultiStore(rewriter, loc, vecVal, ptr, cacheHint);
  } else {
    ...
    buildMultiStore(rewriter, loc, castedVal, ptr, cacheHint);
  }
}
...
struct AscendDPXStoreOpLowering
  ...
  matchAndRewrite(ascend_dpx::StoreOp store,
    ...
    if (mask) {
      ...
      rewriter.setInsertionPointToStart(&ifOp.getThenRegion().front());
      buildStore(rewriter, loc, val, ptr, cacheHint);
