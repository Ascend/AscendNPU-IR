...
check_inputs_of_cumprod_ra(memref_t<__ubuf__ T, 2> *src,
...
/// cumprod ra op description:
/// Returns the cumulative production of elements of input in the first axis.
///
/// \param src (type: memref<a x b x T>)
/// \param dst (type: memref<a x b x T>)
///
/// Constraints:
/// 1. cumprod ra op supports uint8/16/32/64_t, int8/16/32/64_t, float16/32 and
/// bfloat16 type.
/// 2. cumprod ra op only accepts 2d type memrefs as src and dst.
...
vector_cumprod_ra(memref_t<__ubuf__ T, 2> *src, memref_t<__ubuf__ T, 2> *dst, bool reverse) {
  static_assert(
      ...
      "cumprod ra op only support uint8/16/32/64_t, int8/16/32/64_t, "
      "float16/32 and bfloat16 type "
      "operands in template!");
 
  // Input parameter constraints assert.
  check_inputs_of_cumprod_ra(src, dst);
  ...
  //            vmul(dst + i * dst->strides[0], src + i * src->strides[0], dst +
...
cumprod_2d(cumulative_args<int8_t, int8_t> args) {
  ...
  // convert int8_t memref to uint8 memref
  memref_t<__ubuf__ uint8_t, 1> src0_as_uint8;
  memref_t<__ubuf__ uint8_t, 1> src1_as_uint8;
  memref_t<__ubuf__ uint8_t, 1> dst_as_uint8;
  view_as<int8_t, uint8_t, 1>(args.src[0], &src0_as_uint8);
  view_as<int8_t, uint8_t, 1>(args.src[1], &src1_as_uint8);
  view_as<int8_t, uint8_t, 1>(args.dst, &dst_as_uint8);
 
  cumprod_2d<uint8_t, uint8_t>(
    cumulative_args<uint8_t, uint8_t>{&dst_as_uint8, {&src0_as_uint8, &src1_as_uint8}, 
                                      args.dst_stride, {args.src_stride[0], args.src_stride[1]},
                                      args.src_size, args.reverse});
}
...
cumprod_2d(cumulative_args<uint8_t, uint8_t> args) {
  ...
  constexpr int num_per_reg = REG_REGISTER_SIZE / sizeof(uint8_t);
  uint16_t repeat_times = CEIL_DIV(size0, num_per_reg);
  for (uint16_t j = 1; j < src_size; ++j) {
    auto dst_offset_tmp = dst_offset + j * dst_stride;
    int src0_steps = args.reverse ? (src_size -1 - j) : j;
    auto src0_offset_tmp = src0_offset + src0_steps * src0_stride;
    auto src1_offset_tmp = src1_offset + (j-1) * src1_stride;
    __VEC_SCOPE__ {
      vector_u16 src0_u16_even, src0_u16_odd;
      vector_u16 src1_u16_even, src1_u16_odd;
      vector_u16 dst_u16_even, dst_u16_odd;
      VectorReg<uint8_t> src0_reg, src1_reg;
      VectorReg<uint8_t> dst_reg;
      vector_bool full_mask;
      CREATE_MASK_BY_SIZE(full_mask, uint8_t, size0);
      for (uint16_t i = 0; i < repeat_times; i++) {
        vlds(src0_reg, src0_aligned + src0_offset_tmp, i * num_per_reg, NORM);
        vlds(src1_reg, src1_aligned + src1_offset_tmp, i * num_per_reg, NORM);
  ...
  __VEC_SCOPE__ {
    for (uint16_t i = 0; i < repeat_times; i++) {
      ...
      for (uint16_t j = 1; j < src_size; j++) {
        ...
        auto src0_offset_tmp = src0_offset + src0_steps * src0_stride;
        vlds(src0_reg, src0_aligned + src0_offset_tmp, i * num_per_reg, NORM);
        vmul(dst_reg, src0_reg, src1_reg, full_mask, MODE_ZEROING);
        vsts(dst_reg, dst_aligned + dst_offset_tmp, i * num_per_reg, NORM_B32, full_mask);
        src1_reg = dst_reg;
      }
    }
...
// cumprod ra, 2 dim
//===-------------------------------------------------------------------===//
REGISTER_CUMPROD(int8_t) { vector_cumprod_ra<int8_t>(src, dst, reverse); }
REGISTER_CUMPROD(uint8_t) { vector_cumprod_ra<uint8_t>(src, dst, reverse); }
REGISTER_CUMPROD(int16_t) { vector_cumprod_ra<int16_t>(src, dst, reverse); }
REGISTER_CUMPROD(uint16_t) { vector_cumprod_ra<uint16_t>(src, dst, reverse); }
REGISTER_CUMPROD(int32_t) { vector_cumprod_ra<int32_t>(src, dst, reverse); }
REGISTER_CUMPROD(uint32_t) { vector_cumprod_ra<uint32_t>(src, dst, reverse); }
REGISTER_CUMPROD(half) { vector_cumprod_ra<half>(src, dst, reverse); }
REGISTER_CUMPROD(float) { vector_cumprod_ra<float>(src, dst, reverse); }
REGISTER_CUMPROD(bfloat16_t) { vector_cumprod_ra<bfloat16_t>(src, dst, reverse); }
}
