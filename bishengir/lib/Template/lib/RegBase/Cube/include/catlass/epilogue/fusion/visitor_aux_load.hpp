    ...
    struct Callbacks : EmptyCallbacks {
        ...
        CATLASS_DEVICE AscendC::LocalTensor<Element> const& visit(
            ...
            if (stage == VisitStage::LOAD) {
                ...
                // 使用tensorTile的coord创建aux tensor的tile视图
                // tensorTile的coord已经包含全局偏移（通过GetTile累加）
                auto tensorTileAux = GetTile(tensorAux,
                    tensorTile.coord(),  // 使用tensorTile的coord
                    tla::MakeShape(actualRows, actualCols));

                // 使用TLA tile copy
                using CopyGm2UbTlaT = Epilogue::Tile::CopyGm2UbTla<ArchTag, decltype(tensorTileAux), decltype(tensorUb)>;
