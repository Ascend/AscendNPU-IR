...
struct TopologicalVisitor : VisitorImpl<Ops...> {
    ...
    struct Callbacks : CallbacksImpl {
        ...
        template <int I, class ArchTag, class TensorC, class Seq, class... Args>
        CATLASS_DEVICE auto visit_node(
            ...
            auto child_outputs = collect_children<I, ArchTag, TensorC>(
                tensorTile, alignedTileShape, stage, Seq{}, args...
            );

            // 将子节点输出按照 Seq 指定顺序展开，调用第 I 个回调
            auto ret = call_current<I, ArchTag, TensorC>(tensorTile, alignedTileShape, stage, child_outputs);
        ...
        template <int I, class ArchTag, class TensorC, int... ChildIs, class... Args>
        CATLASS_DEVICE auto collect_children(
            ...
            return tla::tuple<decltype(
                this->template visit_node<ChildIs, ArchTag, TensorC>(
        ...
        template <int I, class ArchTag, class TensorC, class ChildOutputs, int... Js>
        CATLASS_DEVICE auto call_current_expand(
            ...
            return tla::get<I>(this->callbacks_tuple).template visit<ArchTag>(
                tensorTile, alignedTileShape, stage,
                tla::get<Js>(child_outputs)...
            );
        }

        template <int I, class ArchTag, class TensorC, class ChildOutputs>
        CATLASS_DEVICE auto call_current(
            ...
            return call_current_expand<I, ArchTag, TensorC>(
                tensorTile, alignedTileShape, stage,
        ...
        CATLASS_DEVICE auto visit(
            ...
            return visit_node<R - 1, ArchTag, TensorC>(tensorTile, alignedTileShape, stage, RootEdges{}, args...);
        }
    };
