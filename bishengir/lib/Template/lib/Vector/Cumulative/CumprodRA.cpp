...
#include "Utils.h"
#include "Vector/Cumulative/CumprodUtils.h"
...
///
/// Constraints:
/// 1. cumprod ra op supports int16_t, int32_t, float16 and float32 types.
...
vector_cumprod_ra(memref_t<__ubuf__ T, 2> *src, memref_t<__ubuf__ T, 2> *dst) {
  // check type T, need to be int16_t, int32_t, floag16 or float32
  static_assert(
      std::is_same<T, int16_t>::value || std::is_same<T, int32_t>::value ||
          std::is_same<T, float>::value || std::is_same<T, half>::value,
      "cumprod ra op only support int16_t, int32_t, float16 and float32 type "
  ...
  for (int64_t i = 1; i < src->sizes[0]; ++i) {
    ...
    vector_eltwise_vv_1d<VectorOpTy::VMUL, T>(&last_dst_1d, &src_1d, &dst_1d);
...
// cumprod ra, 2 dim
//===-------------------------------------------------------------------===//
REGISTE_CUMPROD(2, int16_t)
REGISTE_CUMPROD(2, int32_t)
REGISTE_CUMPROD(2, half)
REGISTE_CUMPROD(2, float)
}