...
template <typename T>
__aicore__ __attribute__((always_inline)) void load_gm_to_cbuf_3d_core(
    memref_t<__gm__ T, 3> *src,
    memref_t<__cbuf__ T, 3> *dst,
  ...
  if (dst->sizes[0] == 1 && dst->sizes[1] == 1 && dst->sizes[2] == 1) {
    ...
    auto dst_ptr = dst->aligned + dst->offset;
    load_gm_to_cbuf_intrin_core(src_ptr, 0, dst_ptr, 0, 1, 1 * sizeof(T),
                                0, 0, pad_mode);
    return;
  }
  ...
  if (src->strides[2] == 1 && dst->strides[2] == 1) [[likely]] {
    ...
    load_gm_to_cbuf_3d_core_with_contiguous_last_dim(src, dst,
                                                     pad_mode);
    return;
  }
  ...
  for (int64_t i = 0; i < size0; i++) {
    ...
    load_gm_to_cbuf_3d_core_with_contiguous_last_dim(
        &gm_3d, &cbuf_3d, pad_mode);
  }
}
