...
print_core_prefix_fmt(__gm__ char *prefix, const int64_t len, const int8_t hex,
                      const __gm__ char **nlast_elem_fmt,
                      const __gm__ char **last_elem_fmt) {
  const __gm__ char *prefix_ptr =
      static_cast<const __gm__ char *>(prefix);
...
CCE_PRINT_CC
[aicore] __attribute__((always_inline)) char
read_bool(BytePtr bytePtr, int64_t index) {
#ifdef CCE_PRINT_SIMT
  return bytePtr[index] ? 'T' : 'F';
#else
  int64_t byteOffset = index / 8;
  int64_t bitOffset = index % 8;
  return ((bytePtr[byteOffset] >> bitOffset) & 1) ? 'T' : 'F';
#endif
}
...
CCE_PRINT_CC
[aicore] __attribute__((always_inline)) void
print_scalar_core(__gm__ char *prefix, const int64_t len, T arg,
                  const int8_t hex) {
...
// Compile-time recursion over dimensions. The loop nest is fully unrolled
// for each CUR_DIM via if constexpr, which results in zero runtime recursion
// overhead.
...
print_nd_core_recursive(const int64_t len, memref_t<MEM_T, RANK> *arg,
                        const int8_t hex, int64_t base, int *cur_iters,
                        const __gm__ char **last_elem_fmt,
                        const __gm__ char **nlast_elem_fmt, MEM_T *arg_ptr) {
  static_assert(CUR_DIM < RANK);
  auto size_i = arg->sizes[CUR_DIM];
  auto stride_i = arg->strides[CUR_DIM];
  float val;
  if constexpr (RANK > 2 && CUR_DIM + 2 == RANK) {
    cce_printf_valid_tensor_type<RANK>(cur_iters);
  }
  if constexpr (CUR_DIM + 2 == RANK || CUR_DIM + 1 == RANK) {
    cce::printf("[");
  }
  if constexpr (CUR_DIM + 1 == RANK) {
    if constexpr (std::is_same_v<T, half> || std::is_same_v<T, bfloat16_t> ||
      ...
      for (int iter = 0; iter < size_i; ++iter) {
        ...
        char val = read_bool(byte_ptr, offset);
        cce::printf(iter < size_i - 1 ? *nlast_elem_fmt : *last_elem_fmt, val);
      }
    } else {
      for (int iter = 0; iter < size_i - 1; ++iter) {
    ...
    if constexpr (RANK == 1) {
      ...
      auto parent_size = arg->sizes[CUR_DIM - 1];
      cce::printf(cur_iters[CUR_DIM - 1] < parent_size - 1 ? "],\n" : "]");
    }
  } else {
    for (int iter = 0; iter < size_i; ++iter) {
      // We count base until the next call prints the innermost dimension.
      auto new_base = base + iter * stride_i;
      cur_iters[CUR_DIM] = iter;
      print_nd_core_recursive<T, MEM_T, CUR_DIM + 1, RANK>(
          len, arg, hex, new_base, cur_iters, last_elem_fmt, nlast_elem_fmt,
          arg_ptr);
    }
  }
  if constexpr (CUR_DIM + 2 == RANK) {
    cce::printf("]\n");
  }
}
...
CCE_PRINT_CC
[aicore] __attribute__((always_inline)) void
print_nd_core(__gm__ char *prefix, const int64_t len,
              memref_t<MEM_T, RANK> *arg, const int8_t hex) {
  pipe_barrier(PIPE_ALL);
  const __gm__ char *nlast_elem_fmt;
  const __gm__ char *last_elem_fmt;
  print_core_prefix_fmt<T>(prefix, len, hex, &nlast_elem_fmt, &last_elem_fmt);
...
CCE_PRINT_CC
[aicore] __attribute__((always_inline)) void
print_assert_msg(const __gm__ T *msg, const int64_t len) {
  const __gm__ char *msg_ptr = static_cast<const __gm__ char *>(msg);
  for (int64_t i = 0; i < len; i++) {
...
static CCE_PRINT_CC
[aicore] __attribute__((always_inline)) int64_t get_debug_block_idx() {
#ifdef CCE_PRINT_SIMT
...
CCE_PRINT_CC
[aicore] __attribute__((always_inline)) void
npuir_cce_assert(bool cond, const __gm__ T *msg, const int64_t len) {
  if (!cond) {
    ...
    cce::printf(
        ...
        get_debug_block_idx(), get_subblockid());
#endif
    print_assert_msg(msg, len);
    cce::printf("%c", '\n');
#endif
...
CCE_PRINT_CC
[aicore] __attribute__((always_inline)) void
assert_scalar_core(__gm__ char *prefix, const int64_t len, bool arg) {
  pipe_barrier(PIPE_ALL);
...
CCE_PRINT_CC
[aicore] __attribute__((always_inline)) void
assert_nd_core_recursive(__gm__ char *prefix, const int64_t len,
...
CCE_PRINT_CC
[aicore] __attribute__((always_inline)) void
assert_nd_core(__gm__ char *prefix, const int64_t len,
               memref_t<MEM_T, RANK> *arg) {
  pipe_barrier(PIPE_ALL);
  auto arg_ptr = arg->aligned + arg->offset;
