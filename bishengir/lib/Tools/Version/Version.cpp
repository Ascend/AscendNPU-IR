...
// Max length of the REVISION to be displayed
static constexpr size_t kRevisionMaxLen = 12;
...
std::string getBiShengIRRevision() {
  ...
  std::string revision = BISHENGIR_REVISION;
  return revision.substr(0, std::min(revision.length(), kRevisionMaxLen));
#else
  return "";
#endif
}
...
#ifdef LLVM_REVISION
  std::string revision = LLVM_REVISION;
  return revision.substr(0, std::min(revision.length(), kRevisionMaxLen));
#else
  return "";
#endif
...
std::string getBiShengIRFullRepositoryVersion() {
  ...
  std::string LLVMVer = getLLVMVersionStr();
  std::string LLVMRev = getLLVMRevision();
  if ( !LLVMVer.empty() || !LLVMRev.empty()) {
    OS << " \nllvm";
    if (!LLVMVer.empty()) {
      OS << ' ' << LLVMVer;
    }
