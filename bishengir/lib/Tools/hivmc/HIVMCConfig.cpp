//===------------- HIVMCConfig.cpp - HIVMC Compile Config -----*- C++-*----===//
//
...
static cl::OptionCategory featCtrlCategory("HIVMC Feature Control Options");
static cl::OptionCategory dfxCtrlCategory("HIVMC DFX Control Options");
static cl::OptionCategory
    generalOptCategory("HIVMC General Optimization Options");
static cl::OptionCategory
    hfusionOptCategory("HIVMC HFusion Optimization Options");
...
struct HIVMCMainConfigCLOptions : public HIVMCMainConfig {
  HIVMCMainConfigCLOptions() {
    ...
    // keeping flag so hivmc doesn't die when bishengir-hivmc-compile
    ...
    static cl::opt<bool, /*ExternalStorage=*/true> enableMixedCV(
        ...
        cl::location(enableMixedCVFlag), cl::init(false));
    ...
    static cl::opt<bool, /*ExternalStorage=*/true> enableAutoVectorizeV2(
        ...
        cl::location(enableAutoVectorizeV2Flag), cl::init(false),
        cl::cat(featCtrlCategory));
    ...
    static cl::opt<bool, /*ExternalStorage=*/true> enableHighPrecision(
        ...
        cl::location(enableHighPrecisionFlag), cl::init(false),
        cl::cat(featCtrlCategory));
...
#endif
    static cl::opt<std::string, /*ExternalStorage=*/true> enableSaveTemps(
        "save-temps",
	cl::desc("Save intermediate compilation results to specified directory"),
        cl::location(saveTempsFlag), cl::init(""),
        cl::cat(dfxCtrlCategory));
    ...
    static cl::opt<bool, /*ExternalStorage=*/true> enableDebugVariables(
        "enable-debug-variables", cl::desc("Enable debug variables"),
        cl::location(enableDebugVariablesFlag), cl::init(false),
        cl::cat(dfxCtrlCategory));
    ...
    cl::callback([this](const int32_t &v) {
      this->simtStackLimitFlag = v;   // populate the std::optional<int32_t>
    }));
    ...
    static cl::opt<bool, /*ExternalStorage=*/true> enableSIMTFastDiv(
        ...
        cl::location(enableSIMTFastDivFlag), cl::init(false),
    ...
    static cl::opt<bool, /*ExternalStorage=*/true> disableReorderInstruction(
        ...
        cl::desc("Disable SIMT reorder instruction pass"),
    ...
    static cl::opt<bool, /*ExternalStorage=*/true> enableSimtReorderInstruction(
        ...
        cl::desc("Enable SIMT reorder instruction pattern"),
    ...
    static cl::opt<unsigned, /*ExternalStorage=*/true> workspaceMultiBufferNum(
        ...
        cl::location(workspaceMultiBufferNumFlag), cl::init(2),
        cl::cat(generalOptCategory));
    ...
    static cl::opt<mlir::hacc::TargetDevice, /*ExternalStorage=*/true> target(
        ...
        cl::init(mlir::hacc::TargetDevice::Unknown),
        cl::values(
...
#undef TO_STRING
    ...
    // -------------------------------------------------------------------------//
    //                       Extra device/host BC options                            //
    // -------------------------------------------------------------------------//

    static cl::opt<std::string, /*ExternalStorage=*/true> extraDeviceBC(
        "link-aicore-bitcode",
        cl::desc("Extra device bitcode files to link (comma-separated)"),
        cl::value_desc("path1.bc,path2.bc"),
        cl::location(extraDeviceBCRawFlag),
        cl::init(""),
        cl::cat(targetCategory));

    static cl::opt<std::string, /*ExternalStorage=*/true> extraHostBC(
        "link-host-bitcode",
        cl::desc("Extra host bitcode files to link (comma-separated)"),
        cl::value_desc("path1.bc,path2.bc"),
        cl::location(extraHostBCRawFlag),
        cl::init(""),
        cl::cat(targetCategory));
    ...
    static llvm::cl::opt<std::string, /*ExternalStorage=*/true>
        debugLlvmActions("debug-llvm", llvm::cl::desc("Debug LLVM action list"),
                         cl::location(debugLLVMActions), cl::init(""));

    static llvm::cl::opt<std::string, /*ExternalStorage=*/true>
        debugLlvmCacheFilePath("debug-llvm-cache-file-path",
                               llvm::cl::desc("Debug LLVM cache file path"),
                               cl::location(debugLLVMCacheFilePath),
                               cl::init(""));
  }
...
ManagedStatic<HIVMCMainConfigCLOptions> clOptionsConfig;

void HIVMCMainConfig::registerCLOptions() {
...
HIVMCMainConfig HIVMCMainConfig::createFromCLOptions() {
  // Enforce <= 3 items
  ...
  // Parse comma-separated extra device BC paths
  std::string extraDeviceBcStr = clOptionsConfig->extraDeviceBCRawFlag;
  if (!extraDeviceBcStr.empty()) {
    std::vector<std::string> paths;
    llvm::StringRef str(extraDeviceBcStr);
    llvm::StringRef token;
    while (!str.empty()) {
      std::tie(token, str) = str.split(',');
      if (!token.empty())
        paths.push_back(token.str());
    }
    clOptionsConfig->setExtraDeviceBCPaths(paths);
  }

  // Parse comma-separated extra host BC paths
  std::string extraHostBcStr = clOptionsConfig->extraHostBCRawFlag;
  if (!extraHostBcStr.empty()) {
    std::vector<std::string> paths;
    llvm::StringRef str(extraHostBcStr);
    llvm::StringRef token;
    while (!str.empty()) {
      std::tie(token, str) = str.split(',');
      if (!token.empty())
    ...
    clOptionsConfig->setExtraHostBCPaths(paths);
  }

  return *clOptionsConfig;
}

void HIVMCMainConfigCLOptions::setCubeTilingTuningCallback() {
...
void HIVMCMainConfigCLOptions::setGridDimCallback() {
