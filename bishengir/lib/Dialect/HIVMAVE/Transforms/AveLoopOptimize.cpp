...
//
// Before unrolling, a shared AVE cost model estimates load, execute and store
// pressure for the complete loop. The pass applies the candidate load,
// elementwise and store deltas and skips plans that would raise the largest
// pipeline cost.
...
static AVEPipelineCost estimateGroupDelta(const SmallWidthGroup &group,
  ...
  if (group.kind == GroupKind::Load) {
    ...
    for (Operation *op :
      ...
      // packs that operand separately.
      for (Value operand : op->getOperands()) {
        if (operand.getType() == op->getResult(0).getType() &&
            !packedResults.contains(operand))
          ++packTreeCount;
      }
...
struct aveLoopOptimizePass
  ...
  void runOnOperation() override {
    ...
    //   narrowing conversion -> supported narrow elementwise ops ->
    //   masked_store
    ...
    // and complete clone groups. Dynamic/nonlinear addresses, cross-lane
    // chains, unsupported masks/types and continuous ONEPT stores remain
    // unchanged.
