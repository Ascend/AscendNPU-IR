...
struct HoistScalarMulFromMatmulPattern
  ...
  LogicalResult matchAndRewrite(linalg::ElemwiseBinaryOp elemOp,
    ...
    // Only hoist if the matmul's init/accumulator is zero. The rewrite
    // s * (A*B + init) -> (s*A)*B + init is only valid when init = 0.
    // Otherwise the init term is left unscaled, producing incorrect results.
    Value init = linalgMatmul.getDpsInits()[0];
    if (!isConstZero(init))
      return failure();
