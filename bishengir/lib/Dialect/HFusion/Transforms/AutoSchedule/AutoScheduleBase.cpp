...
LogicalResult SchedulerBase::markTilingData() { return success(); }
LogicalResult SchedulerBase::runScheduleProcedure(OpBuilder &opBuilder) {
  ...
  if (failed(markTilingData()))
    return currentFunc->emitWarning("Failed to mark tiling data.");
...
LogicalResult SchedulerBase::cacheIO(OpBuilder &opBuilder) {
  ...
  /// Use symbol analysis to merge shape values for tensor.empty
  if (failed(applySymbolAnalysisPass(originalKernel)))
    return failure();
...
LogicalResult SchedulerBase::applySchedule(func::FuncOp &funcOp,
  ...
  if (funcOp->hasAttr("hivm.vector_function")) {
    // use specialized scheduler for VF function
    // TODO: use hacc attr for vf function
    auto scheduler = std::make_unique<VFScheduler>(funcOp);
...
LogicalResult
SchedulerBase::applySymbolAnalysisPass(func::FuncOp target) const {
  AutoScheduleOptions options = getAutoScheduleOptions();
  if (!options.enableSymbolAnalysis) {
    return success();
  }
  PassManager pm(getContext());
  pm.addPass(symbol::createPropagateSymbolPass());
  pm.addPass(symbol::createUnfoldSymbolicIntPass());
LogicalResult SchedulerBase::applyRemoveCacheIOPass(func::FuncOp target) {
  PassManager pm(getContext());
  pm.addPass(mlir::hfusion::createRemoveCacheIO());
  if (failed(pm.run(target)))
    return failure();
  return success();
}
