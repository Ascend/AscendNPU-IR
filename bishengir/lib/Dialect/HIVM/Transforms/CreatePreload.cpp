...
static bool isPreloadWorkspaceSubview(memref::SubViewOp subviewOp) {
  // Master places the preload-workspace mark on the subview itself (see
  // CVPipelining::createAttrForPreloadWS). Also accept a marked source so
  // subviews derived from an already-marked workspace root are detected.
