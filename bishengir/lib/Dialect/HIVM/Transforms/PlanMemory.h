...
public:
  MemLivenessAnalysis(func::FuncOp func, MemPlanMode planMode,
                      ...
                      const DenseSet<int32_t> *aivUbCVMixIds = nullptr,
                      const DenseSet<int32_t> *aicL1CVMixIds = nullptr)
      : func_(func), planMode(planMode),
        ...
        aivUbCVMixIds(aivUbCVMixIds), aicL1CVMixIds(aicL1CVMixIds) {}
...
private:
  ...
  /// when a peer AIV UB buffer with the same cvMixId will supply the offset.
  DenseSet<Value> skipMemPlan;

  /// cvMixIds that have a peer-core tightly coupled buffer to supply offsets.
  const DenseSet<int32_t> *aivUbCVMixIds{nullptr};
  const DenseSet<int32_t> *aicL1CVMixIds{nullptr};
};
