...
static std::optional<int64_t> extractIntFromAttr(Attribute a) {
  if (auto i = a.mlir::dyn_cast<IntegerAttr>()) {
    return i.getValue().getSExtValue();
  }
  if (auto dense = a.mlir::dyn_cast<DenseElementsAttr>()) {
    if (dense.getType().getElementType().mlir::isa<IntegerType>()) {
      if (dense.isSplat()) {
        Attribute v = dense.getSplatValue<Attribute>();
        if (auto ia = v.mlir::dyn_cast<IntegerAttr>())
...
static Value ensureValueWithNewLastDim(
  ...
  Type t = v.getType();
  auto rt = t.mlir::dyn_cast<RankedTensorType>();
  if (!rt)
    return v;
  ...
  if (Operation *def = v.getDefiningOp()) {
    if (clonedOpsMap.count(def)) {
      ...
      unsigned idx = v.mlir::cast<OpResult>().getResultNumber();
...
cloneConstantOp(PatternRewriter &rewriter, Location loc,
  ...
  if (!newResultTypes.empty()) {
    if (auto newResRT = dyn_cast<RankedTensorType>(newResultTypes[0])) {
      ...
      } else if (valAttr.mlir::isa<IntegerAttr>() || valAttr.mlir::isa<FloatAttr>()) {
