...
struct MemInfo_membase {
  Value value{nullptr};
  std::optional<PointerLikeInfo> pointerLikeInfo;
  bool isWorkSpace{false};

  MemInfo_membase() = default;
  explicit MemInfo_membase(Value value, bool isWorkSpace = false)
      : value(value), isWorkSpace(isWorkSpace) {}

  explicit MemInfo_membase(Value value, PointerLikeInfo pointerLikeInfo,
                   bool isWorkSpace = false)
...
MemInfo_membase getMemInfo_membase(Value val);
