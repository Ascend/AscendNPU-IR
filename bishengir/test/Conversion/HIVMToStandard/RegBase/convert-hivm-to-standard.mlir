...
module {
  ...
  func.func @test_mmadmxL1_transpose() {
    ...
    // CHECK: call @mmadmxL1_float8_e5m2_t_float8_e5m2_t_to_float_ta
    ...
    // CHECK: call @mmadmxL1_float8_e5m2_t_float8_e5m2_t_to_float_tb
    ...
    // CHECK: call @mmadmxL1_float8_e5m2_t_float8_e5m2_t_to_float_ta_tb
...
// -----
module {
  // CHECK-LABEL: test_mmadmxL1_mixed_native_fp8
  func.func @test_mmadmxL1_mixed_native_fp8() {
    ...
    // CHECK: call @mmadmxL1_float8_e4m3_t_float8_e5m2_t_to_float
    hivm.hir.mmadmxL1
    ...
    // CHECK: call @mmadmxL1_float8_e4m3_t_float8_e5m2_t_to_float_ta
    hivm.hir.mmadmxL1 {a_transpose}
      ins(%ma_t, %mb, %scaleA, %scaleB, %init, %c256, %c128, %c256 :
    ...
    // CHECK: call @mmadmxL1_float8_e4m3_t_float8_e5m2_t_to_float_tb
    hivm.hir.mmadmxL1 {b_transpose}
      ins(%ma, %mb_t, %scaleA, %scaleB, %init, %c256, %c128, %c256 :
    ...
    // CHECK: call @mmadmxL1_float8_e4m3_t_float8_e5m2_t_to_float_ta_tb
    hivm.hir.mmadmxL1 {a_transpose, b_transpose}
      ins(%ma_t, %mb_t, %scaleA, %scaleB, %init, %c256, %c128, %c256 :
...
// -----
module {
  // CHECK-LABEL: func.func @convert_conv2d_scalar_stride(
  // CHECK-DAG: %[[GROUP:.*]] = arith.constant 2 : i64
  // CHECK-DAG: %[[STRIDE:.*]] = arith.constant 3 : i64
  // CHECK: call @conv2d_group_half_to_float({{.*}}, %[[GROUP]], {{.*}}, %[[STRIDE]], %[[STRIDE]], {{.*}})
  // CHECK-NOT: hivm.hir.Conv2dL1
  func.func @convert_conv2d_scalar_stride(
      %input: memref<1x2x1x128x16xf16, #hivm.address_space<cbuf>>,
      %weight: memref<1x1x5x32x16xf16, #hivm.address_space<cbuf>>,
      %output: memref<128x32xf32, #hivm.address_space<cc>>) {
    %true = arith.constant true
