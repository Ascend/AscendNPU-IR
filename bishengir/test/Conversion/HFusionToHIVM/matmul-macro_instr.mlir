...
// CHECK: scope.scope : () -> ()
// CHECK: %[[OUT:.+]] = bufferization.to_tensor %arg0 restrict writable : memref<16x16xf32>
...
// CHECK: %[[RET:.+]] = hivm.hir.mmadL1 ins(%[[A]], %[[B]], %[[FALSE]], %[[C0]], %[[C0]], %[[C0]] : tensor<16x16xf16>, tensor<16x16xf16>, i1, index, index, index) outs(%[[OUT]] : tensor<16x16xf32>) -> tensor<16x16xf32>
// CHECK: %[[CAST:.+]] = memref.cast %arg0 : memref<16x16xf32> to memref<16x16xf32, strided<[16, 1]>>
// CHECK: bufferization.materialize_in_destination %[[RET]] in writable %[[CAST]] : (tensor<16x16xf32>, memref<16x16xf32, strided<[16, 1]>>) -> ()
// CHECK: scope.return
// CHECK: } {hivm.matmul_limited_in_cube, hivm.tcore_type = #hivm.tcore_type<CUBE>}
// CHECK: return
func.func @test_matmul_with_scope_matmul_limited_in_cube(%arg1: memref<16x16xf32>){
  scope.scope : () -> () {
    ...
    %ret = linalg.matmul ins(%ma_tensor, %mb_tensor : tensor<16x16xf16>, tensor<16x16xf16>) outs(%mc : tensor<16x16xf32>) -> tensor<16x16xf32>
    %subview_104 = memref.subview %arg1[0, 0][16, 16][1, 1] : memref<16x16xf32> to memref<16x16xf32, strided<[16, 1], offset: 0>>
    bufferization.materialize_in_destination %ret in writable %subview_104 : (tensor<16x16xf32>, memref<16x16xf32, strided<[16, 1], offset: 0>>) -> ()
    scope.return
