...
// CHECK-NEXT:   1. The operation type may not be handled in solveProblem TypeSwitch.
// CHECK-NEXT:      -> Add a new Case for ave.hir.vtrc in solveProblem().
// CHECK-NEXT:   2. The specific VecMemType combination is not supported by this op.
// CHECK-NEXT:      -> Check COMB_CASE branches in the corresponding solveProblem function.
// CHECK-NEXT:   3. Conflicting layout requirements from multiple consumers.
// CHECK-NEXT:      -> Check if the operands/results have incompatible layout constraints.
// CHECK-NEXT:   4. Unsupported element bitwidth (only 1/8/16/32 are supported).
