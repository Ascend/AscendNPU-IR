...
func.func @test_load_b8(%arg0: memref<128xi32, #hivm.address_space<ub>>, %arg1: memref<128xi1, #hivm.address_space<ub>>, %arg2: memref<128xi32, #hivm.address_space<ub>>, %arg3: memref<128xi32, #hivm.address_space<ub>>) attributes {hivm.func_core_type = #hivm.func_core_type<AIV>, hivm.vector_function, no_inline} {
  ...
  scf.for %arg4 = %c0 to %c128 step %c64 {
    ...
    %res = ave.hir.vload <NORM> %reinterpret_cast[%c0] {functionType = #ave.func_dist_type<pb32>} : memref<64xi32, strided<[1], offset: ?>, #hivm.address_space<ub>> into vector<64xi32>
    // CHECK: "hivm_regbaseintrins.intr.hivm.vldas"({{.*}}) : (!llvm.ptr<6>) -> vector<32xi8>
    // CHECK: "hivm_regbaseintrins.intr.hivm.vldus.post.s32"({{.*}}, {{.*}}, {{.*}}) : (!llvm.ptr<6>, vector<32xi8>, i32) -> !llvm.struct<(vector<64xi32>, vector<32xi8>, ptr<6>)>
    // CHECK: "hivm_regbaseintrins.intr.hivm.movvp"({{.*}}, {{.*}}) : (vector<64xi32>, i32) -> vector<256xi1>
...
%4 = ave.hir.pge <ALLF>  : vector<64xi1>
scf.for %arg5 = %c0 to %c16 step %c1 {
  scf.for %arg6 = %c0 to %c80 step %c64 {
    ...
    %6 = ave.hir.pltm %arg6, %c80  : vector<64xi1>
    // CHECK: "hivm_regbaseintrins.intr.hivm.vldas"({{.*}}) : (!llvm.ptr<6>) -> vector<32xi8>
    // CHECK: "hivm_regbaseintrins.intr.hivm.vldus.post.s32"({{.*}}, {{.*}}, {{.*}}) : (!llvm.ptr<6>, vector<32xi8>, i32) -> !llvm.struct<(vector<64xi32>, vector<32xi8>, ptr<6>)>
    // CHECK: "hivm_regbaseintrins.intr.hivm.movvp"({{.*}}, {{.*}}) : (vector<64xi32>, i32) -> vector<256xi1>
    %res = ave.hir.vload <NORM> %reinterpret_cast[%c0] {ave.unaligned_ub_access = #ave.unaligned_ub_access, functionType = #ave.func_dist_type<pb32>} : memref<?xi1, strided<[1], offset: ?>, #hivm.address_space<ub>> into vector<64xi1>
    %res_6 = ave.hir.vload <BRC_B32> %arg2[%c0]  : memref<1xf32, #hivm.address_space<ub>> into vector<64xf32>
    %8 = ave.hir.vmul %res_6, %res_6, %6  : vector<64xf32>, vector<64xi1>
    %9 = ave.hir.preg.xor <b8> %res, %4, %6  : vector<64xi1>
    %10 = ave.hir.vsel %9, %8, %3  : vector<64xi1>, vector<64xf32>
    ...
    ave.hir.masked_store <NORM_B32> %reinterpret_cast_11[%c0], %6, %10 {hivm.is_continuous} : memref<?xf32, affine_map<(d0)[s0] -> (d0 + s0)>, #hivm.address_space<ub>>, vector<64xi1>, vector<64xf32>
  }
}
return
}
...
// Test for i1 load/store with functionType-based pbMode (replaces element_alignment_bit_width)
func.func @test_i1_load_with_function_type_pb32(%arg0: memref<64xi1, #hivm.address_space<ub>>, %arg1: memref<64xi32, #hivm.address_space<ub>>) attributes {hivm.func_core_type = #hivm.func_core_type<AIV>, hivm.vector_function, no_inline} {
  %c0 = arith.constant 0 : index
  
  // Test case where functionType=pb32 drives pbMode=32 in load lowering
  // This should trigger the code path at line 1055-1058
  %0 = ave.hir.pge <ALL>  : vector<64xi1>
  
  // CHECK: "hivm_regbaseintrins.intr.hivm.movvp"({{.*}}, {{.*}}) : (vector<64xi32>, i32) -> vector<256xi1>
  %res = ave.hir.vload <NORM> %arg0[%c0] {ave.unaligned_ub_access = #ave.unaligned_ub_access, functionType = #ave.func_dist_type<pb32>} : memref<64xi1, #hivm.address_space<ub>> into vector<64xi1>
  
  // Store result to verify the functionType-driven pbMode is correctly propagated
  %1 = ave.hir.pge <ALL>  : vector<64xi1>
  ave.hir.masked_store <NORM_B32> %arg1[%c0], %1, %res  : memref<64xi32, #hivm.address_space<ub>>, vector<64xi1>, vector<64xi1>
