// RUN: bishengir-opt -memref-dse  -allow-unregistered-dialect -split-input-file %s | FileCheck %s
...
func.func @memref_view_subview(%arg1 : memref<4xi8>) -> (i8, i8) {
  ...
  hivm.hir.vreduce <min> ins(%alloc : memref<4xi8>) outs(%subview : memref<1xi8, strided<[5]>>) unsigned_src = false reduce_dims = [0]
  %load = memref.load %alloc[%c0] : memref<4xi8>

  %view = memref.view %alloc[%c0][] : memref<4xi8> to memref<1xi8>
  memref.store %cst, %alloc[%c0] : memref<4xi8>

  hivm.hir.vreduce <min> ins(%alloc : memref<4xi8>) outs(%view : memref<1xi8>) unsigned_src = false reduce_dims = [0]
...
module {
  func.func @view_like_operation() -> f32 {
    ...
    hivm.hir.vreduce <sum> ins(%reinterpret_cast : memref<29x1xf32, strided<[8, 1]>, #hivm.address_space<ub>>) outs(%reinterpret_cast_7 : memref<1x1xf32, strided<[1, 1]>, #hivm.address_space<ub>>) temp_buffer(%alloc_8 : memref<232xf32, #hivm.address_space<ub>>) unsigned_src = false reduce_dims = [0]
...
// CHECK: %[[ALLOC:.*]] = memref.alloc() {alignment = 64 : i64} : memref<16x16xf32, #hivm.address_space<ub>>
// CHECK: annotation.mark %[[ALLOC]] {effects = ["write", "read"]} : memref<16x16xf32, #hivm.address_space<ub>>
// CHECK:  "some_operation"(%[[ALLOC]]) : (memref<16x16xf32, #hivm.address_space<ub>>) -> ()
...
// CHECK-LABEL: func.func @remove_unused_load(
