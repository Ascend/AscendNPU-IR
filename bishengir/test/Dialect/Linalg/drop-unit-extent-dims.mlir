// RUN: bishengir-opt %s -linalg-fold-unit-extent-dims="enable-experimental-dropping" -split-input-file | FileCheck %s
// RUN: bishengir-opt %s -linalg-fold-unit-extent-dims="enable-experimental-dropping avoid-zero-ranks" -split-input-file | FileCheck --check-prefixes=CHECK-RSTR %s
...
//   CHECK-DAG: %[[INPUT_RESHAPE:.+]] = tensor.collapse_shape %{{.+}} {{\[}}[0, 1]] : tensor<1x1000xf32> into tensor<1000xf32>
//   CHECK-DAG: %[[INIT:.+]] = tensor.empty() : tensor<f32>
//   CHECK-DAG: %[[FILL:.+]] = linalg.fill ins(%cst : f32) outs(%[[INIT]] : tensor<f32>) -> tensor<f32>
...
//  CHECK-DAG: %[[INIT:.+]] = tensor.empty(%{{.+}}) : tensor<?xf32>
//  CHECK-DAG: %[[FILL:.+]] = linalg.fill ins(%{{.+}}{{.*}}outs(%[[INIT]]
//      CHECK: %[[RESULT:.+]] = linalg.generic
...
//  CHECK-DAG:   %[[INIT:.+]] = tensor.empty() : tensor<f32>
//  CHECK-DAG:   %[[FILL:.+]] = linalg.fill ins(%{{.+}}{{.*}}outs(%[[INIT]]
//  CHECK-DAG:   %[[EXPANDED:.+]] = tensor.expand_shape %[[FILL]] [] output_shape [1]
//  CHECK-DAG:   %[[INIT2:.+]] = tensor.empty() : tensor<1xf32>
//      CHECK:   %[[RESULT:.+]] = linalg.generic
// CHECK-SAME:     indexing_maps = [#[[MAP2]], #[[MAP2]], #[[MAP2]]]
// CHECK-SAME:     iterator_types = ["parallel"]
// CHECK-SAME:     ins(%[[RESHAPE]], %[[EXPANDED]] : tensor<?xf32>, tensor<1xf32>)
...
// -----
// Test that the logic with affine maps works as well.
...
// CHECK-DAG:      %[[COLLAPSED:.*]] = memref.collapse_shape %[[ARG0]] {{\[}}[0, 1], [2]]
// CHECK-DAG:      %[[OUT:.*]] = memref.collapse_shape %[[ARG2]] {{\[}}[0, 1], [2, 3], [4]]
// CHECK-SAME:   : memref<?x1x?x1x?xf32> into memref<?x?x?xf32>
// CHECK:      linalg.generic
// CHECK-SAME:   {indexing_maps = [#[[MAP1]], #[[MAP2]], #[[MAP3]]],
// CHECK-SAME:   iterator_types = ["parallel", "parallel", "parallel"]}
// CHECK-SAME:   ins(%[[COLLAPSED]], %[[ARG1]] : memref<?x?xf32, strided<[?, 1]>>, f32)
...
// CHECK:           %[[VAL_14:.*]] = tensor.cast %[[VAL_7]] : tensor<?x61xf32> to tensor<0x61xf32>
// CHECK:           %[[VAL_15:.*]] = tensor.expand_shape %[[VAL_14]] {{\[\[}}0, 1], [2, 3]] output_shape {{\[}}0, 1, 61, 1] : tensor<0x61xf32> into tensor<0x1x61x1xf32>
// CHECK:           %[[VAL_16:.*]] = tensor.cast %[[VAL_15]] : tensor<0x1x61x1xf32> to tensor<?x1x61x1xf32>
// CHECK:           return %[[VAL_16]] : tensor<?x1x61x1xf32>
// CHECK:         }
...
func.func @restore_one_unit_dim(%arg0 : tensor<1x2x1xf32>) -> tensor<1x10x1xf32>
{
  %c0 = arith.constant 0. : f32
  %0 = tensor.empty() : tensor<1x1xf32>
  %1 = linalg.fill ins(%c0: f32) outs(%0: tensor<1x1xf32>) -> tensor<1x1xf32>
  %2 = linalg.reduce ins(%arg0: tensor<1x2x1xf32>) outs(%1: tensor<1x1xf32>) dimensions = [1] (%20: f32, %21: f32){
    %22 = arith.addf %20, %21: f32
    linalg.yield %22: f32
  }
  %3 = tensor.empty() : tensor<1x1x10xf32>
  %4 = linalg.broadcast ins(%2: tensor<1x1xf32>) outs(%3: tensor<1x1x10xf32>) dimensions = [2]
  %5 = tensor.empty() : tensor<1x10x1xf32>
  %6 = linalg.transpose ins(%4: tensor<1x1x10xf32>) outs(%5: tensor<1x10x1xf32>) permutation = [1, 2, 0]
  return %6: tensor<1x10x1xf32>
}

// CHECK-LABEL:   @restore_one_unit_dim
// CHECK-SAME:          %[[INPUT:.*]]:
// CHECK:           %[[COLLAPSED:.*]] = tensor.collapse_shape %[[INPUT]] {{\[\[}}0, 1, 2]]
// CHECK:           %[[REDUCED:.*]] = linalg.reduce ins(%[[COLLAPSED]] : tensor<2xf32>)
// CHECK-SAME:                        dimensions = [0]
// CHECK:           %[[BROADCAST:.*]] = linalg.broadcast ins(%[[REDUCED]] : tensor<f32>)
// CHECK-SAME:                        dimensions = [0]
// CHECK:           %[[EXPANDED:.*]] = tensor.expand_shape %[[BROADCAST]] {{\[\[}}0, 1, 2]] output_shape [1, 10, 1]
// CHECK:           return %[[EXPANDED]]

// CHECK-RSTR-LABEL:   @restore_one_unit_dim
// CHECK-RSTR-SAME:      %[[INPUT:.*]]:
// CHECK-RSTR:           %[[COLLAPSED:.*]] = tensor.collapse_shape %[[INPUT]] {{\[\[}}0, 1, 2]]
// CHECK-RSTR:           %[[REDUCED:.*]] = linalg.reduce ins(%[[COLLAPSED]] : tensor<2xf32>)
// CHECK-RSTR-SAME:                        dimensions = [0]
// CHECK-RSTR:           %[[BROADCAST:.*]] = linalg.broadcast ins(%[[REDUCED]] : tensor<f32>)
// CHECK-RSTR-SAME:                        dimensions = [0]
// CHECK-RSTR:           %[[EXPANDED:.*]] = tensor.expand_shape %[[BROADCAST]] {{\[\[}}0, 1, 2]] output_shape [1, 10, 1]
// CHECK-RSTR:           return %[[EXPANDED]]

// -----

func.func @dont_restore_from_rank_0(%arg0 : tensor<1xf32>) -> tensor<f32>
{
  %c0 = arith.constant 0. : f32
  %0 = tensor.empty() : tensor<f32>
  %1 = linalg.fill ins(%c0: f32) outs(%0: tensor<f32>) -> tensor<f32>
  %2 = linalg.reduce ins(%arg0: tensor<1xf32>) outs(%1: tensor<f32>) dimensions = [0] (%20: f32, %21: f32){
    %22 = arith.addf %20, %21: f32
    linalg.yield %22: f32
  }
  return %2: tensor<f32>
...
// CHECK-RSTR-LABEL:   @dont_restore_from_rank_0
// CHECK-RSTR-SAME:                  %[[ARG:.*]]:
// CHECK-RSTR:           %[[FILL:.*]] = linalg.fill
// CHECK-RSTR-SAME:                       -> tensor<f32>
// CHECK-RSTR:           %[[REDUCE:.*]] = linalg.reduce ins(%[[ARG]] : tensor<1xf32>) outs(%[[FILL]] : tensor<f32>) dimensions = [0]
// CHECK-RSTR:           return %[[REDUCE]]