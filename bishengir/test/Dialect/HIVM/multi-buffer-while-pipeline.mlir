...
// Two non-overlapping phases of equal size then share physical multi-buffer
// addresses from PlanMemory (77824 / 116736 / 0 each appear twice), while
// EnableMultiBuffer emits slot selects on the shared counter.
...
func.func @a5_mix_vector_multibuffer_reuse(
  ...
  // A5-DAG: %[[C155648:.*]] = arith.constant 155648 : i64
  ...
  // A5: hivm.hir.pointer_cast(%[[C155648]]) : memref<9728xf32, #hivm.address_space<ub>>
  ...
  // Pre-loop set_flag pre-roll (pipe directions follow current GSS on A5).
  // A5-DAG: hivm.hir.set_flag[<PIPE_MTE3>, <PIPE_V>, <EVENT_ID1>]
  // A5-DAG: hivm.hir.set_flag[<PIPE_MTE3>, <PIPE_V>, <EVENT_ID2>]

  scf.for %i = %c0 to %c4 step %c1 {
    // A5: %[[CUR:.*]] = memref.load %[[CTR]]
    // A5: arith.remui %[[CUR]], %{{.*}} : i64
    // A5: arith.select {{.*}} : memref<9728xf32, #hivm.address_space<ub>>
    // A5: hivm.hir.wait_flag[<PIPE_MTE3>, <PIPE_V>, %{{.*}}]
