...
// -----
// ScaleA is a function arg (no memref.alloc root) → error path.
module {
  func.func @test_infer_mem_scope_mmadmx_missing_scale_alloc(
      %scaleA: memref<1xui8>, %scaleB: memref<1xui8>) attributes {hacc.function_kind = #hacc.function_kind<DEVICE>} {
    %c16 = arith.constant 16 : index
    %true = arith.constant true
    %a = memref.alloc() : memref<16x16xf8E5M2>
    %b = memref.alloc() : memref<16x16xf8E5M2>
    %c = memref.alloc() : memref<16x16xf32>
    // expected-error@+1 {{Cannot find root memref.alloc for scaleA of this op}}
    hivm.hir.mmadmxL1
      ins(%a, %b, %scaleA, %scaleB, %true, %c16, %c16, %c16 :
...
// -----
// ScaleB is a function arg (no memref.alloc root) → error path.
module {
  func.func @test_infer_mem_scope_mmadmx_missing_scaleb_alloc(
      %scaleB: memref<1xui8>) attributes {hacc.function_kind = #hacc.function_kind<DEVICE>} {
    %c16 = arith.constant 16 : index
    %true = arith.constant true
    %a = memref.alloc() : memref<16x16xf8E5M2>
    %b = memref.alloc() : memref<16x16xf8E5M2>
    ...
    %c = memref.alloc() : memref<16x16xf32>
    // expected-error@+1 {{Cannot find root memref.alloc for ScaleB of this op}}
    hivm.hir.mmadmxL1
      ins(%a, %b, %scaleA, %scaleB, %true, %c16, %c16, %c16 :
          memref<16x16xf8E5M2>, memref<16x16xf8E5M2>,
          memref<1xui8>, memref<1xui8>, i1, index, index, index)
