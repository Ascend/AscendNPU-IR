...
module {
  ...
  func.func @test_madL1_with_PerChannelAdd_in_cond(%arg2: memref<?xf16>, %arg3: memref<?xf16>, %arg4: memref<?xf16>, %arg5: memref<?xf32>, %cond: i1) {
    ...
    // CHECK: memref.store %[[C0:.*]], %[[CNT:.*]][] : memref<i32>
...
// -----
module {
  // CHECK-LABEL: func.func @test_madL1_with_PostPerChannelAdd_withSplitKFalseInit_in_cond(
  func.func @test_madL1_with_PostPerChannelAdd_withSplitKFalseInit_in_cond(%arg2: memref<?xf16>, %arg3: memref<?xf16>, %arg4: memref<?xf16>, %arg5: memref<?xf32>, %cond: i1) {
    %false = arith.constant false
    %c0_i32 = arith.constant 0 : i32
    %c1_i32 = arith.constant 1 : i32
    %c5_i32 = arith.constant 5 : i32
    ...
    %c32 = arith.constant 32 : index
    %c128 = arith.constant 128 : index
    %alloc = memref.alloc() : memref<16x128xf16>
    %9 = bufferization.to_tensor %alloc restrict writable : memref<16x128xf16>
    %alloc_3 = memref.alloc() : memref<128x128xf16>
    %10 = bufferization.to_tensor %alloc_3 restrict writable : memref<128x128xf16>
    %alloc_4 = memref.alloc() : memref<1x128xf32>
    %reinterpret_cast_1 = memref.reinterpret_cast %arg5 to offset: [0], sizes: [1, 128], strides: [128, 1] : memref<?xf32> to memref<1x128xf32, strided<[128, 1]>>
    hivm.hir.load ins(%reinterpret_cast_1 : memref<1x128xf32, strided<[128, 1]>>) outs(%alloc_4 : memref<1x128xf32>)
    %11 = bufferization.to_tensor %alloc_4 restrict writable : memref<1x128xf32>
    %13 = arith.muli %c1_i32, %c8_i32 : i32
    %15 = arith.minsi %13, %c8_i32 : i32
    %17 = tensor.empty() : tensor<16x128xf32>
    // CHECK: %[[CNT:.*]] = memref.alloca() : memref<i32>
    // CHECK: %[[C0:.*]] = arith.constant 0 : i32
    // CHECK: memref.store %[[C0]], %[[CNT]][] : memref<i32>
    // CHECK: %[[EMPTY_INIT:.*]] = tensor.empty() : tensor<16x128xf32>
    // CHECK: %[[FOR_RES:.*]] = scf.for {{.*}} iter_args({{.*}} = %[[EMPTY_INIT]]) -> (tensor<16x128xf32>) : i32 {
    %18 = scf.for %arg9 = %c0_i32 to %c5_i32 step %c1_i32 iter_args(%arg10 = %17) -> (tensor<16x128xf32>) : i32 {
      // CHECK: %[[IFRES:.*]] = scf.if %cond {
      // CHECK: %[[LOAD:.*]] = memref.load %[[CNT]][] : memref<i32>
      // CHECK: %[[INIT:.*]] = arith.cmpi eq, %[[LOAD]], %[[C0]] : i32
      // CHECK: %[[MMAD:.*]] = hivm.hir.mmadL1 ins({{.*}}, {{.*}}, %[[INIT]], {{.*}}, {{.*}}, {{.*}}, %[[BRC_1D]] : tensor<16x128xf16>, tensor<128x128xf16>, i1, index, index, index, tensor<1x128xf32>) outs({{.*}} : tensor<16x128xf32>) -> tensor<16x128xf32>
      // CHECK: %[[ONE:.*]] = arith.constant 1 : i32
      // CHECK: %[[NC:.*]] = arith.addi %[[LOAD]], %[[ONE]] : i32
      // CHECK: memref.store %[[NC]], %[[CNT]][] : memref<i32>
      // CHECK: scf.yield %[[MMAD]] : tensor<16x128xf32>
      // CHECK: } else {
      // CHECK: scf.yield %arg10 : tensor<16x128xf32>
      // CHECK: }
      %ifres = scf.if %cond -> tensor<16x128xf32> {
        %43 = hivm.hir.mmadL1 ins(%9, %10, %false, %c16, %c128, %c128 : tensor<16x128xf16>, tensor<128x128xf16>, i1, index, index, index) outs(%arg10 : tensor<16x128xf32>) -> tensor<16x128xf32>
        scf.yield %43 : tensor<16x128xf32>
      } else {
        scf.yield %arg10 : tensor<16x128xf32>
      }
      scf.yield %ifres : tensor<16x128xf32>
    }
    // CHECK-NOT: hivm.hir.vbrc
    %5 = tensor.empty() : tensor<16x128xf32>
    %19 = hivm.hir.vbrc ins(%11 : tensor<1x128xf32>) outs(%5 : tensor<16x128xf32>) broadcast_dims = [0] -> tensor<16x128xf32>
    // CHECK-NOT: hivm.hir.vadd
    %20 = hivm.hir.vadd ins(%18, %19 : tensor<16x128xf32>, tensor<16x128xf32>) outs(%5 : tensor<16x128xf32>) -> tensor<16x128xf32>
    // CHECK: %[[POST_LOAD:.*]] = memref.load %[[CNT]][] : memref<i32>
    // CHECK: %[[POST_COND:.*]] = arith.cmpi eq, %[[POST_LOAD]], %[[C0]] : i32
    // CHECK: %[[FB:.*]] = scf.if %[[POST_COND]] {
    // CHECK: %[[BRC_INIT:.*]] = tensor.empty() : tensor<16x128xf32>
    // CHECK: %[[BRC:.*]] = hivm.hir.vbrc ins(%[[BRC_1D]] : tensor<1x128xf32>) outs(%[[BRC_INIT]] : tensor<16x128xf32>) broadcast_dims = [0] -> tensor<16x128xf32>
    // CHECK: scf.yield %[[BRC]] : tensor<16x128xf32>
    // CHECK: } else {
    // CHECK: scf.yield %[[FOR_RES]] : tensor<16x128xf32>
    // CHECK: }
    %21 = tensor.empty() : tensor<16x128xf16>
    %22 = hivm.hir.vcast ins(%20 : tensor<16x128xf32>) outs(%21 : tensor<16x128xf16>) round_mode = <rint> -> tensor<16x128xf16>
    %23 = arith.muli %13, %c128 : index
