...
module attributes {hacc.target = #hacc.target<"Ascend950PR_9579">} {
  ...
  func.func @test_vf_mm(%a : tensor<16x16xf16>,
    ...
    // CHECK: %[[EMPTY_T:.*]] = bufferization.to_tensor %[[ALLOC]] restrict writable :
    // CHECK: hivm.hir.copy ins(%[[EXPAND0]] : tensor<1x1x16x16xf16>) outs(%[[ALLOC]] : memref{{.*}})
    %mm = hivm.hir.mmadL1
...
module attributes {hacc.target = #hacc.target<"Ascend950PR_9579">} {
  ...
  // CHECK-SAME: %[[ARG0:.*]]: tensor<16x16xf16>, %[[ARG1:.*]]: tensor<16x16xf16>) -> tensor<16x16xf32> {
  ...
  // CHECK:   %[[EXPAND0:.*]] = tensor.expand_shape {{.*}} {{\[\[0\], \[1, 2\], \[3]\]}} output_shape {{\[1, 1, 16, 16\]}} : tensor<1x16x16xf16> into tensor<1x1x16x16xf16>
  // CHECK:   %[[ALLOC:.*]] = memref.alloc() : memref<1x1x16x16xf16, #hivm.address_space<cbuf>>
  // CHECK:   %[[BUF_TENSOR:.*]] = bufferization.to_tensor %[[ALLOC]] restrict writable : memref{{.*}}
  // CHECK:   hivm.hir.copy ins(%[[EXPAND0]] : tensor<1x1x16x16xf16>) outs(%[[ALLOC]] : memref{{.*}})
  // CHECK:   %[[MM:.*]] = hivm.hir.mmadL1 ins(%[[ARG0]], %[[BUF_TENSOR:.*]], %[[TRUE]], %[[C16]], %[[C16]], %[[C16]] : tensor<16x16xf16>, tensor<1x1x16x16xf16>, i1, index, index, index) outs(%[[EMPTY1]] : tensor<16x16xf32>) -> tensor<16x16xf32>
  // CHECK:   return %[[MM]] : tensor<16x16xf32>
}
...
module attributes {hacc.target = #hacc.target<"Ascend950PR_9579">} {
               ...
               -> tensor<16x16xf32>
  {
    ...
    %mm_dst = tensor.empty() : tensor<16x16xf32>
    %loop_dst = tensor.empty() : tensor<16x16xf16>
    %c16 = arith.constant 16 : index
    %cst_4 = arith.constant 0.000000e+00 : f16
    %0 = tensor.empty() : tensor<16x16xf16>
    %1 = scf.for %arg46 = %c0 to %c16 step %c1 iter_args(%arg47 = %loop_dst) -> (tensor<16x16xf16>) {
  ...
  // CHECK:   %[[MM_DST:.*]] = tensor.empty() : tensor<16x16xf32>
  // CHECK:   %[[LOOP_DST:.*]] = tensor.empty() : tensor<16x16xf16>
  // CHECK:   %[[FOR_RES:.*]] = scf.for %{{.*}} = %[[C0]] to %[[C16]] step %[[C1]] iter_args(%{{.*}} = %[[LOOP_DST]]) -> (tensor<16x16xf16>) {
  ...
  // CHECK:   %[[BUF_TENSOR:.*]] = bufferization.to_tensor %[[ALLOC]] restrict writable : memref{{.*}}
  // CHECK:   hivm.hir.copy ins(%[[EXPAND0]] : tensor<1x1x16x16xf16>) outs(%[[ALLOC]] : memref{{.*}})
  // CHECK:   %[[MM:.*]] = hivm.hir.mmadL1 ins(%[[ARG0]], %[[BUF_TENSOR:.*]], %[[TRUE]], %[[C16]], %[[C16]], %[[C16]] : tensor<16x16xf16>, tensor<1x1x16x16xf16>, i1, index, index, index) outs(%[[MM_DST]] : tensor<16x16xf32>) -> tensor<16x16xf32>
  // CHECK:   return %[[MM]] : tensor<16x16xf32>
}
...
module attributes {hacc.target = #hacc.target<"Ascend950PR_9579">} {
  ...
  // CHECK: %[[T0:.*]] = bufferization.to_tensor %[[ALLOC0]] restrict writable : memref{{.*}}
  // CHECK: hivm.hir.copy ins(%[[EXPAND0]] : tensor<2x1x16x8xf32>) outs(%[[ALLOC0]] : memref{{.*}})
...
module attributes {hacc.target = #hacc.target<"Ascend950PR_9579">} {
  ...
  // CHECK: %[[TT:.*]] = bufferization.to_tensor %[[ALLOC_UB]] restrict writable : memref{{.*}}
...
module attributes {hacc.target = #hacc.target<"Ascend950PR_9579">} {
  ...
  func.func @test_not_affect_affinity() -> tensor<16x16xf32> {
    ...
    %9 = bufferization.to_tensor %alloc_4 restrict writable : memref<16x16xf32>
    // CHECK: {{.*}} = hivm.hir.mmadL1 {a_transpose} ins({{.*}}, %[[COPYED_TENSOR_L1]]
    %10 = hivm.hir.mmadL1 {a_transpose} ins(%9, %8, %true, %c16, %c16, %c16 : tensor<16x16xf32>, tensor<16x16xf32>, i1, index, index, index) outs(%5 : tensor<16x16xf32>) -> tensor<16x16xf32>
...
module attributes {hacc.target = #hacc.target<"Ascend950PR_9579">} {
  ...
  func.func @test_fixpipe_to_mmadl1_tight_coupled(%arg0: tensor<16x16xf32>, %arg1: tensor<16x16xf32>, %arg2: tensor<16x16xf32>, %arg3: tensor<16x16xf32>) -> tensor<16x16xf32> {
    ...
    // CHECK-NEXT: %[[UB_TENSOR:.*]] = bufferization.to_tensor %[[UB_ALLOC]] restrict writable : memref{{.*}}
    ...
    // CHECK-NEXT: %[[CBUF_TENSOR:.*]] = bufferization.to_tensor %[[CBUF_ALLOC]] restrict writable : memref{{.*}}
    // CHECK-NEXT: hivm.hir.copy ins(%[[EXPAND2]] : tensor<2x1x16x8xf32>) outs(%[[CBUF_ALLOC]] : memref{{.*}})
...
module attributes {hacc.target = #hacc.target<"Ascend950PR_9579">} {
  ...
  // CHECK: %[[BUF_TENSOR:.*]] = bufferization.to_tensor %[[ALLOC]] restrict writable : memref{{.*}}
  // CHECK: hivm.hir.copy ins(%[[EXPAND0]] : tensor<4x1x16x16xf16>) outs(%[[ALLOC]] : memref{{.*}})
...
module attributes {hacc.target = #hacc.target<"Ascend950PR_9579">} {
  ...
  func.func @test_collapse_shape_with_annotation(%arg1 : memref<4x4x16xf16>,
    ...
    // CHECK: %[[ALLOC:.*]] = memref.alloc() : memref<1x1x16x16xf16, #hivm.address_space<cbuf>>
    // CHECK: %[[BUF_TENSOR:.*]] = bufferization.to_tensor %[[ALLOC]] restrict writable : memref{{.*}}
    // CHECK: hivm.hir.copy ins(%[[EXPANDED_0]] : tensor<1x1x16x16xf16>) outs(%[[ALLOC]] : memref{{.*}})
...
module attributes {hacc.target = #hacc.target<"Ascend950PR_9579">} {
  ...
  func.func @triton_dot_2(%arg0: memref<?xi8> {hacc.arg_type = #hacc.arg_type<sync_block_lock>}, %arg1: memref<?xi8> {hacc.arg_type = #hacc.arg_type<workspace>}, %arg2: memref<?xi32> {tt.divisibility = 16 : i32, tt.tensor_kind = 1 : i32}, %arg3: memref<?xi8> {tt.divisibility = 16 : i32, tt.tensor_kind = 0 : i32}, %arg4: memref<?xi8> {tt.divisibility = 16 : i32, tt.tensor_kind = 0 : i32}, %arg5: i32, %arg6: i32, %arg7: i32) attributes {SyncBlockLockArgIdx = 0 : i64, WorkspaceArgIdx = 1 : i64, func_dyn_memref_args = dense<[true, true, true, true, true, false, false, false]> : vector<8xi1>, hacc.entry, hacc.function_kind = #hacc.function_kind<DEVICE>, mix_mode = "mix", parallel_mode = "simd"} {
    ...
    // CHECK: %[[TO_TENSOR:.*]] = bufferization.to_tensor %[[ALLOC]] restrict writable : memref{{.*}}
    // CHECK: hivm.hir.copy ins(%[[EXPANDED_2]] : tensor<1x1x16x32xi8>) outs(%[[ALLOC]] : memref{{.*}})
...
module {
  ...
  func.func @test_vf_mm_with_bias(%a : tensor<16x16xf16>,
    ...
    // CHECK: %[[BUF_TENSOR:.*]] = bufferization.to_tensor %[[ALLOC]] restrict writable : memref{{.*}}
    // CHECK: hivm.hir.copy ins(%{{.*}} : tensor<16x16xf16>) outs(%[[ALLOC]] : memref{{.*}})
...
module attributes {hacc.target = #hacc.target<"Ascend950PR_9579">} {
  ...
  // CHECK: %[[ALLOC_UB:.*]] = memref.alloc() : memref<16x16xf16, #hivm.address_space<ub>>
  // CHECK: hivm.hir.fixpipe {{.*}} ins(%{{.*}} : tensor<16x16xf32>) outs(%[[ALLOC_UB]] : memref<16x16xf16, #hivm.address_space<ub>>)
  // CHECK: %[[FIX_TENSOR:.*]] = bufferization.to_tensor %[[ALLOC_UB]] restrict writable : memref{{.*}}
  ...
  // CHECK: %[[BUF_TENSOR:.*]] = bufferization.to_tensor %[[ALLOC_L1]] restrict writable : memref{{.*}}
  // CHECK: hivm.hir.copy ins(%[[EXPAND0]] : tensor<1x1x16x16xf16>) outs(%[[ALLOC_L1]] : memref{{.*}})
...
// CHECK: %[[BUF_TENSOR:.*]] = bufferization.to_tensor %[[ALLOC]] restrict writable
// CHECK: hivm.hir.copy ins(%[[VCAST]] : tensor<2x2x16x16xf16>) outs(%[[ALLOC]] : memref{{.*}})
...
module attributes {hacc.target = #hacc.target<"Ascend950PR_9579">} {
  ...
  // CHECK: %[[ALLOC:.*]] = memref.alloc() : memref<1x1x16x16xf16, #hivm.address_space<cbuf>>
  // CHECK: %[[L1_TENSOR:.*]] = bufferization.to_tensor %[[ALLOC]] restrict writable
