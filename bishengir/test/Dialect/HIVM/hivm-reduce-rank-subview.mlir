...
// CHECK: hivm.hir.vreduce <sum> ins(%[[SUBVIEW_SRC]] : memref<10x10xf32, strided<[10, 1]>>) outs(%[[SUBVIEW_DST]] : memref<1x1xf32, strided<[1, 1]>>) unsigned_src = false reduce_dims = [0, 1]
...
// CHECK: hivm.hir.vreduce <max_with_index> ins(%[[SUBVIEW_SRC]] : memref<10x10xf32, strided<[10, 1]>>) outs(%[[SUBVIEW_DST1]], %[[SUBVIEW_DST2]] : memref<1x1xf32, strided<[1, 1]>>, memref<1x1xi32, strided<[1, 1]>>) unsigned_src = false tie_break_left = true reduce_dims = [0, 1]
func.func @multiple_init_vreduce(%src: memref<1x10x1x10xf32>, %dst: memref<1x1x1x1xf32>, %dst2 : memref<1x1x1x1xi32>) {
  hivm.hir.vreduce <max_with_index> ins(%src: memref<1x10x1x10xf32>)
