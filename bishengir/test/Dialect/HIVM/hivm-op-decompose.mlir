// RUN: bishengir-opt %s -hivm-decompose-op -split-input-file -verify-diagnostics 2>&1 | tee full_outputs_mine.log | FileCheck %s
...
// -----
func.func @test_vcast_op_memref_f32_to_i8_dyn(%d : index) {
  ...
  // CHECK: hivm.hir.vcast ins(%[[ALLOC]] : memref<2x?xf32>) outs(%[[SUBVIEW]] : memref<2x?xf16, strided<[?, 1]>>) round_mode = <round>
  // CHECK: hivm.hir.vcast ins(%[[SUBVIEW]] : memref<2x?xf16, strided<[?, 1]>>) outs(%[[ALLOC0]] : memref<2x?xi8>) round_mode = <round>
...
// -----
func.func @test_vcast_op_memref_i4_to_i8_dyn(%d : index) {
  ...
  // CHECK: %[[ALLOC1:.*]] = memref.alloc(%[[ARG0]]) : memref<2x?xf16>
  // CHECK: %[[SUBVIEW:.*]] = memref.subview %[[ALLOC1]][0, 0] [2, %arg0] [1, 1] : memref<2x?xf16> to memref<2x?xf16, strided<[?, 1]>>
  // CHECK: hivm.hir.vcast ins(%[[ALLOC]] : memref<2x?xi4>) outs(%[[SUBVIEW:.*]] : memref<2x?xf16, strided<[?, 1]>>)
  // CHECK: hivm.hir.vcast ins(%[[SUBVIEW:.*]] : memref<2x?xf16, strided<[?, 1]>>) outs(%[[ALLOC0]] : memref<2x?xi8>)
...
func.func @test_vcast_op_tensor_f32_to_i8_dyn(%d : index) -> tensor<2x?xi8> {
...
func.func @test_vcast_op_tensor_i4_to_i8_dyn(%d : index) -> tensor<2x?xi8> {
  // CHECK: %[[C1:.*]] = arith.constant 1 : index
...
func.func @test_vcast_op_tensor_i4_to_i8_not_empty_dyn(%d : index) -> tensor<2x?xi8> {
  // CHECK: %[[C1:.*]] = arith.constant 1 : index
  // CHECK: %[[T0:.*]] = tensor.empty(%arg0) : tensor<2x?xi4>
...
func.func @test_broadcast_op_multi_memref_01_dyn(%d : index) {
  // CHECK: %[[ALLOC:.*]] = memref.alloc(%[[ARG0]]) : memref<1x1x?xi16>
  // CHECK: %[[ALLOC0:.*]] = memref.alloc(%[[ARG0]]) : memref<16x8x?xi16>
  // CHECK: %[[ALLOC1:.*]] = memref.alloc(%[[ARG0]]) : memref<16x8x?xi16>
  // CHECK: %[[SUBVIEW:.*]] = memref.subview %[[ALLOC1]][0, 0, 0] [1, 8, %[[ARG0]]] [1, 1, 1] : memref<16x8x?xi16> to memref<1x8x?xi16, strided<[?, ?, 1]>>
  // CHECK: hivm.hir.vbrc ins(%[[ALLOC]] : memref<1x1x?xi16>) outs(%[[SUBVIEW]] : memref<1x8x?xi16, strided<[?, ?, 1]>>) broadcast_dims = [1]
  // CHECK: hivm.hir.vbrc ins(%[[SUBVIEW]] : memref<1x8x?xi16, strided<[?, ?, 1]>>) outs(%[[ALLOC0]] : memref<16x8x?xi16>) broadcast_dims = [0]
...
func.func @test_broadcast_op_multi_memref_01_dyn2(%d : index) {
  // CHECK: %[[ALLOC:.*]] = memref.alloc() : memref<1x1x32xi16>
  // CHECK: %[[ALLOC0:.*]] = memref.alloc(%[[ARG0]]) : memref<16x?x32xi16>
  // CHECK: %[[ALLOC1:.*]] = memref.alloc(%[[ARG0]]) : memref<16x?x32xi16>
  // CHECK: %[[SUBVIEW:.*]] = memref.subview %[[ALLOC1]][0, 0, 0] [1, %[[ARG0]], 32] [1, 1, 1] : memref<16x?x32xi16> to memref<1x?x32xi16, strided<[?, 32, 1]>>
  // CHECK: hivm.hir.vbrc ins(%[[ALLOC]] : memref<1x1x32xi16>) outs(%[[SUBVIEW]] : memref<1x?x32xi16, strided<[?, 32, 1]>>) broadcast_dims = [1]
  // CHECK: hivm.hir.vbrc ins(%[[SUBVIEW]] : memref<1x?x32xi16, strided<[?, 32, 1]>>) outs(%[[ALLOC0]] : memref<16x?x32xi16>) broadcast_dims = [0]
...
func.func @test_broadcast_op_multi_tensor_01_dyn(%d : index) -> tensor<16x8x?xi16> {
...
func.func @test_broadcast_op_multi_tensor_01_dyn2(%d : index) -> tensor<16x?x32xi16> {
  // CHECK: %[[c1:.*]] = arith.constant 1 : index
  // CHECK: %[[T0:.*]] = tensor.empty() : tensor<1x1x32xi16>
...
func.func @test_reduce_any() -> tensor<1x1xi1> {
  ...
  // CHECK: %[[ARG5:.*]] = hivm.hir.vreduce <max> ins(%[[ARG3]] : tensor<1x1024xf16>) outs(%[[ARG4]] : tensor<1x1xf16>) unsigned_src = false reduce_dims = [1] -> tensor<1x1xf16>
  ...
  %0 = hivm.hir.vreduce <any> ins(%arg0 : tensor<1x1024xi1>) outs(%arg1 : tensor<1x1xi1>) unsigned_src = false reduce_dims = [1] -> tensor<1x1xi1>
...
func.func @test_reduce_any_memref() {
    ...
    // CHECK: hivm.hir.vreduce <max> ins(%[[alloc_1]] : memref<1x1024xf16>) outs(%[[alloc_2]] : memref<1x1xf16>) unsigned_src = false reduce_dims = [1]
  ...
  hivm.hir.vreduce <any> ins(%arg0 : memref<1x1024xi1>) outs(%arg1 : memref<1x1xi1>) unsigned_src = false reduce_dims = [1]
  return
}
...
func.func @test_reduce_all() -> tensor<1x1xi1> {
  ...
  // CHECK: %[[ARG3:.*]] = hivm.hir.vcast ins(%[[ARG0]] : tensor<1x1024xi1>) outs(%[[ARG2]] : tensor<1x1024xf16>) -> tensor<1x1024xf16>
  // CHECK: %[[ARG4:.*]] = tensor.empty() : tensor<1x1xf16>
  // CHECK: %[[ARG5:.*]] = hivm.hir.vreduce <min> ins(%[[ARG3]] : tensor<1x1024xf16>) outs(%[[ARG4]] : tensor<1x1xf16>) unsigned_src = false reduce_dims = [1] -> tensor<1x1xf16>
  // CHECK: %[[ARG6:.*]] = tensor.empty() : tensor<1x1xi8>
  // CHECK: %[[ARG7:.*]] = hivm.hir.vcast ins(%[[ARG5]] : tensor<1x1xf16>) outs(%[[ARG6]] : tensor<1x1xi8>) -> tensor<1x1xi8>
  // CHECK: %[[ARG8:.*]] = tensor.empty() : tensor<1x1xf16>
  // CHECK: %[[ARG9:.*]] = hivm.hir.vcast ins(%[[ARG7]] : tensor<1x1xi8>) outs(%[[ARG8]] : tensor<1x1xf16>) -> tensor<1x1xf16>
  ...
  %arg0 = tensor.empty() : tensor<1x1024xi1>
  %arg1 = tensor.empty() : tensor<1x1xi1>
  %0 = hivm.hir.vreduce <all> ins(%arg0 : tensor<1x1024xi1>) outs(%arg1 : tensor<1x1xi1>) unsigned_src = false reduce_dims = [1] -> tensor<1x1xi1>
  return %0 : tensor<1x1xi1>
}
...
func.func @test_reduce_sum_ar_b64() {
  ...
  hivm.hir.vreduce <sum> ins(%src : memref<24x32xi64>) outs(%dst : memref<24x1xi64>) unsigned_src = false reduce_dims = [1]
  return
}
...
func.func @test_reduce_min_ar_b64() {
  ...
  %src = memref.alloc() : memref<24x32xi64>
  %dst = memref.alloc() : memref<24x1xi64>
  hivm.hir.vreduce <min> ins(%src : memref<24x32xi64>) outs(%dst : memref<24x1xi64>) unsigned_src = false reduce_dims = [1]
  return
}
...
func.func @test_reduce_max_ar_b64() {
  ...
  %src = memref.alloc() : memref<24x32xi64>
  %dst = memref.alloc() : memref<24x1xi64>
  hivm.hir.vreduce <max> ins(%src : memref<24x32xi64>) outs(%dst : memref<24x1xi64>) unsigned_src = false reduce_dims = [1]
  return
}
...
func.func @test_reduce_min_with_index_int64() {
  ...
  hivm.hir.vreduce <min_with_index> ins(%src : memref<2x2xi64>) outs(%dst1, %dst2 : memref<1x2xi64>, memref<1x2xi32>) unsigned_src = false tie_break_left = true reduce_dims = [0]
  return
}
...
func.func @test_reduce_min_with_index_int32() {
  ...
  %dst2 = memref.alloc() : memref<1x2xi32>
  hivm.hir.vreduce <min_with_index> ins(%src : memref<2x2xi32>) outs(%dst1, %dst2 : memref<1x2xi32>, memref<1x2xi32>) unsigned_src = false tie_break_left = true reduce_dims = [0]
  return
}
...
func.func @test_reduce_min_with_index_int16() {
  ...
  %dst2 = memref.alloc() : memref<1x2xi32>
  hivm.hir.vreduce <min_with_index> ins(%src : memref<2x2xi16>) outs(%dst1, %dst2 : memref<1x2xi16>, memref<1x2xi32>) unsigned_src = false tie_break_left = true reduce_dims = [0]
  return
}
...
func.func @test_reduce_max_with_index_int64() {
  ...
  %dst1 = memref.alloc() : memref<1x2xi64>
  %dst2 = memref.alloc() : memref<1x2xi32>
  hivm.hir.vreduce <max_with_index> ins(%src : memref<2x2xi64>) outs(%dst1, %dst2 : memref<1x2xi64>, memref<1x2xi32>) unsigned_src = false tie_break_left = true reduce_dims = [0]
  return
}
...
func.func @test_reduce_max_with_index_int32() {
  ...
  %dst1 = memref.alloc() : memref<1x2xi32>
  %dst2 = memref.alloc() : memref<1x2xi32>
  hivm.hir.vreduce <max_with_index> ins(%src : memref<2x2xi32>) outs(%dst1, %dst2 : memref<1x2xi32>, memref<1x2xi32>) unsigned_src = false tie_break_left = true reduce_dims = [0]
  return
}
...
func.func @test_reduce_max_with_index_int16() {
  ...
  %dst1 = memref.alloc() : memref<1x2xi16>
  %dst2 = memref.alloc() : memref<1x2xi32>
  hivm.hir.vreduce <max_with_index> ins(%src : memref<2x2xi16>) outs(%dst1, %dst2 : memref<1x2xi16>, memref<1x2xi32>) unsigned_src = false tie_break_left = true reduce_dims = [0]
  return
}
...
func.func @test_reduce_sum_ra_b64() {
  ...
  hivm.hir.vreduce <sum> ins(%src : memref<24x32xi64>) outs(%dst : memref<1x32xi64>) unsigned_src = false reduce_dims = [0]
  return
}
...
func.func @test_reduce_min_ra_b64() {
  ...
  %src = memref.alloc() : memref<24x32xi64>
  %dst = memref.alloc() : memref<1x32xi64>
  hivm.hir.vreduce <min> ins(%src : memref<24x32xi64>) outs(%dst : memref<1x32xi64>) unsigned_src = false reduce_dims = [0]
  return
}
...
func.func @test_reduce_max_ra_b64() {
  ...
  %src = memref.alloc() : memref<24x32xi64>
  %dst = memref.alloc() : memref<1x32xi64>
  hivm.hir.vreduce <max> ins(%src : memref<24x32xi64>) outs(%dst : memref<1x32xi64>) unsigned_src = false reduce_dims = [0]
  return
}
...
func.func @test_reduce_sum_r_b64() {
  ...
  hivm.hir.vreduce <sum> ins(%src : memref<32xi64>) outs(%dst : memref<1xi64>) unsigned_src = false reduce_dims = [0]
  return
}
...
func.func @test_reduce_min_r_b64() {
  ...
  %src = memref.alloc() : memref<32xi64>
  %dst = memref.alloc() : memref<1xi64>
  hivm.hir.vreduce <min> ins(%src : memref<32xi64>) outs(%dst : memref<1xi64>) unsigned_src = false reduce_dims = [0]
  return
}
...
func.func @test_reduce_max_r_b64() {
  ...
  %src = memref.alloc() : memref<32xi64>
  %dst = memref.alloc() : memref<1xi64>
  hivm.hir.vreduce <max> ins(%src : memref<32xi64>) outs(%dst : memref<1xi64>) unsigned_src = false reduce_dims = [0]
  return
}

// -----
// CHECK-LABEL: func.func @test_absi_decompose(
// CHECK-SAME: %[[SRC0:.*]]: memref<8x8xi32>, %[[DST0:.*]]: memref<8x8xi32>,
func.func @test_absi_decompose(%src0 : memref<8x8xi32>, %dst0 : memref<8x8xi32>,
                               %src1_size : index, %dst1_size : index) {
  %src1 = memref.alloc(%src1_size, %src1_size) : memref<?x?xi16>
  %dst1 = memref.alloc(%dst1_size, %dst1_size) : memref<?x?xi16>
  // CHECK-DAG: %[[CST_I16:.*]] = arith.constant 1 : i16
  // CHECK-DAG: %[[CST_I32:.*]] = arith.constant 1 : i32

  // CHECK: %[[SRC1:.*]] = memref.alloc(%[[ARG2:.*]], %[[ARG2]]) : memref<?x?xi16>
  // CHECK: %[[DST1:.*]] = memref.alloc(%[[ARG3:.*]], %[[ARG3]]) : memref<?x?xi16>
  ...
  // CHECK: %[[SUBVIEW:.*]] = memref.subview %[[ALLOC2]]{{.*}} : memref<?x?xi16> to memref<?x?xi16, strided<[?, 1]>>
  // CHECK: hivm.hir.vnot ins(%[[SRC1]] : memref<?x?xi16>) outs(%[[SUBVIEW]] : memref<?x?xi16, strided<[?, 1]>>)
  // CHECK: %[[ALLOC3:.*]] = memref.alloc{{.*}} : memref<?x?xi16>
  // CHECK: %[[SUBVIEW0:.*]] = memref.subview %[[ALLOC3]]{{.*}} : memref<?x?xi16> to memref<?x?xi16, strided<[?, 1]>>
  // CHECK: hivm.hir.vadd ins(%[[SUBVIEW]], %[[CST_I16]] : memref<?x?xi16, strided<[?, 1]>>, i16) outs(%[[SUBVIEW0]] : memref<?x?xi16, strided<[?, 1]>>)
  // CHECK: hivm.hir.vmax ins(%[[SRC1]], %[[SUBVIEW0]] : memref<?x?xi16>, memref<?x?xi16, strided<[?, 1]>>) outs(%[[DST1]] : memref<?x?xi16>)
...
module {
  ...
  // CHECK: %[[alloc_2:.*]] = memref.alloc(%arg0, %arg1) : memref<?x?x4096xf32>
  // CHECK: %[[SUBVIEW:.*]] = memref.subview %[[alloc_2]][0, 0, 0] [1, %arg1, 4096] [1, 1, 1] : memref<?x?x4096xf32> to memref<1x?x4096xf32, strided<[?, 4096, 1]>>
  // CHECK: hivm.hir.vbrc ins(%[[alloc_0]] : memref<1x1x4096xf32>) outs(%[[SUBVIEW]] : memref<1x?x4096xf32, strided<[?, 4096, 1]>>) broadcast_dims = [1]
  // CHECK: hivm.hir.vbrc ins(%[[SUBVIEW]] : memref<1x?x4096xf32, strided<[?, 4096, 1]>>) outs(%[[alloc_1]] : memref<?x?x4096xf32>) broadcast_dims = [0]
...
// -----
module {
  func.func @test_decompose_vcast_mark_buffer_size(%2 : index, %3 : index) {
    ...
    // CHECK: %subview = memref.subview %alloc_1[0, 0] [%arg0, %arg1] [1, 1] : memref<?x?xf16> to memref<?x?xf16, strided<[?, 1]>> 
    // CHECK: hivm.hir.vcast ins(%alloc : memref<?x?xf32>) outs(%subview : memref<?x?xf16, strided<[?, 1]>>)
    // CHECK: hivm.hir.vcast ins(%subview : memref<?x?xf16, strided<[?, 1]>>) outs(%alloc_0 : memref<?x?xi8>)
...
// -----
func.func @test_decompose_vcast_mark_buffer_size_i8_to_i1(%2 : index, %3 : index) {
  // CHECK: %cst = arith.constant 0.000000e+00 : f16
  // CHECK: %alloc = memref.alloc(%arg0, %arg1) : memref<?x?xi8>
  // CHECK: %alloc_0 = memref.alloc(%arg0, %arg1) : memref<?x?xi1>
  // CHECK: %alloc_1 = memref.alloc(%arg0, %arg1) : memref<?x?xf16>
  // CHECK: %subview = memref.subview %alloc_1[0, 0] [%arg0, %arg1] [1, 1] : memref<?x?xf16> to memref<?x?xf16, strided<[?, 1]>> 
  // CHECK: hivm.hir.vcast ins(%alloc : memref<?x?xi8>) outs(%subview : memref<?x?xf16, strided<[?, 1]>>)
  // CHECK: %alloc_2 = memref.alloc(%arg0, %arg1) : memref<?x?xf16>
  // CHECK: %subview_3 = memref.subview %alloc_2[0, 0] [%arg0, %arg1] [1, 1] : memref<?x?xf16> to memref<?x?xf16, strided<[?, 1]>> 
  // CHECK: hivm.hir.vbrc ins(%cst : f16) outs(%subview_3 : memref<?x?xf16, strided<[?, 1]>>)
  // CHECK: hivm.hir.vcmp ins(%subview, %subview_3 : memref<?x?xf16, strided<[?, 1]>>, memref<?x?xf16, strided<[?, 1]>>) outs(%alloc_0 : memref<?x?xi1>) compare_mode = <ne>
...
// -----
// CHECK-LABEL: func @test_decompose_vcast_mark_buffer_size_bool_to_f32
func.func @test_decompose_vcast_mark_buffer_size_bool_to_f32(%2 : index, %3 : index) {
...
// CHECK:           hivm.hir.vdeinterleave ins(%[[SRC]] : memref<32xf16>) outs(%[[EVEN]] : memref<16xf16>)
...
// -----
// CHECK-LABEL: func @test_setatomic_ADD_f32
...
func.func @test_setatomic_ADD_f32() {
  hivm.hir.set_atomic kind = <add>[type = f32]
  return
}

// -----
// CHECK-LABEL: func @test_setatomic_ADD_f16
// CHECK: hivm.hir.set_ctrl false at ctrl[6]
// CHECK: hivm.hir.set_ctrl true at ctrl[7]
// CHECK: hivm.hir.set_ctrl false at ctrl[8]
// CHECK: hivm.hir.set_ctrl false at ctrl[9]
// CHECK: hivm.hir.set_ctrl false at ctrl[10]
func.func @test_setatomic_ADD_f16() {
  hivm.hir.set_atomic kind = <add>[type = f16]
  return
}

// -----
// CHECK-LABEL: func @test_setatomic_ADD_s8
// CHECK: hivm.hir.set_ctrl true at ctrl[6]
// CHECK: hivm.hir.set_ctrl false at ctrl[7]
// CHECK: hivm.hir.set_ctrl true at ctrl[8]
// CHECK: hivm.hir.set_ctrl false at ctrl[9]
// CHECK: hivm.hir.set_ctrl false at ctrl[10]
func.func @test_setatomic_ADD_s8() {
  hivm.hir.set_atomic kind = <add>[type = i8]
  return
}

// -----
// CHECK-LABEL: func @test_setatomic_ADD_s16
// CHECK: hivm.hir.set_ctrl true at ctrl[6]
// CHECK: hivm.hir.set_ctrl true at ctrl[7]
// CHECK: hivm.hir.set_ctrl false at ctrl[8]
// CHECK: hivm.hir.set_ctrl false at ctrl[9]
// CHECK: hivm.hir.set_ctrl false at ctrl[10]
func.func @test_setatomic_ADD_s16() {
  hivm.hir.set_atomic kind = <add>[type = i16]
  return
}

// -----
// CHECK-LABEL: func @test_setatomic_ADD_s32
// CHECK: hivm.hir.set_ctrl false at ctrl[6]
// CHECK: hivm.hir.set_ctrl false at ctrl[7]
// CHECK: hivm.hir.set_ctrl true at ctrl[8]
// CHECK: hivm.hir.set_ctrl false at ctrl[9]
// CHECK: hivm.hir.set_ctrl false at ctrl[10]
func.func @test_setatomic_ADD_s32() {
  hivm.hir.set_atomic kind = <add>[type = i32]
  return
}

// -----
// CHECK-LABEL: func @test_setatomic_ADD_bf16
// CHECK: hivm.hir.set_ctrl false at ctrl[6]
// CHECK: hivm.hir.set_ctrl true at ctrl[7]
// CHECK: hivm.hir.set_ctrl true at ctrl[8]
// CHECK: hivm.hir.set_ctrl false at ctrl[9]
// CHECK: hivm.hir.set_ctrl false at ctrl[10]
func.func @test_setatomic_ADD_bf16() {
  hivm.hir.set_atomic kind = <add>[type = bf16]
  return
}

// -----
// CHECK-LABEL: func @test_setatomic_MAX_f32
// CHECK: hivm.hir.set_ctrl true at ctrl[6]
// CHECK: hivm.hir.set_ctrl false at ctrl[7]
// CHECK: hivm.hir.set_ctrl false at ctrl[8]
// CHECK: hivm.hir.set_ctrl true at ctrl[9]
// CHECK: hivm.hir.set_ctrl false at ctrl[10]
func.func @test_setatomic_MAX_f32() {
  hivm.hir.set_atomic kind = <max>[type = f32]
  return
}

// -----
// CHECK-LABEL: func @test_setatomic_MAX_f16
// CHECK: hivm.hir.set_ctrl false at ctrl[6]
// CHECK: hivm.hir.set_ctrl true at ctrl[7]
// CHECK: hivm.hir.set_ctrl false at ctrl[8]
// CHECK: hivm.hir.set_ctrl true at ctrl[9]
// CHECK: hivm.hir.set_ctrl false at ctrl[10]
func.func @test_setatomic_MAX_f16() {
  hivm.hir.set_atomic kind = <max>[type = f16]
  return
}

// -----
// CHECK-LABEL: func @test_setatomic_MAX_s8
// CHECK: hivm.hir.set_ctrl true at ctrl[6]
// CHECK: hivm.hir.set_ctrl false at ctrl[7]
// CHECK: hivm.hir.set_ctrl true at ctrl[8]
// CHECK: hivm.hir.set_ctrl true at ctrl[9]
// CHECK: hivm.hir.set_ctrl false at ctrl[10]
func.func @test_setatomic_MAX_s8() {
  hivm.hir.set_atomic kind = <max>[type = i8]
  return
}

// -----
// CHECK-LABEL: func @test_setatomic_MAX_s16
// CHECK: hivm.hir.set_ctrl true at ctrl[6]
// CHECK: hivm.hir.set_ctrl true at ctrl[7]
// CHECK: hivm.hir.set_ctrl false at ctrl[8]
// CHECK: hivm.hir.set_ctrl true at ctrl[9]
// CHECK: hivm.hir.set_ctrl false at ctrl[10]
func.func @test_setatomic_MAX_s16() {
  hivm.hir.set_atomic kind = <max>[type = i16]
  return
}

// -----
// CHECK-LABEL: func @test_setatomic_MAX_s32
// CHECK: hivm.hir.set_ctrl false at ctrl[6]
// CHECK: hivm.hir.set_ctrl false at ctrl[7]
// CHECK: hivm.hir.set_ctrl true at ctrl[8]
// CHECK: hivm.hir.set_ctrl true at ctrl[9]
// CHECK: hivm.hir.set_ctrl false at ctrl[10]
func.func @test_setatomic_MAX_s32() {
  hivm.hir.set_atomic kind = <max>[type = i32]
  return
}

// -----
// CHECK-LABEL: func @test_setatomic_MAX_bf16
// CHECK: hivm.hir.set_ctrl false at ctrl[6]
// CHECK: hivm.hir.set_ctrl true at ctrl[7]
// CHECK: hivm.hir.set_ctrl true at ctrl[8]
// CHECK: hivm.hir.set_ctrl true at ctrl[9]
// CHECK: hivm.hir.set_ctrl false at ctrl[10]
func.func @test_setatomic_MAX_bf16() {
  hivm.hir.set_atomic kind = <max>[type = bf16]
  return
}

// -----
// CHECK-LABEL: func @test_setatomic_MIN_f32
// CHECK: hivm.hir.set_ctrl true at ctrl[6]
// CHECK: hivm.hir.set_ctrl false at ctrl[7]
// CHECK: hivm.hir.set_ctrl false at ctrl[8]
// CHECK: hivm.hir.set_ctrl false at ctrl[9]
// CHECK: hivm.hir.set_ctrl true at ctrl[10]
func.func @test_setatomic_MIN_f32() {
  hivm.hir.set_atomic kind = <min>[type = f32]
  return
}

// -----
// CHECK-LABEL: func @test_setatomic_MIN_f16
// CHECK: hivm.hir.set_ctrl false at ctrl[6]
// CHECK: hivm.hir.set_ctrl true at ctrl[7]
// CHECK: hivm.hir.set_ctrl false at ctrl[8]
// CHECK: hivm.hir.set_ctrl false at ctrl[9]
// CHECK: hivm.hir.set_ctrl true at ctrl[10]
func.func @test_setatomic_MIN_f16() {
  hivm.hir.set_atomic kind = <min>[type = f16]
  return
}

// -----
// CHECK-LABEL: func @test_setatomic_MIN_s8
// CHECK: hivm.hir.set_ctrl true at ctrl[6]
// CHECK: hivm.hir.set_ctrl false at ctrl[7]
// CHECK: hivm.hir.set_ctrl true at ctrl[8]
// CHECK: hivm.hir.set_ctrl false at ctrl[9]
// CHECK: hivm.hir.set_ctrl true at ctrl[10]
func.func @test_setatomic_MIN_s8() {
  hivm.hir.set_atomic kind = <min>[type = i8]
  return
}

// -----
// CHECK-LABEL: func @test_setatomic_MIN_s16
// CHECK: hivm.hir.set_ctrl true at ctrl[6]
// CHECK: hivm.hir.set_ctrl true at ctrl[7]
// CHECK: hivm.hir.set_ctrl false at ctrl[8]
// CHECK: hivm.hir.set_ctrl false at ctrl[9]
// CHECK: hivm.hir.set_ctrl true at ctrl[10]
func.func @test_setatomic_MIN_s16() {
  hivm.hir.set_atomic kind = <min>[type = i16]
  return
}

// -----
// CHECK-LABEL: func @test_setatomic_MIN_s32
// CHECK: hivm.hir.set_ctrl false at ctrl[6]
// CHECK: hivm.hir.set_ctrl false at ctrl[7]
// CHECK: hivm.hir.set_ctrl true at ctrl[8]
// CHECK: hivm.hir.set_ctrl false at ctrl[9]
// CHECK: hivm.hir.set_ctrl true at ctrl[10]
func.func @test_setatomic_MIN_s32() {
  hivm.hir.set_atomic kind = <min>[type = i32]
  return
}

// -----
// CHECK-LABEL: func @test_setatomic_MIN_bf16
// CHECK: hivm.hir.set_ctrl false at ctrl[6]
// CHECK: hivm.hir.set_ctrl true at ctrl[7]
// CHECK: hivm.hir.set_ctrl true at ctrl[8]
// CHECK: hivm.hir.set_ctrl false at ctrl[9]
// CHECK: hivm.hir.set_ctrl true at ctrl[10]
func.func @test_setatomic_MIN_bf16() {
  hivm.hir.set_atomic kind = <min>[type = bf16]
  return
}

// -----
// CHECK-LABEL: func @test_setatomic_NONE_f32
// CHECK: hivm.hir.set_ctrl false at ctrl[6]
// CHECK: hivm.hir.set_ctrl false at ctrl[7]
// CHECK: hivm.hir.set_ctrl false at ctrl[8]
func.func @test_setatomic_NONE_f32() {
  hivm.hir.set_atomic kind = <none>[type = f32]
  return
}

// -----
// CHECK-LABEL: func @test_setatomic_NONE_f16
// CHECK: hivm.hir.set_ctrl false at ctrl[6]
// CHECK: hivm.hir.set_ctrl false at ctrl[7]
// CHECK: hivm.hir.set_ctrl false at ctrl[8]
func.func @test_setatomic_NONE_f16() {
  hivm.hir.set_atomic kind = <none>[type = f16]
  return
}

// -----
// CHECK-LABEL: func @test_setatomic_NONE_s8
// CHECK: hivm.hir.set_ctrl false at ctrl[6]
// CHECK: hivm.hir.set_ctrl false at ctrl[7]
// CHECK: hivm.hir.set_ctrl false at ctrl[8]
func.func @test_setatomic_NONE_s8() {
  hivm.hir.set_atomic kind = <none>[type = i8]
  return
}

// -----
// CHECK-LABEL: func @test_setatomic_NONE_s16
// CHECK: hivm.hir.set_ctrl false at ctrl[6]
// CHECK: hivm.hir.set_ctrl false at ctrl[7]
// CHECK: hivm.hir.set_ctrl false at ctrl[8]
func.func @test_setatomic_NONE_s16() {
  hivm.hir.set_atomic kind = <none>[type = i16]
  return
}

// -----
// CHECK-LABEL: func @test_setatomic_NONE_s32
// CHECK: hivm.hir.set_ctrl false at ctrl[6]
// CHECK: hivm.hir.set_ctrl false at ctrl[7]
// CHECK: hivm.hir.set_ctrl false at ctrl[8]
func.func @test_setatomic_NONE_s32() {
  hivm.hir.set_atomic kind = <none>[type = i32]
  return
}

// -----
// CHECK-LABEL: func @test_setatomic_NONE_bf16
// CHECK: hivm.hir.set_ctrl false at ctrl[6]
// CHECK: hivm.hir.set_ctrl false at ctrl[7]
// CHECK: hivm.hir.set_ctrl false at ctrl[8]
func.func @test_setatomic_NONE_bf16() {
  hivm.hir.set_atomic kind = <none>[type = bf16]
  return
}