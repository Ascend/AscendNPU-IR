...
func.func @brc_reduce_flatten_combine_reassociations(%src1: memref<1x1x1x2x4x1xi32>, 
  ...
  // CHECK: hivm.hir.vreduce <max> ins({{.*}} memref<8x8x8xi32>) outs({{.*}} memref<1x8x1xi32>) unsigned_src = false reduce_dims = {{\[}}0, 2]
  hivm.hir.vreduce <max> ins(%dst1 : memref<1x1x8x2x4x8xi32>) outs(%src1 : memref<1x1x1x2x4x1xi32>) unsigned_src = false reduce_dims = [2, 5]
  ...
  // CHECK: hivm.hir.vreduce <sum> ins({{.*}} memref<?x16x64xi32>) outs({{.*}} memref<1x16x1xi32>) unsigned_src = false reduce_dims = {{\[}}0, 2]
  hivm.hir.vreduce <sum> ins(%dst2 : memref<1x1x?x2x4x2x8x8x1xi32>) outs(%src2 : memref<1x1x1x2x4x2x1x1x1xi32>) unsigned_src = false reduce_dims = [2, 6, 7]
  return
}
...
func.func @strided_combine_reassociations(
  ...
  // CHECK-SAME: outs({{.*}} memref<1x2x1xi32, strided<[?, ?, 1]>>) unsigned_src = false reduce_dims = {{\[}}0, 2]
  hivm.hir.vreduce <max> ins(%dst1 : memref<1x8x2x1x8x1xi32, strided<[?, ?, ?, ?, 1, 1]>>) 
                         outs(%src1 : memref<1x1x2x1x1x1xi32, strided<[?, ?, ?, ?, 1, 1]>>) unsigned_src = false reduce_dims = [1, 4]
  ...
  // CHECK-SAME: outs({{.*}} memref<16x1x2x1xi32, strided<[?, ?, ?, 1]>>) unsigned_src = false reduce_dims = {{\[}}1, 3]
  hivm.hir.vreduce <max> ins(%dst2 : memref<16x1x8x2x1x8x1xi32, strided<[?, ?, ?, ?, ?, 1, 1]>>) 
                         outs(%src2 : memref<16x1x1x2x1x1x1xi32, strided<[?, ?, ?, ?, ?, 1, 1]>>) unsigned_src = false reduce_dims = [2, 5]
  return
}
...
func.func @non_contiguous_stride(%arg0: memref<7x17x15xf16, strided<[272, 16, 1]>, #hivm.address_space<ub>>, %arg1: memref<1x17x15xf16, strided<[272, 16, 1]>, #hivm.address_space<ub>>, %arg2: memref<1x17x15xi32, strided<[272, 16, 1]>, #hivm.address_space<ub>>) {
  hivm.hir.vreduce <max_with_index> ins(%arg0 : memref<7x17x15xf16, strided<[272, 16, 1]>, #hivm.address_space<ub>>) outs(%arg1, %arg2 : memref<1x17x15xf16, strided<[272, 16, 1]>, #hivm.address_space<ub>>, memref<1x17x15xi32, strided<[272, 16, 1]>, #hivm.address_space<ub>>) unsigned_src = false tie_break_left = true reduce_dims = [0]
  return
}
...
func.func @triton_argmax_3d(%arg0: memref<7x17x15x1xf16, strided<[272, 16, 1, 1]>, #hivm.address_space<ub>>, %arg1: memref<1x17x15x1xf16, strided<[272, 16, 1, 1]>, #hivm.address_space<ub>>, %arg2: memref<1x17x15x1xi32, strided<[272, 16, 1, 1]>, #hivm.address_space<ub>>) {
  hivm.hir.vreduce <max_with_index> ins(%arg0 : memref<7x17x15x1xf16, strided<[272, 16, 1, 1]>, #hivm.address_space<ub>>) outs(%arg1, %arg2 : memref<1x17x15x1xf16, strided<[272, 16, 1, 1]>, #hivm.address_space<ub>>, memref<1x17x15x1xi32, strided<[272, 16, 1, 1]>, #hivm.address_space<ub>>) unsigned_src = false tie_break_left = true reduce_dims = [0]
  return
}
...
func.func @triton_cumsum_2(%arg0: memref<1x1x4x1xf16, strided<[4, 4, 1, 1]>, #hivm.address_space<ub>>, %arg1: memref<1x1x4x1xf16, strided<[4, 4, 1, 1]>, #hivm.address_space<ub>>) {
  hivm.hir.vcumsum ins(%arg0 : memref<1x1x4x1xf16, strided<[4, 4, 1, 1]>, #hivm.address_space<ub>>) outs(%arg1: memref<1x1x4x1xf16, strided<[4, 4, 1, 1]>, #hivm.address_space<ub>>) cum_dims = [2] reverse = true
  return
}
...
func.func @triton_cumprod_0(%arg0: memref<3x1x4x1xf16, strided<[4, 4, 1, 1]>, #hivm.address_space<ub>>, %arg1: memref<3x1x4x1xf16, strided<[4, 4, 1, 1]>, #hivm.address_space<ub>>) {
  hivm.hir.vcumsum ins(%arg0 : memref<3x1x4x1xf16, strided<[4, 4, 1, 1]>, #hivm.address_space<ub>>) outs(%arg1: memref<3x1x4x1xf16, strided<[4, 4, 1, 1]>, #hivm.address_space<ub>>) cum_dims = [0] reverse = true
  return
}
...
// CHECK: %[[COLLAPSED_0:.*]] = memref.collapse_shape %[[ALLOC_0]] [[_:.*]] into memref<1x1x2xf16>
// CHECK: %[[COLLAPSED_1:.*]] = memref.collapse_shape %[[ALLOC_1]] [[_:.*]] into memref<1x1x2xf16>
// CHECK: hivm.hir.vflip ins(%[[COLLAPSED_0]] : memref<1x1x2xf16>) outs(%[[COLLAPSED_1]] : memref<1x1x2xf16>) flip_axis = 1
