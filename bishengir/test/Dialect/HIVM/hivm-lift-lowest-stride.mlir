...
module {
  func.func @test_lift_vreduce_ops(
    ...
    // CHECK: hivm.hir.vreduce <max_with_index> ins(%{{.*}} : memref<?x32x1xf16, strided<[?, 16, 1]>>) outs(%{{.*}}, %{{.*}} : memref<?x1x1xf16, strided<[?, 16, 1]>>, memref<?x1x1xi32, strided<[?, 16, 1]>>) temp_buffer(%{{.*}} : memref<16384xf32>) unsigned_src = false tie_break_left = true reduce_dims = [1]
...
module {
  func.func @test_lift_cumsum_op(%src: memref<2x25xi16, strided<[800, 32]>>, %dst: memref<2x25xi16, strided<[800, 32]>>) {
    // CHECK: hivm.hir.vcumsum ins(%{{.*}} : memref<2x25x1xi16, strided<[800, 32, 1]>>) outs(%{{.*}} : memref<2x25x1xi16, strided<[800, 32, 1]>>) cum_dims = [0] reverse = true
