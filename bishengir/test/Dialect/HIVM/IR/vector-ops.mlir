...
func.func @test_vector_scalar_only_trait_incorrect_type(
  %src1 : memref<?xf16>, %dst : memref<?xf16>, %cst : f16) {
  // expected-error@+1 {{failed to verify that operand at index 1 is scalar-only}}
  bishengir_test.vector_scalar_only ins(%src1, %src1 : memref<?xf16>, memref<?xf16>)
                                    outs(%dst : memref<?xf16>)
  return
}
...
func.func @test_binary_vector_op_only_vs(
  %src1 : memref<?x?xi16>, %src2 : memref<?x?xi16>, %dst : memref<?x?xi16>, %cst : i16) {
  // expected-error@+1 {{failed to verify that operand at index 1 is scalar-only}}
...
module attributes {hacc.target = #hacc.target<"Ascend950PR_9589">} {
  func.func @test_vbrc_op_fp8(%src1 : memref<1xf8E4M3FN>, %dst1 : memref<?xf8E4M3FN>,
                                %src2 : tensor<1xf8E5M2>, %dst2 : tensor<?xf8E5M2>) {
    hivm.hir.vbrc ins(%src1 : memref<1xf8E4M3FN>)
                  outs(%dst1 : memref<?xf8E4M3FN>)
                  broadcast_dims = [0]

    hivm.hir.vbrc ins(%src2 : tensor<1xf8E5M2>)
                  outs(%dst2 : tensor<?xf8E5M2>)
                  broadcast_dims = [0] -> tensor<?xf8E5M2>
...
func.func @test_vreduce_op_memref(%src1 : memref<?xi32>, %dst1 : memref<1xi32>,
  ...
  hivm.hir.vreduce <max_with_index> ins(%src1 : memref<?xi32>)
                                    outs(%dst1, %dst_index : memref<1xi32>, memref<1xi32>)
                                    unsigned_src = false
                                    tie_break_left = true
                                    reduce_dims = [0]
  hivm.hir.vreduce <min_with_index> ins(%src1 : memref<?xi32>)
                                    outs(%dst1, %dst_index : memref<1xi32>, memref<1xi32>)
                                    unsigned_src = false
                                    tie_break_left = true
                                    reduce_dims = [0]
...
func.func @test_vreduce_op_tensor(%src2 : tensor<?x?xi32>, %dst2 : tensor<?x1xi32>,
  ...
  %rst_value, %rst_index = hivm.hir.vreduce <max_with_index> 
    ins(%src2 : tensor<?x?xi32>)
...
func.func @test_cumsum_memref(%src : memref<2x?x?xf32>, %dst : memref<2x?x?xf32>) {
  ...
  %f16 = memref.alloc() : memref<2x16xf16>
  %f32 = memref.alloc() : memref<2x16xf32>
  hivm.hir.vcumsum ins(%s16 : memref<2x16xi16>) outs(%s16 : memref<2x16xi16>) cum_dims = [0] reverse = true
  hivm.hir.vcumsum ins(%s32 : memref<2x16xi32>) outs(%s32 : memref<2x16xi32>) cum_dims = [1] reverse = true
  hivm.hir.vcumsum ins(%s64 : memref<2x16xi64>) outs(%s64 : memref<2x16xi64>) cum_dims = [1] reverse = true
  hivm.hir.vcumsum ins(%f16 : memref<2x16xf16>) outs(%f16 : memref<2x16xf16>) cum_dims = [0] reverse = true
  hivm.hir.vcumsum ins(%f32 : memref<2x16xf32>) outs(%f32 : memref<2x16xf32>) cum_dims = [1] reverse = true
  hivm.hir.vcumsum ins(%src : memref<2x?x?xf32>) outs(%dst : memref<2x?x?xf32>) cum_dims = [2] reverse = true
  return
}
...
func.func @test_cumprod_memref(%src : memref<2x?x?xf32>, %dst : memref<2x?x?xf32>) {
  ...
  %f16 = memref.alloc() : memref<2x16xf16>
  %f32 = memref.alloc() : memref<2x16xf32>
  hivm.hir.vcumprod ins(%s16 : memref<2x16xi16>) outs(%s16 : memref<2x16xi16>) cum_dims = [0] reverse = true
  hivm.hir.vcumprod ins(%s32 : memref<2x16xi32>) outs(%s32 : memref<2x16xi32>) cum_dims = [1] reverse = true
  hivm.hir.vcumprod ins(%s64 : memref<2x16xi64>) outs(%s64 : memref<2x16xi64>) cum_dims = [0] reverse = true
  hivm.hir.vcumprod ins(%f16 : memref<2x16xf16>) outs(%f16 : memref<2x16xf16>) cum_dims = [0] reverse = true
  hivm.hir.vcumprod ins(%f32 : memref<2x16xf32>) outs(%f32 : memref<2x16xf32>) cum_dims = [1] reverse = true
  hivm.hir.vcumprod ins(%src : memref<2x?x?xf32>) outs(%dst : memref<2x?x?xf32>) cum_dims = [2] reverse = true
  return
}
...
func.func @test_cumsum_tensor(%src : tensor<2x?x?xf32>, %dst : tensor<2x?x?xf32>) {
  ...
  %f16 = tensor.empty() : tensor<2x16xf16>
  %f32 = tensor.empty() : tensor<2x16xf32>
  %res1 = hivm.hir.vcumsum ins(%s16 : tensor<2x16xi16>) outs(%s16 : tensor<2x16xi16>) cum_dims = [0] reverse = true -> tensor<2x16xi16>
  %res2 = hivm.hir.vcumsum ins(%s32 : tensor<2x16xi32>) outs(%s32 : tensor<2x16xi32>) cum_dims = [1] reverse = true -> tensor<2x16xi32>
  %res3 = hivm.hir.vcumsum ins(%s64 : tensor<2x16xi64>) outs(%s64 : tensor<2x16xi64>) cum_dims = [0] reverse = true -> tensor<2x16xi64>
  %res4 = hivm.hir.vcumsum ins(%f16 : tensor<2x16xf16>) outs(%f16 : tensor<2x16xf16>) cum_dims = [0] reverse = true -> tensor<2x16xf16>
  %res5 = hivm.hir.vcumsum ins(%f32 : tensor<2x16xf32>) outs(%f32 : tensor<2x16xf32>) cum_dims = [1] reverse = true -> tensor<2x16xf32>
  %res6 = hivm.hir.vcumsum ins(%src : tensor<2x?x?xf32>) outs(%dst : tensor<2x?x?xf32>) cum_dims = [2] reverse = true -> tensor<2x?x?xf32>
  return
}
...
func.func @test_cumprod_tensor(%src : tensor<2x?x?xf32>, %dst : tensor<2x?x?xf32>) {
  ...
  %f16 = tensor.empty() : tensor<2x16xf16>
  %f32 = tensor.empty() : tensor<2x16xf32>
  %res1 = hivm.hir.vcumprod ins(%s16 : tensor<2x16xi16>) outs(%s16 : tensor<2x16xi16>) cum_dims = [0] reverse = true -> tensor<2x16xi16>
  %res2 = hivm.hir.vcumprod ins(%s32 : tensor<2x16xi32>) outs(%s32 : tensor<2x16xi32>) cum_dims = [1] reverse = true -> tensor<2x16xi32>
  %res3 = hivm.hir.vcumprod ins(%s64 : tensor<2x16xi64>) outs(%s64 : tensor<2x16xi64>) cum_dims = [1] reverse = true -> tensor<2x16xi64>
  %res4 = hivm.hir.vcumprod ins(%f16 : tensor<2x16xf16>) outs(%f16 : tensor<2x16xf16>) cum_dims = [0] reverse = true -> tensor<2x16xf16>
