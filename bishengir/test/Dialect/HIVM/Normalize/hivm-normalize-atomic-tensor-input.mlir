...
// CHECK: scope.scope : () -> () {
// CHECK: bufferization.to_memref %{{.*}} : memref<256xi16>
...
// CHECK: scope.scope : () -> () {
// CHECK: bufferization.to_memref %{{.*}} : memref<?x?xi16, strided<[256, 1]>>
// CHECK: memref.copy %{{.*}}, %{{.*}} : memref<?x?xi16, strided<[256, 1]>> to memref<?x?xi16, strided<[256, 1]>>
// CHECK: %[[LOCK:.*]] = hivm.hir.create_sync_block_lock : memref<1xi64>
// CHECK: hivm.hir.sync_block_lock lock_var(%[[LOCK]] : memref<1xi64>)
