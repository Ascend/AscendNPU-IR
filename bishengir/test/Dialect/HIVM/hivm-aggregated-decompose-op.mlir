...
func.func @test_reduce_op_multi_memref_01() {
  ...
  hivm.hir.vreduce <sum> ins(%src : memref<16x8x32xf32>) outs(%dst : memref<1x1x32xf32>) unsigned_src = false reduce_dims = [0, 1]
  return
}
...
func.func @test_reduce_op_multi_memref_01_dyn(%d : index) {
  ...
  hivm.hir.vreduce <sum> ins(%src : memref<16x8x?xf32>) outs(%dst : memref<1x1x?xf32>) unsigned_src = false reduce_dims = [0, 1]
  return
}

// -----
// BEFOREALIGN-LABEL: func.func @test_reduce_op_multi_memref_01_dyn2
func.func @test_reduce_op_multi_memref_01_dyn2(%d : index) {
  ...
  %dst = memref.alloc() : memref<1x1x32xf32>
  hivm.hir.vreduce <sum> ins(%src : memref<16x?x32xf32>) outs(%dst : memref<1x1x32xf32>) unsigned_src = false reduce_dims = [0, 1]
  return
}
...
func.func @test_reduce_op_multi_memref_013() {
  ...
  hivm.hir.vreduce <sum> ins(%src : memref<16x8x10x32xf32>) outs(%dst : memref<1x1x10x1xf32>) unsigned_src = false reduce_dims = [0, 1, 3]
  return
}
...
func.func @test_reduce_op_multi_memref_023() {
  ...
  hivm.hir.vreduce <sum> ins(%src : memref<16x8x10x32xf32>) outs(%dst : memref<1x8x1x1xf32>) unsigned_src = false reduce_dims = [0, 2, 3]
  return
}
...
func.func @test_reduce_op_multi_memref_0234() {
  ...
  hivm.hir.vreduce <sum> ins(%src : memref<16x8x10x32x16x16xf32>) outs(%dst : memref<1x8x1x1x1x16xf32>) unsigned_src = false reduce_dims = [0, 2, 3, 4]
  return
}
...
func.func @test_reduce_op_multi_tensor_01() -> tensor<1x1x32xf32> {
  ...
  %res = hivm.hir.vreduce <sum> ins(%src : tensor<16x8x32xf32>) outs(%dst : tensor<1x1x32xf32>) unsigned_src = false reduce_dims = [0, 1] -> tensor<1x1x32xf32>
...
func.func @test_reduce_op_multi_tensor_01_dyn(%d : index) -> tensor<1x1x?xf32> {
  ...
  %res = hivm.hir.vreduce <sum> ins(%src : tensor<16x8x?xf32>) outs(%dst : tensor<1x1x?xf32>) unsigned_src = false reduce_dims = [0, 1] -> tensor<1x1x?xf32>
...
func.func @test_reduce_op_multi_tensor_01_dyn2(%d : index) -> tensor<1x1x32xf32> {
  ...
  %dst = tensor.empty() : tensor<1x1x32xf32>
  %res = hivm.hir.vreduce <sum> ins(%src : tensor<16x?x32xf32>) outs(%dst : tensor<1x1x32xf32>) unsigned_src = false reduce_dims = [0, 1] -> tensor<1x1x32xf32>
  return %res : tensor<1x1x32xf32>
}
...
func.func @test_reduce_op_multi_tensor_013() -> tensor<1x1x10x1xf32> {
  ...
  %res = hivm.hir.vreduce <sum> ins(%src : tensor<16x8x10x32xf32>) outs(%dst : tensor<1x1x10x1xf32>) unsigned_src = false reduce_dims = [0, 1, 3] -> tensor<1x1x10x1xf32>
...
// AFTERDEINTERLEAVE-SAME:    %[[VAL_0:.+]]: index,
// AFTERDEINTERLEAVE-SAME:    %[[VAL_1:.+]]: index
func.func @test_i8_vdeinterleave_memref(%arg0: index, %arg1: index) {
  %0 = memref.alloc(): memref<32xi8>
  %1 = memref.subview %0[0] [%arg0] [1] : memref<32xi8> to memref<?xi8>
  %2 = memref.alloc(): memref<32xi8>
  %3 = memref.subview %2[0] [%arg1] [1] : memref<32xi8> to memref<?xi8>
  // AFTERDEINTERLEAVE:           %[[VAL_2:.*]] = memref.alloc() : memref<32xi8>
  // AFTERDEINTERLEAVE:           %[[VAL_3:.*]] = memref.subview %[[VAL_2]][0] {{\[}}%[[VAL_0]]] [1] : memref<32xi8> to memref<?xi8>
  // AFTERDEINTERLEAVE:           %[[VAL_4:.*]] = memref.alloc() : memref<32xi8>
  // AFTERDEINTERLEAVE:           %[[VAL_5:.*]] = memref.subview %[[VAL_4]][0] {{\[}}%[[VAL_1]]] [1] : memref<32xi8> to memref<?xi8>
  // AFTERDEINTERLEAVE:           %[[VAL_6:.*]] = memref.alloc() : memref<32xf16>
  // AFTERDEINTERLEAVE:           %[[VAL_7:.*]] = memref.subview %[[VAL_6]][0] {{\[}}%[[VAL_0]]] [1] : memref<32xf16> to memref<?xf16, strided<[1]>>
  // AFTERDEINTERLEAVE:           hivm.hir.vcast ins(%[[VAL_3]] : memref<?xi8>) outs(%[[VAL_7]] : memref<?xf16, strided<[1]>>)
  // AFTERDEINTERLEAVE:           %[[VAL_8:.*]] = memref.alloc() : memref<32xf16>
  // AFTERDEINTERLEAVE:           %[[VAL_9:.*]] = memref.subview %[[VAL_8]][0] {{\[}}%[[VAL_1]]] [1] : memref<32xf16> to memref<?xf16, strided<[1]>>
  // AFTERDEINTERLEAVE:           hivm.hir.vdeinterleave ins(%[[VAL_7]] : memref<?xf16, strided<[1]>>) outs(%[[VAL_9]] : memref<?xf16, strided<[1]>>) channel_num = 32 index_mode = <CHANNEL_0>
  // AFTERDEINTERLEAVE:           hivm.hir.vcast ins(%[[VAL_9]] : memref<?xf16, strided<[1]>>) outs(%[[VAL_5]] : memref<?xi8>) round_mode = <trunc>
  // AFTERDEINTERLEAVE:           return
  hivm.hir.vdeinterleave ins(%1 : memref<?xi8>) outs(%3 : memref<?xi8>) channel_num = 32 index_mode = <CHANNEL_0>
  return
}

// -----
// AFTERDEINTERLEAVE-LABEL:   @test_i8_vdeinterleave_tensor
// AFTERDEINTERLEAVE-SAME:    %[[VAL_0:.+]]: tensor<?xi8>
func.func @test_i8_vdeinterleave_tensor(%arg0: tensor<?xi8>) -> tensor<?xi8> {
  // AFTERDEINTERLEAVE:       %[[VAL_1:.+]] = arith.constant 0 : index
  // AFTERDEINTERLEAVE:       %[[VAL_2:.+]] = tensor.dim %[[VAL_0]], %[[VAL_1]] : tensor<?xi8>
  // AFTERDEINTERLEAVE:       %[[VAL_3:.+]] = tensor.empty(%[[VAL_2]]) : tensor<?xi8>
  // AFTERDEINTERLEAVE:       %[[VAL_4:.+]] = tensor.dim %[[VAL_0]], %[[VAL_1]] : tensor<?xi8>
  // AFTERDEINTERLEAVE:       %[[VAL_5:.+]] = tensor.empty(%[[VAL_4]]) : tensor<?xf16>
  // AFTERDEINTERLEAVE:       %[[VAL_6:.+]] = hivm.hir.vcast ins(%[[VAL_0]] : tensor<?xi8>) outs(%[[VAL_5]] : tensor<?xf16>) -> tensor<?xf16>
  // AFTERDEINTERLEAVE:       %[[VAL_7:.+]] = tensor.dim %[[VAL_3]], %[[VAL_1]] : tensor<?xi8>
  // AFTERDEINTERLEAVE:       %[[VAL_8:.+]] = tensor.empty(%[[VAL_7]]) : tensor<?xf16>
  // AFTERDEINTERLEAVE:       %[[VAL_9:.+]] = hivm.hir.vdeinterleave ins(%[[VAL_6]] : tensor<?xf16>) outs(%[[VAL_8]] : tensor<?xf16>) channel_num = 32 index_mode = <CHANNEL_0> -> tensor<?xf16>
  // AFTERDEINTERLEAVE:       %[[VAL_10:.+]] = hivm.hir.vcast ins(%[[VAL_9]] : tensor<?xf16>) outs(%[[VAL_3]] : tensor<?xi8>) round_mode = <trunc> -> tensor<?xi8>
  // AFTERDEINTERLEAVE:       return %[[VAL_10]] : tensor<?xi8>
  %c0 = arith.constant 0 : index
...
// AFTERDEINTERLEAVE-SAME:    %[[VAL_0:.+]]: memref<32xi8>,
// AFTERDEINTERLEAVE-SAME:    %[[VAL_1:.+]]: memref<16xi8>,
// AFTERDEINTERLEAVE-SAME:    %[[VAL_2:.+]]: memref<16xi8>)
func.func @test_i8_vdeinterleave_multi_outs(%src: memref<32xi8>, %dst_even: memref<16xi8>,
                                          %dst_odd: memref<16xi8>) {
  // AFTERDEINTERLEAVE:       %[[VAL_3:.+]] = memref.alloc() : memref<32xf16>
  // AFTERDEINTERLEAVE:       hivm.hir.vcast ins(%[[VAL_0]] : memref<32xi8>) outs(%[[VAL_3]] : memref<32xf16>)
  // AFTERDEINTERLEAVE:       %[[VAL_4:.+]] = memref.alloc() : memref<16xf16>
  // AFTERDEINTERLEAVE:       %[[VAL_5:.+]] = memref.alloc() : memref<16xf16>
  // AFTERDEINTERLEAVE:       hivm.hir.vdeinterleave ins(%[[VAL_3]] : memref<32xf16>) outs(%[[VAL_4]], %[[VAL_5]] : memref<16xf16>, memref<16xf16>)
  // AFTERDEINTERLEAVE:       hivm.hir.vcast ins(%[[VAL_4]] : memref<16xf16>) outs(%[[VAL_1]] : memref<16xi8>)
  // AFTERDEINTERLEAVE:       hivm.hir.vcast ins(%[[VAL_5]] : memref<16xf16>) outs(%[[VAL_2]] : memref<16xi8>)
  // AFTERDEINTERLEAVE:       return
...
// AFTERDEINTERLEAVE-SAME:    %[[VAL_0:.+]]: tensor<32xi8>
func.func @test_i8_vdeinterleave_multi_outs_tensor(%src: tensor<32xi8>) -> (tensor<16xi8>, tensor<16xi8>) {
  // AFTERDEINTERLEAVE:       %[[VAL_1:.+]] = tensor.empty() : tensor<16xi8>
  // AFTERDEINTERLEAVE:       %[[VAL_2:.+]] = tensor.empty() : tensor<16xi8>
  // AFTERDEINTERLEAVE:       %[[VAL_3:.+]] = tensor.empty() : tensor<32xf16>
  // AFTERDEINTERLEAVE:       %[[VAL_4:.+]] = hivm.hir.vcast ins(%[[VAL_0]] : tensor<32xi8>) outs(%[[VAL_3]] : tensor<32xf16>) -> tensor<32xf16>
  // AFTERDEINTERLEAVE:       %[[VAL_5:.+]] = tensor.empty() : tensor<16xf16>
  // AFTERDEINTERLEAVE:       %[[VAL_6:.+]] = tensor.empty() : tensor<16xf16>
  // AFTERDEINTERLEAVE:       %[[VAL_7:.+]]:2 = hivm.hir.vdeinterleave ins(%[[VAL_4]] : tensor<32xf16>) outs(%[[VAL_5]], %[[VAL_6]] : tensor<16xf16>, tensor<16xf16>) -> tensor<16xf16>, tensor<16xf16>
  // AFTERDEINTERLEAVE:       %[[VAL_8:.+]] = hivm.hir.vcast ins(%[[VAL_7]]#0 : tensor<16xf16>) outs(%[[VAL_1]] : tensor<16xi8>) round_mode = <trunc> -> tensor<16xi8>
  // AFTERDEINTERLEAVE:       %[[VAL_9:.+]] = hivm.hir.vcast ins(%[[VAL_7]]#1 : tensor<16xf16>) outs(%[[VAL_2]] : tensor<16xi8>) round_mode = <trunc> -> tensor<16xi8>
  // AFTERDEINTERLEAVE:       return %[[VAL_8]], %[[VAL_9]] : tensor<16xi8>, tensor<16xi8>
...
func.func @test_dynamic_pad(%arg0: index, %arg1: index, %in: memref<128x2048xf32>, %cst: f32) {
