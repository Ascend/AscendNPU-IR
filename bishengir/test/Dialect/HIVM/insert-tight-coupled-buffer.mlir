...
module attributes {hacc.target = #hacc.target<"Ascend950PR_9579">} {
  ...
  // CHECK:   %[[LOAD1:.*]] = hivm.hir.load ins(%[[ARG0]] : tensor<16x16xf16>) outs(%{{.*}} : tensor<16x16xf16>) {"inserted-load"} core_type = <CUBE> -> tensor<16x16xf16>
...
module attributes {hacc.target = #hacc.target<"Ascend950PR_9579">} {
  ...
  func.func @test_fixpipe_to_mmadl1_tight_coupled(%arg0: tensor<16x16xf32>, %arg1: tensor<16x16xf32>, %arg2: tensor<16x16xf32>, %arg3: tensor<16x16xf32>) -> tensor<16x16xf32> attributes {hacc.entry, hacc.function_kind = #hacc.function_kind<DEVICE>} {
    %true = arith.constant true
    %c16 = arith.constant 16 : index
    // CHECK: %[[ARG0_LOAD:.*]] = hivm.hir.load ins(%[[ARG0]] : tensor<16x16xf32>) outs(%{{.*}} : tensor<16x16xf32>) {"inserted-load"} core_type = <CUBE> -> tensor<16x16xf32>
    // CHECK: %[[ARG1_LOAD:.*]] = hivm.hir.load ins(%[[ARG1]] : tensor<16x16xf32>) outs(%{{.*}} : tensor<16x16xf32>) {"inserted-load"} core_type = <CUBE> -> tensor<16x16xf32>
    // CHECK: %[[ARG2_LOAD:.*]] = hivm.hir.load ins(%[[ARG2]] : tensor<16x16xf32>) outs(%{{.*}} : tensor<16x16xf32>) {"inserted-load"} core_type = <CUBE> -> tensor<16x16xf32>
...
module attributes {hacc.target = #hacc.target<"Ascend950PR_9579">} {
  ...
  // CHECK: %[[ARG0_LOAD:.*]] = hivm.hir.load ins(%[[ARG0]] : tensor<16x16xf16>) outs(%{{.*}} : tensor<16x16xf16>) {"inserted-load"} core_type = <CUBE> -> tensor<16x16xf16>
