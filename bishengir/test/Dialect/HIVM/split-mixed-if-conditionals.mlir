...
// -----
// CHECK-LABEL: @no_split_mix_if_with_nested_for_loop
func.func @no_split_mix_if_with_nested_for_loop(%arg0: i1, %arg1: i1, %arg2: index, %arg3: tensor<32x32xf32>, %arg4: tensor<32x32xf32>, %arg5: tensor<32x32xf16>, %arg6: tensor<32x32xf16>, %arg7: tensor<32x32xf32>) -> tensor<32x32xf32> attributes {hacc.entry, hivm.func_core_type = #hivm.func_core_type<MIX>} {
  ...
  // CHECK:        %[[c32:.*]] = arith.constant 32 : index
  // CHECK:        %[[VAL_0:.*]] = scf.if %[[arg0:.*]] -> (tensor<32x32xf32>) {
  // CHECK:          %[[VAL_2:.*]] = scf.for %{{.*}} = %{{.*}} to %{{.*}} step %{{.*}} iter_args(%{{.*}} = %{{.*}}) -> (tensor<32x32xf32>) {
  // CHECK:            %[[VAL_3:.*]] = hivm.hir.mmadL1 {already_set_real_mkn, fixpipe_already_inserted = true} ins(%{{.*}}, %{{.*}}, %[[arg1:.*]], %[[c32]], %[[c32]], %[[c32]] : tensor<32x32xf16>, tensor<32x32xf16>, i1, index, index, index) outs(%{{.*}} : tensor<32x32xf32>) -> tensor<32x32xf32>
  // CHECK:            %[[VAL_4:.*]] = hivm.hir.vadd ins(%{{.*}}, %{{.*}} : tensor<32x32xf32>, tensor<32x32xf32>) outs(%{{.*}} : tensor<32x32xf32>) -> tensor<32x32xf32>
  // CHECK:            scf.yield %{{.*}} : tensor<32x32xf32>
  // CHECK:          }
  // CHECK:          scf.yield %{{.*}} : tensor<32x32xf32>
  // CHECK:        } else {
  ...
  %0 = scf.if %arg0 -> (tensor<32x32xf32>) {
    ...
    %res = scf.for %i = %c0 to %c4 step %c1 iter_args(%acc = %init) -> (tensor<32x32xf32>) {
      %3 = tensor.empty() : tensor<32x32xf16>
      %4 = hivm.hir.copy ins(%arg5 : tensor<32x32xf16>) outs(%3 : tensor<32x32xf16>) -> tensor<32x32xf16>
      %5 = hivm.hir.copy ins(%arg6 : tensor<32x32xf16>) outs(%3 : tensor<32x32xf16>) -> tensor<32x32xf16>
      %6 = tensor.empty() : tensor<32x32xf32>
...
// -----
// CHECK-LABEL: @split_mix_if_with_nested_if
func.func @split_mix_if_with_nested_if(%arg0: i1, %arg1: i1, %arg2: index, %arg3: tensor<32x32xf32>, %arg4: tensor<32x32xf32>, %arg5: tensor<32x32xf16>, %arg6: tensor<32x32xf16>, %arg7: tensor<32x32xf32>) -> tensor<32x32xf32> attributes {hacc.entry, hivm.func_core_type = #hivm.func_core_type<MIX>} {
  %c32 = arith.constant 32 : index
  // CHECK:        %[[c32:.*]] = arith.constant 32 : index
  // CHECK:        %[[VAL_0:.*]] = scf.if %[[arg0:.*]] -> (tensor<32x32xf32>) {
  // CHECK:          %[[VAL_5:.*]]:3 = scf.if %[[arg1:.*]] -> (tensor<32x32xf16>, tensor<32x32xf16>, tensor<32x32xf32>) {
  // CHECK:            scf.yield %{{.*}} : tensor<32x32xf16>, tensor<32x32xf16>, tensor<32x32xf32>
  // CHECK:          } else {
  // CHECK:            scf.yield %{{.*}} : tensor<32x32xf16>, tensor<32x32xf16>, tensor<32x32xf32>
  // CHECK:          } {hivm.split_done = true}
  // CHECK:          %[[VAL_6:.*]] = scf.if %[[arg1]] -> (tensor<32x32xf32>) {
  // CHECK:            %[[VAL_11:.*]] = hivm.hir.mmadL1 {already_set_real_mkn, fixpipe_already_inserted = true} ins(%{{.*}}, %{{.*}}, %[[arg1]], %[[c32]], %[[c32]], %[[c32]] : tensor<32x32xf16>, tensor<32x32xf16>, i1, index, index, index) outs(%[[VAL_5]]#2 : tensor<32x32xf32>) -> tensor<32x32xf32>
  // CHECK:            scf.yield %{{.*}} : tensor<32x32xf32>
  // CHECK:          } else {
  // CHECK:            scf.yield %{{.*}} : tensor<32x32xf32>
  // CHECK:          } {hivm.split_done = true}
  // CHECK:          %[[VAL_7:.*]] = scf.if %[[arg1]] -> (tensor<32x32xf32>) {
  // CHECK:            scf.yield %{{.*}} : tensor<32x32xf32>
  // CHECK:          } else {
  // CHECK:            scf.yield %{{.*}} : tensor<32x32xf32>
  // CHECK:          } {hivm.split_done = true}
  // CHECK:          %[[VAL_8:.*]] = scf.if %[[arg1]] -> (tensor<32x32xf32>) {
  // CHECK:            %[[VAL_12:.*]] = hivm.hir.vadd ins(%[[VAL_6]], %[[VAL_7]] : tensor<32x32xf32>, tensor<32x32xf32>) outs(%[[VAL_5]]#2 : tensor<32x32xf32>) -> tensor<32x32xf32>
  // CHECK:            scf.yield %{{.*}} : tensor<32x32xf32>
  // CHECK:          } else {
  // CHECK:            scf.yield %{{.*}} : tensor<32x32xf32>
  // CHECK:          } {hivm.split_done = true}
  // CHECK:          %[[VAL_9:.*]] = scf.if %[[arg1]] -> (tensor<32x32xf32>) {
  // CHECK:            scf.yield %{{.*}} : tensor<32x32xf32>
  // CHECK:          } else {
  // CHECK:            scf.yield %{{.*}} : tensor<32x32xf32>
  // CHECK:          } {hivm.split_done = true}
  // CHECK:          %[[VAL_10:.*]] = scf.if %[[arg1]] -> (tensor<32x32xf32>) {
  // CHECK:            scf.yield %[[VAL_8]] : tensor<32x32xf32>
  // CHECK:          } else {
  // CHECK:            scf.yield %[[VAL_9]] : tensor<32x32xf32>
  // CHECK:          } {hivm.split_done = true}
  ...
  // CHECK:        } {hivm.split_done = true}
  %0 = scf.if %arg0 -> (tensor<32x32xf32>) {
    %inner = scf.if %arg1 -> (tensor<32x32xf32>) {
      %3 = tensor.empty() : tensor<32x32xf16>
      %4 = hivm.hir.copy ins(%arg5 : tensor<32x32xf16>) outs(%3 : tensor<32x32xf16>) -> tensor<32x32xf16>
      %5 = hivm.hir.copy ins(%arg6 : tensor<32x32xf16>) outs(%3 : tensor<32x32xf16>) -> tensor<32x32xf16>
      %6 = tensor.empty() : tensor<32x32xf32>
    ...
    %3 = tensor.empty() : tensor<32x32xf32>
    %4 = hivm.hir.copy ins(%arg4 : tensor<32x32xf32>) outs(%3 : tensor<32x32xf32>) -> tensor<32x32xf32>
    scf.yield %4 : tensor<32x32xf32>
  }
  %1 = tensor.empty() : tensor<32x32xf32>
  %2 = hivm.hir.fixpipe {dma_mode = #hivm.dma_mode<nz2nd>} ins(%0 : tensor<32x32xf32>) outs(%1 : tensor<32x32xf32>) -> tensor<32x32xf32>
