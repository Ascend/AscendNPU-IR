...
func.func @test_vreduce_op_static() {
  ...
  hivm.hir.vreduce <max_with_index> ins(%src_r : memref<10xi32>)
  ...
  // CHECK: hivm.hir.vreduce{{.*}} temp_buffer({{.*}}memref<8xi32>)
  hivm.hir.vreduce <max_with_index> ins(%src_ar : memref<2x10xi32>)
  ...
  hivm.hir.vreduce <max_with_index> ins(%src_ra : memref<10x2xi32>)
  ...
  // CHECK: hivm.hir.vreduce{{.*}} temp_buffer({{.*}}memref<24xi32>)
  hivm.hir.vreduce <max_with_index> ins(%src_mid : memref<2x10x2xi32>)
...
func.func @test_vreduce_op_dynamic(%offset: index, %d: index) {
  ...
  hivm.hir.vreduce <min_with_index> ins(%src : memref<?x2xf32>)
...
func.func @test_reduce_temp_buffer() {
  ...
  hivm.hir.vreduce <max> ins(%reinterpret_cast_1 : memref<256x1xf32, strided<[8, 1]>, #hivm.address_space<ub>>) outs(%reinterpret_cast_2 : memref<1x1xf32, strided<[1, 1]>, #hivm.address_space<ub>>) unsigned_src = false reduce_dims = [0]
  return
}
...
func.func @test_reducear_temp_buffer() {
  ...
  hivm.hir.vreduce <min> ins(%collapse_shape : memref<1680x4xi32, strided<[8, 1]>, #hivm.address_space<ub>>) outs(%collapse_shape_1 : memref<1680x1xi32, #hivm.address_space<ub>>) unsigned_src = false reduce_dims = [1]
  return
}
...
func.func @test_reducear_xori_temp_buffer() {
  ...
  hivm.hir.vreduce <xori> ins(%collapse_shape : memref<1680x4xi32, strided<[8, 1]>, #hivm.address_space<ub>>) outs(%collapse_shape_1 : memref<1680x1xi32, #hivm.address_space<ub>>) unsigned_src = false reduce_dims = [1]
  return
}
...
// CHECK-LABEL: func @test_vsort_op_int32
func.func @test_vsort_op_int32() {
  %src = memref.alloc() : memref<32xi32>
  %dst_value = memref.alloc() : memref<32xi32>
  %dst_index = memref.alloc() : memref<32xi32>
  // CHECK: memref.alloc() : memref<384xi32>
  // CHECK: hivm.hir.vsort{{.*}}temp_buffer({{.*}}memref<384xi32>)
  hivm.hir.vsort ins(%src : memref<32xi32>)
                 outs(%dst_value : memref<32xi32>)
                 descending = false
                 sort_axis = 0
  // CHECK: memref.alloc() : memref<384xi32>
  // CHECK: hivm.hir.vsort{{.*}}temp_buffer({{.*}}memref<384xi32>)
...
// CHECK-LABEL: func @test_vsort_op_int64
func.func @test_vsort_op_int64() {
  %src = memref.alloc() : memref<32xi64>
  %dst_value = memref.alloc() : memref<32xi64>
  %dst_index = memref.alloc() : memref<32xi32>
  // CHECK: memref.alloc() : memref<320xi64>
  // CHECK: hivm.hir.vsort{{.*}}temp_buffer({{.*}}memref<320xi64>)
  hivm.hir.vsort ins(%src : memref<32xi64>)
                 outs(%dst_value : memref<32xi64>)
                 descending = false
                 sort_axis = 0
  // CHECK: memref.alloc() : memref<320xi64>
  // CHECK: hivm.hir.vsort{{.*}}temp_buffer({{.*}}memref<320xi64>)
...
func.func @test_cumsum_i1_2d_dim1_extra_buffer(%src: memref<3x256xi1>,
                                               %dst: memref<3x256xi1>) {
  // CHECK-LABEL: func.func @test_cumsum_i1_2d_dim1_extra_buffer
  // CHECK: memref.alloc() : memref<256x256xi1>
  // CHECK: hivm.hir.vcumsum{{.*}}temp_buffer({{.*}}memref<256x256xi1>)
  hivm.hir.vcumsum ins(%src : memref<3x256xi1>) outs(%dst : memref<3x256xi1>) cum_dims = [1] reverse = false
  return
}

// -----

func.func @test_cumsum_i1_3d_dim1_extra_buffer(%src: memref<3x1024x2xi1>,
                                               %dst: memref<3x1024x2xi1>) {
  // CHECK-LABEL: func.func @test_cumsum_i1_3d_dim1_extra_buffer
  // CHECK: memref.alloc() : memref<3x1024x256xi1>
  // CHECK: hivm.hir.vcumsum{{.*}}temp_buffer({{.*}}memref<3x1024x256xi1>)
  hivm.hir.vcumsum ins(%src : memref<3x1024x2xi1>) outs(%dst : memref<3x1024x2xi1>) cum_dims = [1] reverse = false
  return
}
