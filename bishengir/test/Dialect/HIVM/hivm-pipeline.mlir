// RUN: bishengir-opt -hacc-append-device-spec=target=Ascend910B1 -lower-hivm-pipeline %s | FileCheck %s
// RUN: bishengir-opt -hacc-append-device-spec=target=Ascend910B1 -lower-hivm-pipeline="enable-hivm-inject-barrier-all-sync=true" %s | FileCheck %s -check-prefix=CHECK-BAR-ALL
