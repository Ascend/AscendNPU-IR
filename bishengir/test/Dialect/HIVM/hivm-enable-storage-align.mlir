...
func.func @test_mark_reduce_max_with_index_inner(%arg0: memref<?xf32, #hivm.address_space<gm>> {tt.divisibility = 16 : i32}, %arg1: memref<?xi32, #hivm.address_space<gm>> {tt.divisibility = 16 : i32}, %arg2: memref<?xf32, #hivm.address_space<gm>> {tt.divisibility = 16 : i32}, %arg3: memref<?xi32, #hivm.address_space<gm>>, %arg4: i32) attributes {func_dyn_memref_args = dense<[true, true, true, true, false]> : vector<5xi1>, global_kernel = "local", hacc.dso_local = "", hacc.entry = "", hacc.function_kind = #hacc.function_kind<DEVICE>, hacc.spir_kernel = "", hivm.func_core_type = #hivm.func_core_type<AIV>} {
  ...
  annotation.mark %alloc {hivm.stride_align_dims = array<i32: 0>, hivm.stride_align_value_in_byte = array<i32: 32>} : memref<4096x4xf32, #hivm.address_space<ub>>
  // CHECK: hivm.hir.vreduce <max_with_index> ins(%[[SUBVIEW]] : memref<4096x4xf32, strided<[8, 1]>, #hivm.address_space<ub>>)
...
func.func @test_cumsum_unalignment(%arg0: memref<5x3x3x3x3x5xi32, #hivm.address_space<gm>>, %arg1: memref<5x3x3x3x3x5xi32, #hivm.address_space<gm>>, %arg2: memref<5x3x3x3x3x5xi32, #hivm.address_space<gm>>) attributes {hacc.entry, hivm.func_core_type = #hivm.func_core_type<AIV>} {
  ...
  hivm.hir.vcumsum ins(%alloc : memref<5x3x3x3x3x5xi32, #hivm.address_space<ub>>) outs(%alloc_0 : memref<5x3x3x3x3x5xi32, #hivm.address_space<ub>>) cum_dims = [2] reverse = true
  // CHECK: memref.alloc() : memref<5x3x3x8x3x5x1xi32, #hivm.address_space<ub>>
  ...
  hivm.hir.vcumprod ins(%alloc : memref<5x3x3x3x3x5xi32, #hivm.address_space<ub>>) outs(%alloc_1 : memref<5x3x3x3x3x5xi32, #hivm.address_space<ub>>) cum_dims = [2] reverse = true
...
// CHECK-LABEL: func @call_with_2_args_same_outlined_vf_0
// CHECK: memref.subview %arg2{{\[}}%arg3, 0] {{\[}}1, 2] {{\[}}1, 1]
// CHECK-SAME: to memref<1x2xf16, strided<{{\[}}16, 1]
module attributes {dlti.target_system_spec = #dlti.target_system_spec<"NPU" : #hacc.target_device_spec<#dlti.dl_entry<"AI_CORE_COUNT", 32 : i32>, #dlti.dl_entry<"CUBE_CORE_COUNT", 32 : i32>, #dlti.dl_entry<"VECTOR_CORE_COUNT", 64 : i32>, #dlti.dl_entry<"UB_SIZE", 2031616 : i32>, #dlti.dl_entry<"L1_SIZE", 4194304 : i32>, #dlti.dl_entry<"L0A_SIZE", 524288 : i32>, #dlti.dl_entry<"L0B_SIZE", 524288 : i32>, #dlti.dl_entry<"L0C_SIZE", 2097152 : i32>, #dlti.dl_entry<"UB_ALIGN_SIZE", 256 : i32>, #dlti.dl_entry<"L1_ALIGN_SIZE", 256 : i32>, #dlti.dl_entry<"L0C_ALIGN_SIZE", 4096 : i32>, #dlti.dl_entry<"ARCH", "dav-c310">>>, hacc.target = #hacc.target<"Ascend950PR_9589">, hivm.module_core_type = #hivm.module_core_type<AIV>} {
  func.func @call_with_2_args_same_outlined_vf_0(%arg0: memref<2x2xf16, #hivm.address_space<ub>>, %arg1: memref<2x1048x2xi8, #hivm.address_space<ub>>, %arg2: memref<2x2xf16, #hivm.address_space<ub>>) attributes {hivm.func_core_type = #hivm.func_core_type<AIV>, hivm.vector_function} {
    %cst = arith.constant 0.000000e+00 : f16
    %c0_i8 = arith.constant 0 : i8
    %c2 = arith.constant 2 : index
    %c1 = arith.constant 1 : index
    %c1048 = arith.constant 1048 : index
    %c0 = arith.constant 0 : index
    %0 = vector.constant_mask [1, 1, 2] : vector<1x1x128xi1>
    %1 = vector.constant_mask [1, 2] : vector<1x128xi1>
    scf.for %arg3 = %c0 to %c2 step %c1 {
      %subview = memref.subview %arg0[%arg3, 0] [1, 2] [1, 1] : memref<2x2xf16, #hivm.address_space<ub>> to memref<1x2xf16, strided<[2, 1], offset: ?>, #hivm.address_space<ub>>
      %2 = vector.transfer_read %subview[%c0, %c0], %cst, %1 {in_bounds = [true, true]} : memref<1x2xf16, strided<[2, 1], offset: ?>, #hivm.address_space<ub>>, vector<1x128xf16>
      %3 = scf.for %arg4 = %c0 to %c1048 step %c1 iter_args(%arg5 = %2) -> (vector<1x128xf16>) {
        %subview_1 = memref.subview %arg1[%arg3, %arg4, 0] [1, 1, 2] [1, 1, 1] : memref<2x1048x2xi8, #hivm.address_space<ub>> to memref<1x1x2xi8, strided<[2096, 2, 1], offset: ?>, #hivm.address_space<ub>>
        %6 = vector.transfer_read %subview_1[%c0, %c0, %c0], %c0_i8, %0 {in_bounds = [true, true, true]} : memref<1x1x2xi8, strided<[2096, 2, 1], offset: ?>, #hivm.address_space<ub>>, vector<1x1x128xi8>
        %7 = arith.sitofp %6 {enable_saturate = false, round_mode = #hfusion.round_mode<rint>} : vector<1x1x128xi8> to vector<1x1x128xf16>
        %8 = vector.shape_cast %7 : vector<1x1x128xf16> to vector<1x128xf16>
        scf.yield %8 : vector<1x128xf16>
      }
      %subview_0 = memref.subview %arg2[%arg3, 0] [1, 2] [1, 1] : memref<2x2xf16, #hivm.address_space<ub>> to memref<1x2xf16, strided<[2, 1], offset: ?>, #hivm.address_space<ub>>
      %4 = arith.fptosi %3 {enable_saturate = false, round_mode = #hfusion.round_mode<trunc>} : vector<1x128xf16> to vector<1x128xi8>
      %5 = arith.sitofp %4 {enable_saturate = false, round_mode = #hfusion.round_mode<rint>} : vector<1x128xi8> to vector<1x128xf16>
      vector.transfer_write %5, %subview_0[%c0, %c0], %1 {in_bounds = [true, true]} : vector<1x128xf16>, memref<1x2xf16, strided<[2, 1], offset: ?>, #hivm.address_space<ub>>
    }
    return
  }
  func.func @call_with_2_args_same(%arg0: memref<?xi8, #hivm.address_space<gm>> {hacc.arg_type = #hacc.arg_type<sync_block_lock>}, %arg1: memref<?xi8, #hivm.address_space<gm>> {hacc.arg_type = #hacc.arg_type<workspace>}, %arg2: memref<?xi8, #hivm.address_space<gm>> {tt.divisibility = 16 : i32, tt.tensor_kind = 0 : i32}, %arg3: memref<?xi32, #hivm.address_space<gm>> {tt.divisibility = 16 : i32, tt.tensor_kind = 0 : i32}, %arg4: memref<?xi8, #hivm.address_space<gm>> {tt.divisibility = 16 : i32, tt.tensor_kind = 1 : i32}, %arg5: memref<?xf16, #hivm.address_space<gm>> {tt.divisibility = 16 : i32, tt.tensor_kind = 1 : i32}, %arg6: i32, %arg7: i32, %arg8: i32) attributes {SyncBlockLockArgIdx = 0 : i64, WorkspaceArgIdx = 1 : i64, func_dyn_memref_args = dense<[true, true, true, true, true, true, false, false, false]> : vector<9xi1>, hacc.entry, hacc.function_kind = #hacc.function_kind<DEVICE>, hivm.func_core_type = #hivm.func_core_type<AIV>, mix_mode = "aiv", parallel_mode = "simd"} {
    hivm.hir.set_ctrl false at ctrl[60]
    hivm.hir.set_ctrl true at ctrl[48]
    %0 = arith.muli %arg6, %arg7 : i32
    %1 = arith.muli %0, %arg8 : i32
    annotation.mark %1 {logical_block_num} : i32
    %reinterpret_cast = memref.reinterpret_cast %arg2 to offset: [0], sizes: [2, 1048, 2], strides: [2096, 2, 1] : memref<?xi8, #hivm.address_space<gm>> to memref<2x1048x2xi8, strided<[2096, 2, 1]>, #hivm.address_space<gm>>
    %alloc = memref.alloc() : memref<2x1048x2xi8, #hivm.address_space<ub>>
    annotation.mark %alloc {hivm.stride_align_dims = array<i32: 2>, hivm.stride_align_value_in_byte = array<i32: 32>} : memref<2x1048x2xi8, #hivm.address_space<ub>>
    hivm.hir.load ins(%reinterpret_cast : memref<2x1048x2xi8, strided<[2096, 2, 1]>, #hivm.address_space<gm>>) outs(%alloc : memref<2x1048x2xi8, #hivm.address_space<ub>>)
    %reinterpret_cast_0 = memref.reinterpret_cast %arg3 to offset: [0], sizes: [2, 1048, 2], strides: [2096, 2, 1] : memref<?xi32, #hivm.address_space<gm>> to memref<2x1048x2xi32, strided<[2096, 2, 1]>, #hivm.address_space<gm>>
    %collapse_shape = memref.collapse_shape %reinterpret_cast_0 [[0, 1, 2]] : memref<2x1048x2xi32, strided<[2096, 2, 1]>, #hivm.address_space<gm>> into memref<4192xi32, strided<[1]>, #hivm.address_space<gm>>
    %alloc_1 = memref.alloc() : memref<4192xi32, #hivm.address_space<ub>>
    hivm.hir.load ins(%collapse_shape : memref<4192xi32, strided<[1]>, #hivm.address_space<gm>>) outs(%alloc_1 : memref<4192xi32, #hivm.address_space<ub>>)
    %alloc_2 = memref.alloc() {alignment = 64 : i64} : memref<2x2xf16, #hivm.address_space<ub>>
    annotation.mark %alloc_2 {hivm.stride_align_dims = array<i32: 1>, hivm.stride_align_value_in_byte = array<i32: 32>} : memref<2x2xf16, #hivm.address_space<ub>>
    call @call_with_2_args_same_outlined_vf_0(%alloc_2, %alloc, %alloc_2) {hivm.vector_function} : (memref<2x2xf16, #hivm.address_space<ub>>, memref<2x1048x2xi8, #hivm.address_space<ub>>, memref<2x2xf16, #hivm.address_space<ub>>) -> ()
    %reinterpret_cast_3 = memref.reinterpret_cast %arg5 to offset: [0], sizes: [2, 2], strides: [2, 1] : memref<?xf16, #hivm.address_space<gm>> to memref<2x2xf16, strided<[2, 1]>, #hivm.address_space<gm>>
    hivm.hir.store ins(%alloc_2 : memref<2x2xf16, #hivm.address_space<ub>>) outs(%reinterpret_cast_3 : memref<2x2xf16, strided<[2, 1]>, #hivm.address_space<gm>>)
    return
  }
}
