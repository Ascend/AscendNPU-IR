...
// -----
module {
  // CHECK-LABEL: func @test_cumsum_2d_i64_dim1_lowered_to_scalar
  // CHECK: scf.for
  // CHECK: scf.for
  // CHECK: arith.addi {{.*}} : i64
  func.func @test_cumsum_2d_i64_dim1_lowered_to_scalar(%src: memref<2x16xi64>, %dst: memref<2x16xi64>) attributes {hacc.entry} {
    hivm.hir.vcumsum ins(%src : memref<2x16xi64>) outs(%dst : memref<2x16xi64>) cum_dims = [1] reverse = false
    return
  }
}
...
// -----
module {
  // CHECK-LABEL: func @test_cumsum_i32_not_lowered_to_scalar
  // CHECK-NOT: scf.for
  func.func @test_cumsum_i32_not_lowered_to_scalar(%src: memref<2x16xi32>, %dst: memref<2x16xi32>) attributes {hacc.entry} {
    hivm.hir.vcumsum ins(%src : memref<2x16xi32>) outs(%dst : memref<2x16xi32>) cum_dims = [1] reverse = false
    return
  }
}
