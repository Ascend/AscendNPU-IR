...
func.func @tensor.expand_shape(%t1: tensor<?x10xf32>, %sz0: index) -> tensor<2x?x10xf32> {
  ...
  // CHECK-ORIGINAL: %[[C0:.*]] = arith.constant 0 : index
  // CHECK-ORIGINAL: %[[DIM:.*]] = memref.dim %[[m1]], %[[C0]] : memref<?x10xf32>
  // CHECK-ORIGINAL: %[[C2:.*]] = arith.constant 2 : index
  // CHECK-ORIGINAL: %[[VAL_1:.*]] = arith.divui %[[DIM]], %[[C2]] : index
  // CHECK-ORIGINAL: %[[expanded:.*]] = memref.expand_shape %[[m1]] {{\[\[}}0, 1], [2]] output_shape [2, %[[VAL_1]], 10] : memref<?x10xf32> into memref<2x?x10xf32>
...
//  CHECK-SAME:     %[[o1:.*]]: index, %[[s1:.*]]: index, %[[sz0:.*]]: index
func.func @tensor.expand_shape_of_slice(
  ...
  // CHECK-ORIGINAL: %[[C7:.*]] = arith.constant 7 : index
  // CHECK-ORIGINAL: %[[VAL_1:.*]] = arith.divui %{{.*}}, %[[C7]] : index
  // CHECK-ORIGINAL: %[[expanded:.*]] = memref.expand_shape %[[subview]] {{\[\[}}0, 1], [2, 3]] output_shape [%[[VAL_1]], 7, 2, 5] : memref<?x10xf32, strided<[20, 1], offset: ?>> into memref<?x7x2x5xf32, strided<[140, 20, 5, 1], offset: ?>>
