...
func.func @test_reduce_with_index_canonicalized_to_linalg(%arg0: tensor<24x768x768xf32>) -> tensor<24x768x1xf32> attributes {hacc.entry, hacc.function_kind = #hacc.function_kind<DEVICE>} {
  ...
  %4:2 = hfusion.reduce_with_index {tie_break_left = true, unsigned_src = false} <max> ins(%arg0 : tensor<24x768x768xf32>) outs(%3, %1 : tensor<24x768xf32>, tensor<24x768xi64>) dimensions = [2] -> tensor<24x768xf32>, tensor<24x768xi64>
...
// -----
// CHECK-LABEL: func.func @linalg_reduce_with_index_scalar_outside_region
module {
  func.func @linalg_reduce_with_index_scalar_outside_region(%0: tensor<2x51x3x13xbf16>, %1: tensor<2x51x3x13xi32>) -> tensor<2x51x3x13xi32> {
    %true = arith.constant true
    %2 = tensor.empty() : tensor<2x51x3x13xbf16>
    %3 = tensor.empty() : tensor<2x51x3x13xi32>
    %4 = tensor.empty() : tensor<2x51x3x13xf32>
    %5 = hfusion.cast {cast = #hfusion.type_fn<cast_signed>, enable_overflow = true, enable_saturate = false, round_mode = #hfusion.round_mode<rint>} ins(%0 : tensor<2x51x3x13xbf16>) outs(%4 : tensor<2x51x3x13xf32>) -> tensor<2x51x3x13xf32>
    %6 = hfusion.cast {cast = #hfusion.type_fn<cast_signed>, enable_overflow = true, enable_saturate = false, round_mode = #hfusion.round_mode<rint>} ins(%2 : tensor<2x51x3x13xbf16>) outs(%4 : tensor<2x51x3x13xf32>) -> tensor<2x51x3x13xf32>
    // CHECK: linalg.fill ins({{.*}}i32) outs({{.*}}tensor<2x51x3x13xi32>) -> tensor<2x51x3x13xi32>
    %reduced:2 = linalg.reduce ins(%5, %1 : tensor<2x51x3x13xf32>, tensor<2x51x3x13xi32>) outs(%6, %3 : tensor<2x51x3x13xf32>, tensor<2x51x3x13xi32>) dimensions = []  {reduce_mode = "min_with_index", tie_break_left = "true", unsigned_src = "false"}
      (%in: f32, %in_5: i32, %init: f32, %init_6: i32) {
        %12 = arith.cmpf olt, %in, %init : f32
        %13 = arith.cmpf oeq, %in, %init : f32
        %14 = arith.cmpf une, %in, %in : f32
        %15 = arith.cmpf une, %init, %init : f32
        %16 = arith.xori %15, %true : i1
        %17 = arith.andi %14, %16 : i1
        %18 = arith.ori %12, %17 : i1
        %19 = arith.andi %14, %15 : i1
        %20 = arith.ori %13, %19 : i1
        %21 = arith.cmpi slt, %in_5, %init_6 : i32
        %22 = arith.andi %20, %21 : i1
        %23 = arith.ori %18, %22 : i1
        %24 = arith.select %23, %in, %init : f32
        %25 = arith.select %23, %in_5, %init_6 : i32
        linalg.yield %24, %25 : f32, i32
      }
