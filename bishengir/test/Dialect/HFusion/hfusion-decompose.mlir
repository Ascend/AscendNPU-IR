...
module {
  func.func @dot_scale_kernel_2D(%arg0: memref<?xi8>, %arg1: memref<?xi8>, %arg2: memref<?xf8E5M2>, %arg3: i32, %arg4: memref<?xi8>, %arg5: memref<?xf8E5M2>, %arg6: i32, %arg7: memref<?xi8>, %arg8: memref<?xf32>, %arg9: i32, %arg10: i32, %arg11: i32, %arg12: i32, %arg13: i32, %arg14: i32, %arg15: i32) {
    ...
    // CHECK: %[[SCALE_A_I16:.*]] = hfusion.cast {{.*}} ins(%{{.*}} : tensor<64x2xi8>) outs(%[[EMPTY_SCALE_A_I16]] : tensor<64x2xi16>) -> tensor<64x2xi16>
    // CHECK: %[[EMPTY_SCALE_B_I16:.*]] = tensor.empty() : tensor<64x2xi16>
    // CHECK: %[[SCALE_B_I16:.*]] = hfusion.cast {{.*}} ins(%{{.*}}: tensor<64x2xi8>) outs(%[[EMPTY_SCALE_B_I16]] : tensor<64x2xi16>) -> tensor<64x2xi16>
    // CHECK: %[[EMPTY_SEVEN_A:.*]] = tensor.empty() : tensor<64x2xi16>
    // CHECK: %[[SEVEN_A:.*]] = linalg.fill ins(%[[CST_7]] : i16) outs(%[[EMPTY_SEVEN_A]] : tensor<64x2xi16>) -> tensor<64x2xi16>
    // CHECK: %[[SCALE_A_SHL_7:.*]] = hfusion.elemwise_binary {fun = #hfusion.binary_fn<shli>} ins(%[[SCALE_A_I16]], %[[SEVEN_A]] : tensor<64x2xi16>, tensor<64x2xi16>) outs(%[[EMPTY_SEVEN_A]] : tensor<64x2xi16>) -> tensor<64x2xi16>
    // CHECK: %[[EMPTY_SEVEN_B:.*]] = tensor.empty() : tensor<64x2xi16>
    // CHECK: %[[SEVEN_B:.*]] = linalg.fill ins(%[[CST_7]] : i16) outs(%[[EMPTY_SEVEN_B]] : tensor<64x2xi16>) -> tensor<64x2xi16>
    // CHECK: %[[SCALE_B_SHL_7:.*]] = hfusion.elemwise_binary {fun = #hfusion.binary_fn<shli>} ins(%[[SCALE_B_I16]], %[[SEVEN_B]] : tensor<64x2xi16>, tensor<64x2xi16>) outs(%[[EMPTY_SEVEN_B]] : tensor<64x2xi16>) -> tensor<64x2xi16>
    ...
    // CHECK: %[[SCALE_A_F32:.*]] = hfusion.cast {{.*}} ins(%[[SCALE_A_BF16]] : tensor<64x2xbf16>) outs(%[[EMPTY_SCALE_A_F32]] : tensor<64x2xf32>) -> tensor<64x2xf32>
    // CHECK: %[[EMPTY_SCALE_B_F32:.*]] = tensor.empty() : tensor<64x2xf32>
    // CHECK: %[[SCALE_B_F32:.*]] = hfusion.cast {{.*}} ins(%[[SCALE_B_BF16]] : tensor<64x2xbf16>) outs(%[[EMPTY_SCALE_B_F32]] : tensor<64x2xf32>) -> tensor<64x2xf32>
    // CHECK: %[[EMPTY_A_F16:.*]] = tensor.empty() : tensor<64x64xf16>
    // CHECK: %[[A_F16:.*]] = hfusion.cast {{.*}} ins(%2 : tensor<64x64xf8E5M2>) outs(%[[EMPTY_A_F16]] : tensor<64x64xf16>) -> tensor<64x64xf16>
    // CHECK: %[[EMPTY_B_F16:.*]] = tensor.empty() : tensor<64x64xf16>
    // CHECK: %[[B_F16:.*]] = hfusion.cast {{.*}} ins(%4 : tensor<64x64xf8E5M2>) outs(%[[EMPTY_B_F16]] : tensor<64x64xf16>) -> tensor<64x64xf16>
    // CHECK: %[[EMPTY_SCALE_A_F16:.*]] = tensor.empty() : tensor<64x2xf16>
    // CHECK: %[[SCALE_A_F16:.*]] = hfusion.cast {{.*}} ins(%[[SCALE_A_F32]] : tensor<64x2xf32>) outs(%[[EMPTY_SCALE_A_F16]] : tensor<64x2xf16>) -> tensor<64x2xf16>
    // CHECK: %[[EMPTY_SCALE_B_F16:.*]] = tensor.empty() : tensor<64x2xf16>
    // CHECK: %[[SCALE_B_F16:.*]] = hfusion.cast {{.*}} ins(%[[SCALE_B_F32]] : tensor<64x2xf32>) outs(%[[EMPTY_SCALE_B_F16]] : tensor<64x2xf16>) -> tensor<64x2xf16>
