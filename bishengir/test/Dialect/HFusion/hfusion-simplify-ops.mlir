...
// The rewrite s*(A*B + bias) != (s*A)*B + bias when bias != 0, so the mul
// must stay outside the matmul.
// CHECK-LABEL: func.func @no_hoist_negation_bias_matmul
// CHECK: linalg.matmul
// CHECK: linalg.elemwise_binary {fun = #linalg.binary_fn<mul>}
// CHECK: return
func.func @no_hoist_negation_bias_matmul(%a: tensor<4x8xf32>, %b: tensor<8x4xf32>, %bias: tensor<4x4xf32>) -> tensor<4x4xf32> {
...
// -----
// Test: scalar mul NOT hoisted when matmul accumulates onto a non-zero bias.
// CHECK-LABEL: func.func @no_hoist_scale_bias_matmul
// CHECK: linalg.matmul
// CHECK: linalg.elemwise_binary {fun = #linalg.binary_fn<mul>}
// CHECK: return
func.func @no_hoist_scale_bias_matmul(%a: tensor<4x8xf32>, %b: tensor<8x4xf32>, %bias: tensor<4x4xf32>) -> tensor<4x4xf32> {
  %matmul = linalg.matmul ins(%a, %b : tensor<4x8xf32>, tensor<8x4xf32>) outs(%bias : tensor<4x4xf32>) -> tensor<4x4xf32>
...
// -----
// Test: scalar mul NOT hoisted when the matmul accumulates onto another
// matmul result (the attention-score pattern). The second matmul's init is
// the first matmul result, so scaling only its LHS drops the accumulated term.
// CHECK-LABEL: func.func @no_hoist_scale_matmul_accumulate
// CHECK: linalg.matmul
// CHECK: linalg.matmul
// CHECK: linalg.elemwise_binary {fun = #linalg.binary_fn<mul>}
// CHECK: return
func.func @no_hoist_scale_matmul_accumulate(%a: tensor<4x8xf32>, %b: tensor<8x4xf32>, %c: tensor<4x8xf32>, %d: tensor<8x4xf32>) -> tensor<4x4xf32> {
  %init = tensor.empty() : tensor<4x4xf32>
  %cst_zero = arith.constant 0.000000e+00 : f32
  %fill = linalg.fill ins(%cst_zero : f32) outs(%init : tensor<4x4xf32>) -> tensor<4x4xf32>
  %matmul1 = linalg.matmul ins(%a, %b : tensor<4x8xf32>, tensor<8x4xf32>) outs(%fill : tensor<4x4xf32>) -> tensor<4x4xf32>
  %matmul2 = linalg.matmul ins(%c, %d : tensor<4x8xf32>, tensor<8x4xf32>) outs(%matmul1 : tensor<4x4xf32>) -> tensor<4x4xf32>
  %cst_scale = arith.constant 2.000000e+00 : f32
  %out = tensor.empty() : tensor<4x4xf32>
  %scaled = linalg.elemwise_binary {fun = #linalg.binary_fn<mul>} ins(%matmul2, %cst_scale : tensor<4x4xf32>, f32) outs(%out : tensor<4x4xf32>) -> tensor<4x4xf32>
  return %scaled : tensor<4x4xf32>
}