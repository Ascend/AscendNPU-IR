...
// Case 3: function type mismatch (@baz) - origin () -> i32, injected (i32) -> i32;
//         pass replaces body and updates function type to match injected.
...
module {
  ...
  // Type mismatch: origin has no parameters, injected has one parameter.
  // After inject, @baz should have signature (i32) -> i32 and injected body.
  func.func @baz() -> i32 {
    %c0 = arith.constant 0 : i32
    return %c0 : i32
  }
...
// CHECK:   }
// CHECK:   func.func @baz(%{{.*}}: i32) -> i32 {
// CHECK:     %[[y:.*]] = arith.addi %{{.*}}, %{{.*}} : i32
// CHECK:     return %[[y]] : i32
// CHECK:   }
// CHECK: }
