//===- bishengir-fusion-ods-yaml-gen.cpp - HFusion ODS generation from yaml===//
//
...
// derived from the mathematical expression of hfusion named ops. Typically a
...
// Original Source:
// https://github.com/llvm/llvm-project/blob/main/mlir/tools/mlir-linalg-ods-gen/mlir-linalg-ods-yaml-gen.cpp
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "hfusion-ods-gen"
...
struct HFusionYAMLContext {
  MLIRContext *mlirContext;
};

struct HFusionOpMetadata {
  std::string name;
...
enum class HFusionOperandDefKind {
  InputTensor,
  Scalar,
  OutputTensor,
  IndexAttr,
...
enum class HFusionIteratorTypeDef {
  parallel,
  reduction,
};

struct HFusionIndexingMapsConfig {
...
struct HFusionStructuredOpConfig {
  SmallVector<HFusionOperandDef> args;
  HFusionIndexingMapsConfig indexingMaps;
  SmallVector<HFusionIteratorTypeDef> iteratorTypes;
  std::vector<ScalarAssign> assignments;
};

struct HFusionOpConfig {
  std::optional<HFusionOpMetadata> metadata;
  std::optional<HFusionStructuredOpConfig> structuredOp;
};
...
LLVM_YAML_IS_SEQUENCE_VECTOR(HFusionOperandDef)
LLVM_YAML_IS_SEQUENCE_VECTOR(SerializedAffineMap)
LLVM_YAML_IS_SEQUENCE_VECTOR(HFusionIteratorTypeDef)
LLVM_YAML_IS_SEQUENCE_VECTOR(ScalarAssign)
LLVM_YAML_IS_SEQUENCE_VECTOR(ScalarExpression)
LLVM_YAML_IS_DOCUMENT_LIST_VECTOR(HFusionOpConfig)
...
/// `HFusionStructuredOpConfig`).
template <> struct MappingTraits<HFusionOpConfig> {
  static void mapping(IO &io, HFusionOpConfig &info) {
...
///   - A list of named arguments (`HFusionOperandDef`), which can be inputs,
///     outputs, or index attributes.
///   - List of indexing maps (see `HFusionIndexingMaps`).
///   - Iterator types (see `HFusionIteratorTypeDef`).
///   - List of scalar level assignment (see `ScalarAssign`).
template <> struct MappingTraits<HFusionStructuredOpConfig> {
  static void mapping(IO &io, HFusionStructuredOpConfig &info) {
...
///     arguments.
template <> struct MappingTraits<HFusionOperandDef> {
  static void mapping(IO &io, HFusionOperandDef &info) {
...
template <> struct ScalarEnumerationTraits<HFusionOperandDefKind> {
  static void enumeration(IO &io, HFusionOperandDefKind &value) {
    io.enumCase(value, "input_tensor", HFusionOperandDefKind::InputTensor);
    io.enumCase(value, "scalar", HFusionOperandDefKind::Scalar);
    io.enumCase(value, "output_tensor", HFusionOperandDefKind::OutputTensor);
    io.enumCase(value, "index_attr", HFusionOperandDefKind::IndexAttr);
    io.enumCase(value, "unary_fn_attr", HFusionOperandDefKind::UnaryFnAttr);
    io.enumCase(value, "binary_fn_attr", HFusionOperandDefKind::BinaryFnAttr);
    io.enumCase(value, "compare_fn_attr", HFusionOperandDefKind::CompareFnAttr);
...
/// Iterator type enum.
template <> struct ScalarEnumerationTraits<HFusionIteratorTypeDef> {
  static void enumeration(IO &io, HFusionIteratorTypeDef &value) {
    io.enumCase(value, "parallel", HFusionIteratorTypeDef::parallel);
    io.enumCase(value, "reduction", HFusionIteratorTypeDef::reduction);
  }
};

/// Metadata about the op (name, C++ name, and documentation).
template <> struct MappingTraits<HFusionOpMetadata> {
  static void mapping(IO &io, HFusionOpMetadata &info) {
    io.mapRequired("name", info.name);
...
template <> struct MappingTraits<HFusionIndexingMapsConfig> {
  static void mapping(IO &io, HFusionIndexingMapsConfig &info) {
...
template <> struct ScalarEnumerationTraits<ScalarFnKind> {
  static void enumeration(IO &io, ScalarFnKind &value) {
    ...
    io.enumCase(value, "compare", ScalarFnKind::Compare);
...
///   - `add(lhs, rhs)`
///   - `mul(lhs, rhs)`
template <> struct MappingTraits<ScalarFn> {
...
public:
  GenerationContext(MLIRContext *context, raw_ostream *odsOut,
                    raw_ostream *defnOut)
      : context_(context), loc_(UnknownLoc::get(context)), odsOut_(odsOut),
        defnOut_(defnOut) {}

  MLIRContext *getContext() { return context_; }

  void setLoc(Location loc) { this->loc_ = loc; }
  Location getLoc() { return loc_; }

  bool shouldGenerateOds() { return odsOut_; }
  bool shouldGenerateDefns() { return defnOut_; }

  raw_ostream &odss() {
    assert(odsOut_ && "ODS stream not defined");
    return *odsOut_;
  }

  raw_ostream &defns() {
    assert(defnOut_ && "Definition stream not defined");
    return *defnOut_;
  }
...
findTensorDefArgIndex(StringRef name,
                      SmallVectorImpl<HFusionOperandDef> &args) {
...
findTypeValue(StringRef typeVar, SmallVectorImpl<HFusionOperandDef> &args) {
  ...
  for (const auto &it : llvm::enumerate(args)) {
    if (it.value().kind != HFusionOperandDefKind::InputTensor &&
        it.value().kind != HFusionOperandDefKind::Scalar &&
        it.value().kind != HFusionOperandDefKind::OutputTensor)
      continue;
...
static bool isFunctionAttribute(HFusionOperandDefKind kind) {
  return kind == HFusionOperandDefKind::UnaryFnAttr ||
         kind == HFusionOperandDefKind::BinaryFnAttr ||
         kind == HFusionOperandDefKind::CompareFnAttr ||
...
static bool isAttribute(HFusionOperandDefKind kind) {
  return kind == HFusionOperandDefKind::IndexAttr ||
         isFunctionAttribute(kind) || isBoolAttribute(kind);
}

// Get the enum name for the given operand kind.
std::string convertOperandKindToEnumName(HFusionOperandDefKind kind) {
  switch (kind) {
  case HFusionOperandDefKind::UnaryFnAttr:
    return std::string("UnaryFn");
  case HFusionOperandDefKind::BinaryFnAttr:
    return std::string("BinaryFn");
  case HFusionOperandDefKind::CompareFnAttr:
    return std::string("CompareFn");
  case HFusionOperandDefKind::TernaryFnAttr:
    return std::string("TernaryFn");
  case HFusionOperandDefKind::TypeFnAttr:
...
// of a hfusion.generic.
//===----------------------------------------------------------------------===//

// Template for HFusion named ops' ODS definitions. Parameters:
// {0}: ODS/C++ op name
...
def {0} : HFusionStructuredBase_Op<"{1}", !listconcat([AttrSizedOperandSegments],
  /*extraInterfaces=*/[{2}])> {
    {3}
    let arguments = (ins
...
ArrayAttr {0}::getIndexingMaps() {{
  static const char memoizeAttr[] = "hfusion.memoized_indexing_maps";
...
static LogicalResult generateNamedGenericOpOds(HFusionOpConfig &opConfig,
  ...
  if (llvm::any_of(opConfig.structuredOp->args, [](HFusionOperandDef &arg) {
    ...
    for (HFusionOperandDef &arg : opConfig.structuredOp->args) {
      ...
      // Add bool attributes to the op definition and builders.
      if (isBoolAttribute(arg.kind)) {
        static const char defFmt[] =
            "DefaultValuedOptionalAttr<BoolAttr, \"{0}\">:${1}";
        attrDefs.push_back(llvm::formatv(
            defFmt, llvm::formatv("{0}", arg.defaultFn), arg.name));
        attrParams.push_back(llvm::formatv(paramFmt, arg.name));
        attrStmts.push_back(llvm::formatv(stmtFmt, arg.name));
      }
      // Add the index attributes to the op definition and builders.
      if (arg.kind == HFusionOperandDefKind::IndexAttr) {
    ...
    if (llvm::any_of(opConfig.structuredOp->args, [](HFusionOperandDef &arg) {
          return arg.kind == HFusionOperandDefKind::IndexAttr;
        })) {
      attrMethods = R"(
...
generateNamedGenericOpDefns(HFusionOpConfig &opConfig,
                            GenerationContext &genContext) {
  ...
  int64_t numOfArgs =
      llvm::count_if(opConfig.structuredOp->args, [](HFusionOperandDef &arg) {
        return arg.kind == HFusionOperandDefKind::InputTensor ||
               arg.kind == HFusionOperandDefKind::Scalar ||
               arg.kind == HFusionOperandDefKind::OutputTensor;
      });
  ...
  if (!isRankPolymorphic) {
    ...
    llvm::interleaveComma(opConfig.structuredOp->iteratorTypes, ss,
                          [&](HFusionIteratorTypeDef it) {
                            switch (it) {
                            case HFusionIteratorTypeDef::parallel:
                              ss << "utils::IteratorType::parallel";
                              break;
                            case HFusionIteratorTypeDef::reduction:
...
static LogicalResult generateOp(HFusionOpConfig &opConfig,
                                GenerationContext &genContext) {
...
int main(int argc, char **argv) {
  llvm::cl::ParseCommandLineOptions(argc, argv, "HFusion ODS Gen from YAML");
  ...
  MLIRContext mlirContext;
  HFusionYAMLContext yamlContext{&mlirContext};

  std::vector<HFusionOpConfig> opConfigs;
