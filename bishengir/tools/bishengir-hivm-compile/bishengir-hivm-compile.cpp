//===- bishengir-hivm-compile.cpp - BiShengIR Compile Driver ----*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// Main entry function for bishengir-hivm-compile built as standalone binary.
//
//===----------------------------------------------------------------------===//
...
static std::string registerAndParseCLIOptions(int argc, char **argv) {
...
int main(int argc, char **argv) {
  ...
  if (!file) {
    llvm::errs() << "Error opening " << inputFile << ": " << errorMessage
                 << '\n';
    return EXIT_FAILURE;
  }
  ...
  if (!moduleRef) {
    llvm::errs() << "Error opening " << inputFile << '\n';
    return EXIT_FAILURE;
  }
  ...
  if (failed(bishengir::runBiShengIRHIVMPipeline(module, config))) {
    llvm::errs() << "Failed to run BiShengIR HIVM pipeline\n";
    return EXIT_FAILURE;
  }
